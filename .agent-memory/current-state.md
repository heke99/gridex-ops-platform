...
Renewed user instruction remains310/311 green→main→next masterplan-v2 phase. Actual01ee1d55 schema34999483634/job104484026625 identifies29 changed hashes,267 exact policy identities,0 missing/extra. Five retained full rows reproduce dequalification of auth.role() under the metadata witness search_path. Reviewed fix removes only auth from witness search_path and updates its SOURCE_SHA, keeping all267 policy hashes. Root qualification10 tests PASS; agent formula9/reference25 PASS.

Actual retained-delete34999483673 fails before behavior because fixture omitted later source-authored function search_path configuration. Reviewed fixture now replays exact June11 DO block before unchanged two function hashes/full rows;8 tests PASS. Residual job104484026825 has only stale four-forward expected counts; fixed607/520/595/six while exact601/514 preserved,20 tests and independent review PASS.

New independent portable final-invariant diagnostic runs144+514+6 and exact five final SQL checks on owned network-disabled PG17 without native/schema/type acceptance. Root/reviewer caught psql stdin -f - omission and corrected it before publication;6 tests PASS, independent review ACCEPT. It emits only closed rule/count/hash diagnostics after private cleanup. Existing full schema and retained-delete workflows also gain exact closedPR311 branch push triggers. This lets new fixes run without canceling PR31001ee1d55 native34999483575/job104484522245, now running. No history/reference/type manifest changes.
