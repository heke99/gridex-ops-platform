...
Active subtask: integrate the genuinely qualified native provider-role repair
and the existing portable/routine descendant, then complete native replay and types.
Also review remaining PRs without reintroducing obsolete code or temporary probes.

## Latest verified update — native provider roles

- The old ordinary native run35113974007 on58dfddba has ENDED with failure,
  not an ongoing job. It reached144/514/12, five final SQL checks and2496 actor
  cases, then stopped at REMOVED_POLICY_ROLE_GRAPH_REQUIRED; cleanup passed.
  No full-native schema or application-type artifact exists from that run.
- Run35123389668 reproduced this guard on a pristine official Supabase image.
  Native provider roles have an existing SET-only Storage/authenticator path.
- Run35126794371 passed the bounded repair on exact tree3b70eec7, publishing
  clean98afe8a2:74 regression tests, ten real invalid-role negatives, one exact
  PostgreSQL cycle rejection, rollback/recovery and genuine lifecycle cleanup.
  It does not certify the full historical schema or generated application types.
- Existing clean36d0f80b is merged without its temporary workflow/payloads.
  It preserves the separate portable Storage owner contract and two explicit
  reviewed function decisions. Its portable144/514/12 result was qualified in
  run35116624508; the final schema comparison was still DIFFERENT.
- Current source includes a dedicated PR role/boundary workflow. Historical SQL,
  the independent schema reference and generated-type manifest are unchanged.
- Next: run the complete native release chain on this exact integrated head;
  inspect terminal schema/type artifacts, resolve reviewed schema differences,
  adopt only real generated bytes and require current-head green CI before merge.
- PR310 and main are NOT merged by this integration. No hosted database mutation
  or external Ediel/transport message occurred. Evidence lives in
  quality/audits/ediel-masterplan-v2/pr310-native-provider-role-integration.md.

The sections below are earlier preserved history; this update supersedes any
older statement that35113974007 or a prior native replay is still running.
