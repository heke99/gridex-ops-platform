## 2026-09-25 — Active PR372 R1/R2 native qualification

Authoritative checkout is this branch. Published remote head `66b12f98695a91316814674daeea5326b195e355`, exact local tree `7e8456830c264c56e619267bd54ebb5c09430b57`; the local commit SHA differs because GitHub API publication uses the previous remote parent. OPS `36107605706` passed 325/325 native but stopped at expected schema snapshot drift; its direct budget probe may have warmed the processor. A new local projection forward and cold-order fixture await native replay. Previous head `8e7c6268` passed verify/quality but clean replay native 323/324: the 1,001-row processor fixture saved no qualification snapshot. A direct service RPC budget probe and historical alias regression plus forward alias migration are on `66b12f9`; inspect that run before another edit. Independent reviewer `/root/pr372_review` requested changes R1/R2 on older `16505294`; neither is independently reapproved. Task3b signed/legacy/retention, Task4 negatives and whole-diff review remain open. PR372 draft and unmerged; PR310 untouched. Preserve all later work.
