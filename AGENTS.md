...
2. Read `.agent-memory/current-state.md`, the sole active status, task, blockers,
   verification boundaries and next action.
3. Read `.agent-memory/checkpoint.json`, which is a continuity pointer only.
4. Read relevant domain-memory files.
5. Search `.agent-memory/decisions.md`.
6. Search `.agent-memory/known-failures.md`.
7. Inspect `git status` and `git diff` when Git metadata is available.
8. Inspect the actual implementation.
9. Continue from the recorded next action. Treat `.agent-memory/archive/` as
   historical context, never active evidence.
...
After every atomic subtask, inspect changes and run targeted verification. Update
`current-state.md` at a durable status transition or before session end, keep
`checkpoint.json` as a continuity pointer, record the exact next action, and
continue.

Before session end, update `current-state.md` and append durable evidence to
completed work, the verification matrix and the session log. Those evidence
files do not replace the sole current status. Keep checkpoint, handover and
blocker files as pointers to the current state. Never store secrets, production
customer data or raw credentials in project memory.
