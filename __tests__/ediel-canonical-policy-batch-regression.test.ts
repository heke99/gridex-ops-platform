...
describe('canonical Ediel policy batch regression', () => {
  ...
  it('preserves catalog coverage while root hints cannot qualify date/reporting/meter-change conditions', () => {
    ...
    // Preserve full-registry coverage and other determined hints; dates/reporting/meter-change need per-object evidence.
    ...
    for (const messageCode of EXPECTED_PRODAT_CODES) {
      const results = evaluateProdatDependentConditions({
        ...
        facts: {
          ...
          registerObjects: [{ meteringPointId: 'CATALOG-OBJECT', identityAgency: '89', expectedRegisterCount: 2, meterReadingsSentInUtilts: true }],
    ...
    expect(unknownIds.sort()).toEqual(['Z06:210','Z09:210','Z09:211','Z10:210','Z10:242','Z10:254','Z13:321','Z13:323','Z14:321','Z14:323'])
    const evaluatedIds = EXPECTED_PRODAT_CODES.flatMap((messageCode) =>
      evaluateProdatDependentConditions({
        ...
        facts: {
          ...
          registerObjects: [{ meteringPointId: 'CATALOG-OBJECT', identityAgency: '89', expectedRegisterCount: 2, meterReadingsSentInUtilts: true }], endUserAddressAvailable: true,
