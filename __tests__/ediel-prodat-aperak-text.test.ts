...
const observe=(id:string,m:ReturnType<typeof source>)=>{const d=resolveCanonicalRuntimeDecision(m),p=d.responsePlan.find(p=>p.family==='APERAK');let wire=null,error=null;try{if(p)wire=extract(buildAperakDraft({sourceMessage:m,outcome:p.outcome==='negative'?'negative':'positive',applicationErrors:p.applicationErrors}).rawPayload!)}catch(e){error=String(e)};const result={id,syntax:d.syntaxDecision,application:d.applicationDecision,disposition:d.prodatProcessingDisposition,issues:d.issues,plan:p,wire,error};output.push(result);return result}
