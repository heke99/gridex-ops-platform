...
const data=()=>({suite:'PRODAT',roleCode:'supplier',testCaseCode:'1.2.5',title:'Synthetic',sourceNote:'Synthetic',groups:['A','B'].map(id=>{
...
const run=()=>({id:'RUN',company_id:'tenant',role_code:'supplier',test_case_code:'1.2.5',test_suite:'PRODAT',notes:null} as any)
const facts=()=>({endUserAddressObjects:['A','B'].map(id=>({meteringPointId:id,identityAgency:'9',endUser:{id:'199001011234',qualifier:'SE2',agency:'260'},availability:'available',addressLines:['',`BOX ${id}`,"c/o :+?'"],source:{kind:'caller_selection',companyId:'tenant',reference:'synthetic-source'}}))} as any)
...
describe('TGT independent address source ownership',()=>{
 it('supports address facts without inventing register inventory',()=>{
  const c=saved();const result=readTgtRegisterFacts(c) as any
  expect(result.endUserAddressObjects.map((r:any)=>r.source)).toEqual(['A','B'].map(()=>expect.objectContaining({kind:'tgt',companyId:'tenant',runId:'RUN',stepNo:1,code:'Z03'})))
  expect(result.registerObjects).toBeUndefined()
 })
 for(const change of ['run','company','step','code','source'])it(`rejects changed ${change}`,()=>{
  const c=saved();if(change==='run')c.run.id='OTHER';if(change==='company')c.run.company_id='OTHER';if(change==='step')c.stepNo=2;if(change==='code')c.code='Z01';if(change==='source')c.testData.groups[0].fields.find((f:any)=>f.fieldCode==='229-2').values.Z03D='Changed'
 ...
 it('does not reinterpret absent source data as unavailable without an assertion',()=>{
  const c=ctx();for(const g of c.testData.groups)g.fields=g.fields.filter((f:any)=>!f.fieldCode.startsWith('229'))
 ...
 it('reads positional raw source before scalar sanitization',()=>{
  const rows=getPortalDataRows({testSuite:'PRODAT',roleCode:'supplier',testCaseCode:'1.2.5',importedTestData:data(),systemTestContext:{actorEdielId:'12345',testBrpEdielId:'11111'}} as any,{family:'PRODAT',code:'Z03',stepNo:1} as any)
  expect((rows[0] as any).customerAddressLines).toEqual(['','BOX A',"c/o :+?'"])
 })
})

const draftParams=()=>{const c=saved();return {actorUserId:'actor',testRunId:'RUN',testSuite:'PRODAT',roleCode:'supplier',testCaseCode:'1.2.5',stepNo:1,importedTestData:c.testData,registerFacts:readTgtRegisterFacts(c),
 systemTestContext:{companyId:'tenant',testSuite:'PRODAT',actorSettingId:null,actorEdielId:'12345',actorName:null,senderSubaddress:null,testPortalEdielId:'54321',testPortalName:null,testPortalEmail:null,defaultReceiverSubaddress:'PRODAT',testBrpEdielId:'11111',testBrpName:null,settings:null}} as any}
describe('actual authorized TGT draft and persisted row',()=>{
 it('carries distinct sparse addresses and source/run evidence for both objects',()=>{
  ...
  const m={company_id:'tenant',message_family:'PRODAT',message_code:'Z03',message_standard:'edifact',direction:'outbound',environment:'test',mime_type:'application/EDIFACT',raw_payload:d.rawPayload,parsed_payload:d.messageInput.parsedPayload} as any
 ...
 for(const mutation of ['run','company','source','caller'])it(`rejects draft ${mutation} ownership drift`,()=>{
  ...
  if(mutation==='source')p.importedTestData.groups[0].fields.find((f:any)=>f.fieldCode==='228').values.Z03D='Changed';
