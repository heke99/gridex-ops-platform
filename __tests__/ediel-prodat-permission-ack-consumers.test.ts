...
const manual=extract<(p:{sourceMessage:EdielMessageRow;actorUserId:string;roleCode:string;fallbackOutcome?:'positive'|'negative';fallbackApplicationErrors?:EdielAperakApplicationError[]})=>Promise<Result>>('app/admin/ediel/actions.part-3.ts',['resolveBackendAperakDecision'],{assertIncomingProdatEnergyProductReview,validateProdatPermissionMessage,resolveAndStoreProdatAperakErrors,resolveTgtTestDataForAckAction:async()=>({testData:null,selectedRow:null}),resolveProdatPermissionContextForAck:async()=>{state.contextCalls++;if(state.contextUnavailable)throw Error('SYNTHETIC_UNAVAILABLE');return {hasMatchingPriorPermissionFlow:true,matchReason:'synthetic'}},createEdielMessageEvent:async(e:Record<string,unknown>)=>state.events.push(e)})
