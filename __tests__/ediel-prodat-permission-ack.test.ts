...
for(const [code,reason,status,end] of [
 ...
 expect(c.syntaxDecision).toBe('accepted');expect(selected(c.responsePlan.applicationErrors??[])).toEqual([])
...
for(const [code,reason,status,end,errors] of [
 ...
 for(const errorsFound of [manual(m).applicationErrors,resolveCanonicalRuntimeDecision(m).responsePlan.applicationErrors??[],decideProdatAperak({message:m}).applicationErrors])expect(selected(errorsFound).map(e=>[e.ercCode,e.fieldCode])).toEqual(errors)
})
...
it('inapplicable fields and unknown subtype do not create selected errors',()=>{
 ...
 expect(selected(resolveCanonicalRuntimeDecision(m).responsePlan.applicationErrors??[])).toEqual([])}
