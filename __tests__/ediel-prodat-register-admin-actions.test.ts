...
vi.mock('@/app/admin/ediel/actions.part-4',()=>({parseInboundCaseMode:(v:unknown)=>typeof v==='string'?v:null}))
...
describe('actual admin action tenant and object decision boundaries',()=>{
 ...
 it('passes explicit object choices, not a shared root default',async()=>{
  ...
  expect(p.mode).toBeNull();expect(p.objectDecisions).toEqual(['A','B'].map(id=>({meteringPointId:id,identityAgency:'89',mode:'create_new_customer',selectedCustomerId:null,selectedSiteId:null,selectedMeteringPointId:null})))
 })
