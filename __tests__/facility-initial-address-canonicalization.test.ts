...
describe('facility initial address canonicalization migration', () => {
  it('does not treat the first canonical address hash as an address change', () => {
  ...
  it('keeps real established-address mutation fail-closed', () => {
    expect(migration).toContain("status = 'needs_review', stale_reason = 'site_address_changed_after_operation_started'")
    expect(migration).toContain("status = 'manual_review_required', blocker_reason = 'Anläggningsadressen ändrades. Skapa en ny nätägarresolution och begäran.'")
