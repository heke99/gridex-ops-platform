...
const websiteSpec = websiteOpenApi as unknown as OpenApiSpec
...
describe('market-price public API contract', () => {
  it('keeps runtime, registry and OpenAPI on version 2026-07-30.2', () => {
    ...
    expect(contract).toContain("WEBSITE_INTEGRATION_CONTRACT_VERSION = '2026-07-30.2'")
    expect(registry).toContain("path: '/api/v1/website/market-price/current'")
    expect(websiteSpec.info.version).toBe('2026-07-30.2')
    expect(customerPortalOpenApi.info.version).toBe('2026-07-30.2')
  })
  ...
  it('documents the current market-price endpoint only in website OpenAPI', () => {
    ...
    expect(response.properties.contract_schema_version?.const).toBe('2026-07-30.2')
  })
