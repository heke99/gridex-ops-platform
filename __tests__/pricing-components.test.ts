...
function component(overrides: Partial<PriceComponent> & Pick<PriceComponent, 'componentType' | 'name' | 'calculationType' | 'amount'>): PriceComponent {
...
describe('calculatePriceComponents', () => {
  it('converts an öre/kWh markup to SEK and applies 25 % VAT', () => {
    const { lines, errors } = calculatePriceComponents({
      ...
      components: [
        component({ componentType: 'markup_ore_per_kwh', name: 'Påslag', calculationType: 'per_kwh', amount: 5, unit: 'ore_per_kwh' }),
      ],
      baseAmountExVat: 550,
      vatRate: 0.25,
    })
  ...
  it('charges a monthly fee exactly once per billing period by default', () => {
    const { lines } = calculatePriceComponents({
      underlay: underlay(),
      components: [
        component({ componentType: 'fixed_monthly_fee', name: 'Månadsavgift', calculationType: 'fixed_monthly', amount: 49, unit: 'sek_month' }),
      ],
      baseAmountExVat: 550,
      vatRate: 0.25,
    })
  ...
  it('charges an invoice fee once', () => {
    const { lines } = calculatePriceComponents({
      underlay: underlay(),
      components: [
        component({ componentType: 'invoice_fee', name: 'Fakturaavgift', calculationType: 'fixed_once', amount: 29, unit: 'sek_invoice' }),
      ],
      baseAmountExVat: 550,
      vatRate: 0.25,
    })
  ...
  it('applies a per-kWh discount as a negative line with negative VAT', () => {
    const { lines } = calculatePriceComponents({
      underlay: underlay({ quantityKwh: 1000 }),
      components: [
        component({ componentType: 'discount_ore_per_kwh', name: 'Rabatt', calculationType: 'discount_per_kwh', amount: 2, unit: 'ore_per_kwh' }),
      ],
      baseAmountExVat: 550,
      vatRate: 0.25,
    })
  ...
  it('applies a fixed discount as a negative amount even when configured positive', () => {
    const { lines } = calculatePriceComponents({
      underlay: underlay(),
      components: [
        component({ componentType: 'campaign_discount', name: 'Kampanj', calculationType: 'discount_fixed', amount: 100 }),
      ],
      baseAmountExVat: 550,
      vatRate: 0.25,
    })
  ...
  it('computes percent_of_spot from the spot base amount', () => {
    const { lines, errors } = calculatePriceComponents({
      underlay: underlay(),
      components: [
        component({ componentType: 'spot_markup', name: 'Spotandel', calculationType: 'percent_of_spot', amount: 10 }),
      ],
      baseAmountExVat: 800,
      spotAmountExVat: 500,
      vatRate: 0.25,
  ...
  it('skips VAT for components marked not VAT applicable', () => {
    const { lines } = calculatePriceComponents({
      underlay: underlay(),
      components: [
        component({ componentType: 'admin_fee', name: 'Myndighetsavgift', calculationType: 'fixed_once', amount: 10, vatApplicable: false }),
      ],
      baseAmountExVat: 550,
      vatRate: 0.25,
    })
  ...
  it('reports unknown calculation types as warnings without producing a line', () => {
    const { lines, warnings } = calculatePriceComponents({
      underlay: underlay(),
      components: [
        component({ componentType: 'custom_addon', name: 'Konstig', calculationType: 'mystery_type', amount: 5 }),
      ],
      baseAmountExVat: 550,
      vatRate: 0.25,
    })
