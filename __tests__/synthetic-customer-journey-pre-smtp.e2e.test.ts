...
describe('synthetic customer journey stops immediately before external SMTP', () => {
  it('walks application -> legal evidence -> PDF -> all customer mail -> switch Z03 -> recipient certificate gate', () => {
    ...
    const resolverUse = smimeBranch.indexOf('await resolveOutboundRecipientCertificate')
    ...
    expect(fenceSource).toContain('if(!potential)return sendEdielEmail(input)')
    expect(fenceSource).toContain('await sendEdielEmail(input,{beforeProviderCall:')
    const helperSource = readModuleFamily('lib/email/sendEdielEmail.ts')
    const archiveUse = helperSource.indexOf('await archiveSmimeRawMime(input.raw)')
