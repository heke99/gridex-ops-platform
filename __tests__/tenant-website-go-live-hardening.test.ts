...
describe('tenant website canonical go-live hardening', () => {
  ...
  it('keeps admin UX aligned with canonical go-live instead of a generic Aktivera path', () => {
    ...
    const tenantBranchStart = apiClientsPage.indexOf('const tenantWebsite = isTenantWebsiteClient(client)')
    expect(tenantBranchStart).toBeGreaterThan(-1)
    const tenantBranch = apiClientsPage.slice(tenantBranchStart, tenantBranchStart + 2200)
    expect(tenantBranch).toContain('Sätt live / revalidera')
    expect(tenantBranch).toContain('goLiveHref(client.company_id)')
    expect(tenantBranch).not.toContain('>Aktivera<')
    expect(tenantBranch).not.toContain('value="active"')
