  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-100 px-5 py-4">
        ...
        <p className="mt-1 text-sm text-slate-500">Praktiska blockerare som operations ska kunna agera på utan att leta i flera menyer.</p>
      </div>
      {rows.length === 0 ? (
        <div className="px-5 py-8 text-sm text-slate-500">Inga tydliga blockerare i den här kön just nu.</div>
      ) : (
            ...
            <Link key={`${row.href}-${row.id}`} href={row.href} className="block px-5 py-4 transition hover:bg-emerald-50/50">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  {row.meta ? <p className="mt-1 text-xs text-slate-400">{row.meta}</p> : null}
                </div>
