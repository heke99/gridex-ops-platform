...
async function findBestSupplierSwitchRoute(params: {
  ...
  const { supabase, gridOwnerId, companyId } = params;
  ...
  const preferred = await supabase
    ...
    .select(
      "id, route_type, route_name, target_system, target_email, is_active",
    )
    .eq("is_active", true)
...
async function ensureOutboundForSwitch(params: {
  ...
  await createSupplierSwitchEvent(supabase, {
    ...
    companyId: requestRow.company_id ?? null,
  });
...
async function ensureSupplierRecord(params: {
  ...
  const saved = await saveElectricitySupplier(supabase, {
    ...
    contact_name: null,
    email: supplierEmail,
    customer_service_email: supplierEmail,
    switching_email: supplierEmail,
    contract_email: null,
    website: null,
    phone: supplierPhone,
...
export async function createDynamicSupplierSwitchRequestAction(
  ...
  const guard = await requireAdminActionAccess({ allOf: ["switching.write"] });
  ...
  if (site.customer_id !== customerId) {
    throw new Error("Anläggningen tillhör inte vald kund.");
  }
  ...
  const saveNewCurrentSupplier = checkboxValue(
    formData,
    "save_new_current_supplier",
  );
  const saveNewIncomingSupplier = checkboxValue(
    formData,
    "save_new_incoming_supplier",
  );
  ...
  if (
    !refreshedSite ||
    refreshedSite.company_id !== companyId ||
    refreshedSite.customer_id !== customerId
  ) {
    throw new Error("Kunde inte läsa uppdaterad anläggning i rätt bolag.");
  }
  ...
  const [meteringPoints, powersOfAttorney] = await Promise.all([
    listMeteringPointsForSite(supabase, siteId).then((rows) =>
      rows.filter((row) => row.company_id === companyId),
    ),
    listPowersOfAttorneyByCustomerId(supabase, customerId, { companyId }),
  ]);
  ...
  const { data: requestRow, error: requestError } = await supabase
    ...
    .insert({
      ...
      current_supplier_name:
        currentSupplierName ?? refreshedSite.current_supplier_name,
      current_supplier_org_number:
