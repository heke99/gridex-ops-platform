...
function collectAperakApplicationErrors(
  formData: FormData,
...
function parseEdielTestRoleCode(
  value: FormDataEntryValue | null,
): EdielTestRoleCode {
  const raw = formString(value);
...
type BackendProdatAperakDecision = {
  outcome: AckOutcome;
...
const REPLACEABLE_TGT_ACK_STATUSES = new Set([
  "draft",
  "queued",
  "prepared",
  "failed",
...
function parseLineItemReferencesByZ07(
...
function deriveS142LineItemReferences(
  sourcePayload?: string | null,
): EdielAperakApplicationError[] {
...
async function prepareSwitchProdatAction(
  formData: FormData,
  messageCode: ProdatSwitchCode,
) {
  const context = await requireAnyPermissionServer([
...
function normalizeCustomerIdQualifier(
  value: string | null,
): "SE1" | "SE2" | "1" | null {
