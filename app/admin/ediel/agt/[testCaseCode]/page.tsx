...
function Badge({ tone, children }: { tone: Tone; children: ReactNode }) {
  return (
    <span
      className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${badgeTone(tone)}`}
    >
      {children}
    </span>
  );
...
function statusTone(status: string | null | undefined): Tone {
  ...
  if (
    ["sent", "received", "acknowledged", "processed", "success"].includes(value)
  )
    return "emerald";
  if (
    [
    ...
    return "amber";
  if (["failed", "cancelled", "error", "rejected"].includes(value))
    return "red";
  return "slate";
}
...
function directionText(testCase: EdielAgtTestCaseDefinition) {
  return testCase.direction === "actor_to_portal"
    ? "Leverantör → Edielportalen"
    : "Edielportalen → Leverantör";
}

function isAckLikeStep(
  step: EdielAgtTestCaseDefinition["expectedSteps"][number],
): boolean {
...
function messageMatchesExpectedStep(
  step: EdielAgtTestCaseDefinition["expectedSteps"][number],
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {message.ack_outcome ? (
          <Badge tone={message.ack_outcome === "positive" ? "emerald" : "red"}>
            {message.ack_outcome}
          </Badge>
        ) : null}
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-700">
            Förväntade steg och kopplade meddelanden för just detta test.
          </p>
        </div>
        <Badge tone={run ? statusTone(run.status) : "slate"}>
          {run ? `run ${run.status}` : "ingen run"}
        </Badge>
      </div>
      ...
      <div className="mt-4 space-y-3">
        {testCase.expectedSteps.map((step) => {
          const validLinked = links
            .map((link) => ({
              link,
              message: messagesById.get(link.ediel_message_id),
            }))
            .filter(
              (
                item,
                ...
                return (
                  item.link.step_no === step.stepNo &&
                  message.status !== "cancelled" &&
                  messageMatchesExpectedStep(step, message)
                );
              },
            )
            .sort(
              (a, b) =>
                Date.parse(b.message.created_at ?? "") -
                Date.parse(a.message.created_at ?? ""),
            );
          const visibleLinked = validLinked.slice(0, 1);
          const hiddenCount = Math.max(
            validLinked.length - visibleLinked.length,
            0,
          );
          return (
            <div
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <div className="text-sm font-semibold text-slate-950">
                    Steg {step.stepNo}: {step.title}
                  </div>
                  <div className="mt-1 text-xs text-slate-700">
                    {step.actor} · {step.direction} · {step.family} {step.code}
                  </div>
                </div>
                <Badge tone={visibleLinked.length > 0 ? "emerald" : "slate"}>
                  {visibleLinked.length > 0 ? "kopplad" : "väntar"}
                </Badge>
              </div>
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {hiddenCount > 0 ? (
                    <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-semibold text-amber-800">
                      {hiddenCount} äldre/duplicerade kopplingar är dolda.
                      Använd rensningsknappen om du vill makulera gamla
                      oskickade meddelanden för detta test.
                    </div>
                  ) : null}
                </div>
              ) : null}
...
export default async function AgtCasePage({
  ...
  const messagesById = new Map(
    linkedMessages.map((message) => [message.id, message]),
  );
  ...
  const candidateInbound = recentInbound
    .filter((message) =>
      testCase.direction === "portal_to_actor"
        ...
        : canAttachToCase(testCase, message),
    )
  ...
  const linkedSourceIds = Array.from(
    new Set(
      linkedMessages
        .filter((message) => isPrimaryInbound(testCase, message))
  ...
  const route =
    testCase.suite === "PRODAT" ? runtime.prodat.route : runtime.utilts.route;
  const profile =
    testCase.suite === "PRODAT"
      ? runtime.prodat.profile
      : runtime.utilts.profile;
  const portalEdielId =
    runtime.systemTestSettings?.testPortalEdielId ??
    profile?.receiver_ediel_id ??
    null;
  const portalSmtp =
    runtime.systemTestSettings?.testPortalEmail ?? route?.target_email ?? null;
  const receiverSubaddress =
    runtime.systemTestSettings?.defaultReceiverSubaddress ??
  ...
  return (
    <div className="space-y-6">
      <AdminHeader
        ...
        subtitle={
          companyId
            ? "Tenantstyrd AGT-motor. Alla runs, routes och payloads är filtrerade till valt bolag."
            : "Plattformens AGT-motor. Välj bolag från aktörstestkortet för tenantstyrd körning."
        }
        userEmail={context.email}
        workspaceName="Plattformskontroll"
        workspaceMode="platform"
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex flex-wrap gap-2">
              ...
              <Badge tone={actorToPortal ? "amber" : "emerald"}>
                {directionText(testCase)}
              </Badge>
            </div>
            <h1 className="mt-3 text-2xl font-semibold text-slate-950">
              {testCase.portalTitle}
            </h1>
          ...
          <div className="flex flex-wrap gap-2">
            <Link
              href={
                companyId
                  ? `/admin/platform/actor-testing/${companyId}`
                  : "/admin/ediel/agt"
              }
      ...
      <section className="grid gap-4 lg:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="text-xs font-semibold uppercase tracking-wide text-slate-700">
            Aktiv tenant/leverantör
          </div>
          <div className="mt-1 text-sm font-semibold text-slate-950">
            {runtime.actor?.actor_name ?? "—"}
          </div>
          <div className="mt-1 font-mono text-xs text-slate-700">
            {runtime.actor?.actor_ediel_id ?? "saknas"}
          </div>
          <div className="mt-1 font-mono text-[11px] text-slate-500">
            {companyId ? `company_id ${companyId}` : "global vy"}
          </div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="text-xs font-semibold uppercase tracking-wide text-slate-700">
            DB-konfigurerad testportal
          </div>
          <div className="mt-1 text-sm font-semibold text-slate-950">
            {portalEdielId ?? "saknas"}
          </div>
          <div className="mt-1 text-xs text-slate-700">
            {portalSmtp ?? "saknar mottagarmail"}
          </div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="text-xs font-semibold uppercase tracking-wide text-slate-700">
          ...
          <div className="mt-1 text-sm font-semibold text-slate-950">
            {route?.route_name ?? "saknas"}
          </div>
          <div className="mt-1 text-xs text-slate-700">
            {profile?.encryption_mode ?? "—"} · {profile?.ack_mode ?? "—"}
          </div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="text-xs font-semibold uppercase tracking-wide text-slate-700">
          ...
          <div className="mt-1 text-sm font-semibold text-slate-950">
            {run ? run.status : "ingen aktiv run"}
          </div>
          <div className="mt-1 text-xs text-slate-700">
            {run ? formatDate(run.created_at) : "skapa run först"}
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          {testCase.prodatOutboundTemplate ? (
            <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm leading-6 text-emerald-900">
              <div className="font-semibold text-emerald-950">
                2026A outbound-template
              </div>
              <div className="mt-1">
                GridCore skickar bara om payloaden matchar vald AGT-template.
                För detta test: 223/Z13 ={" "}
                <span className="font-mono font-semibold">
                  {testCase.prodatOutboundTemplate.reasonForTransaction}
                </span>{" "}
                och 217/Z04 ={" "}
                <span className="font-mono font-semibold">
                  {testCase.prodatOutboundTemplate.meteringMethod}
                </span>
                .
              </div>
            </div>
            ...
            {actorToPortal ? (
              ...
              <form action={pollAgtMailboxForCaseAction}>
                ...
                <input
                  type="hidden"
                  name="mailbox"
                  value={runtime.actor?.mailbox ?? "INBOX"}
                />
          ...
          <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm leading-6 text-amber-900">
            För Aktör → Portal genererar GridCore nu endast ett
            prepared/draft-meddelande och öppnar meddelandesidan. Där
            kontrollerar du raw payload, UNB, BGM, NAD, CCI/CAV och referenser
            innan du klickar Skicka. För Portal → Aktör skapas kvittensförslag
            från inbound-raden, men även dessa ska öppnas som payload innan
            skick. Produktion/live-flöden ska inte gå via den här testvyn.
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-700">
            Använd detta om IMAP inte hunnit hämta meddelandet eller om du vill
            klistra in EDIFACT från portalen. För Portal → Aktör skapar motorn
            AGT-svar från portalens affärsmeddelande. För Aktör → Portal kopplas
            endast portalens CONTRL/APERAK till auditkedjan.
          </p>
          <form
            action={importAgtRawInboundForCaseAction}
      ...
      {linkedAckPairs.some((pair) => pair.acks.length > 0) ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-5">
          <h2 className="text-lg font-semibold text-slate-950">
            Skapade kvittensförslag för kopplade inbound
          </h2>
          <div className="mt-4 space-y-3">
            {linkedAckPairs
              .flatMap((pair) => pair.acks)
              .map((ack) => (
                <div
                  key={ack.id}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold text-slate-950">
              Möjliga inbound att koppla
            </h2>
            <p className="mt-1 text-sm text-slate-700">
              Senaste inbound som matchar förväntad familj/kod för{" "}
        ...
        <div className="mt-4 space-y-3">
          {candidateAckPairs.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-700">
              Inga matchande inbound hittades än. Starta testet i Edielportalen
              och använd IMAP-importen ovan.
            </div>
          ) : (
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-2 text-xs text-slate-700">
                      Mottaget:{" "}
                      {formatDate(
                        message.message_received_at ?? message.created_at,
                      )}
                    </div>
                    {isPrimaryInbound(testCase, message) ? (
                      <div className="mt-2 text-xs font-semibold text-emerald-700">
                        Detta är portalens affärsmeddelande för testet. Motorn
                        skapar rätt kvittensförslag.
                      </div>
                    ) : (
                      <div className="mt-2 text-xs font-semibold text-emerald-700">
                        Detta är portalens kvittens/svarsmeddelande för ett
                        outbound-test och kan kopplas till testkedjan.
                      </div>
                    )}
                  </div>
                  <div className="flex flex-wrap gap-2">
      ...
      <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-sm leading-6 text-emerald-900">
        <div className="font-semibold text-emerald-950">
          SaaS-regel för detta fönster
        </div>
        <p className="mt-1">
          Testfallet styrs av AGT-registret och aktiv tenant-runtime. Inga
          tenantnamn ska ligga som specialfall i motorn. För andra leverantörer
          byts Ediel-id, mailbox, routes, eventuell sender-subadress och BRP i
          runtime.
        </p>
        {testCase.suite === "PRODAT" ? (
          <p className="mt-2">
            PRODAT AGT mot portalen använder DB-konfigurerad receiver{" "}
            {portalEdielId ?? "saknas"}:ZZ:{receiverSubaddress ?? "saknas"}.
            Sender-subadress ska följa Edielregistret per tenant och sparas på
            aktörskortet.
          </p>
        ) : null}
      </section>
