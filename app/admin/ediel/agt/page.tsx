  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6">
        ...
        >
          ...
          <div className="rounded-2xl border border-slate-200 p-4">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <label className="text-sm text-slate-700">
                ...
                <input
                  ...
                  className={inputClassName()}
                  placeholder="t.ex. portalens registrerade testadress"
                />
              </label>
              <label className="text-sm text-slate-700">
            ...
            <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800">
              Knappen skapar/uppdaterar aktiv test-aktör, DB-konfigurerad
              systemtestportal/test-BRP, PRODAT-route och UTILTS-route.
              Testportalens Ediel-ID sparas i databasen och får inte hårdkodas i
              builder eller runtime. Balansansvarig Ediel-ID sparas och används
              som NAD+Z02 i L1/L7.
            </div>
          </div>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <RouteCard
          title="PRODAT AGT route"
          family="PRODAT"
          route={runtime.prodat.route}
          profile={runtime.prodat.profile}
          portalEdielId={portalEdielId}
          receiverSubaddress={receiverSubaddress}
        />
        <RouteCard
          title="UTILTS AGT route"
          family="UTILTS"
          route={runtime.utilts.route}
          profile={runtime.utilts.profile}
          portalEdielId={portalEdielId}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6">
        <div className="mb-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-700">
                Kör ett test åt gången. L1/L7 genererar först
                draft/prepared-payload som öppnas för kontroll innan du skickar.
                L2–L5 ska vänta på inbound från Edielportalen.
              </p>
            </div>
            <Badge tone="emerald">testläge separat från produktion</Badge>
          </div>
          <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-700">
            Den här sidan ska bara användas för AGT. Verkliga kundflöden skapas
            från kundkort, operations och live Ediel-meddelanden.
          </div>
        </div>
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          {EDIEL_AGT_SUPPLIER_2026A_CASES.map((testCase) => {
            ...
            return (
              <div
                key={`${testCase.suite}-${testCase.testCaseCode}`}
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-slate-700">
                      {testCase.suite} · {testCase.messageCode} ·{" "}
                      {directionLabel(testCase.direction)}
                    </div>
                  </div>
                  <Badge tone={caseTone(hasRun)}>
                    {hasRun ? `run ${activeRun?.status}` : "ej skapad"}
                  </Badge>
                </div>
                <p className="mt-3 text-sm leading-6 text-slate-700">
                  {notesText(testCase.notes)}
                </p>
                ...
                {actorToPortal ? (
                  <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs leading-5 text-emerald-900">
                    ...
                    <div>
                      UNB sender: leverantörens Ediel-id + registrerad
                      sender-subadress om den finns.
                    </div>
                    <div>
                      UNB receiver: {portalEdielId ?? "—"}
                      {receiverSubaddress ? `:ZZ:${receiverSubaddress}` : ""}
                    </div>
                    <div>
                      L1/L7 ska innehålla NAD+Z02 enligt portalens validering.
                      Fyll i balansansvarig/BRP Ediel-id innan du skickar
                      outbound.
                    </div>
                  </div>
                ) : (
                  <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs leading-5 text-emerald-900">
                    ...
                    <div>
                      Starta testet i Edielportalen och importera inbound{" "}
                      {testCase.messageCode}. Skapa sedan CONTRL + APERAK från
                      inbound-raden.
                    </div>
                  </div>
                )}
      ...
      <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-emerald-900">
          Spara AGT-runtime först. Starta L1 i Edielportalen, skapa L1-run här,
          fyll i balansansvarig/BRP Ediel-id och generera draften. Kontrollera
          payloaden på meddelandesidan innan du klickar Skicka. L2-L5 är Portal
          → Aktör: starta testet i portalen, importera inbound PRODAT och skapa
          CONTRL + APERAK från inbound-raden. Öppna även kvittenspayload innan
          skick. Kör L7 sist som outbound Z09-draft.
        </p>
      </section>
    </div>
  );
