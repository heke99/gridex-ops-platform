...
function JsonBlock({ value }: { value: unknown }) {
 return (
 <pre className="overflow-x-auto rounded-2xl bg-emerald-700 p-4 text-xs text-slate-100">
...
function renderVersionWindow(window: ResolvedVersionWindow | null) {
 ...
 <div className="text-xs uppercase tracking-wide text-slate-700">Selected version</div>
 <div className="mt-2 text-sm text-slate-900">{window.selectedVersion ?? '—'}</div>
 </div>
 <div className="rounded-2xl border border-slate-200 p-4">
 <div className="text-xs uppercase tracking-wide text-slate-700">Current / previous</div>
 ...
 <div className="text-xs uppercase tracking-wide text-slate-700">Accepted versions</div>
 <div className="mt-2 flex flex-wrap gap-2">
 {window.acceptedVersions.length > 0 ? (
 window.acceptedVersions.map((version) => <Pill key={version} text={version} />)
 ) : (
 <span className="text-sm text-slate-700">Inga accepted versions rapporterade.</span>
 )}
 </div>
 </div>
 </div>
...
export default async function AdminEdielMessageDetailPage({
 ...
 <AdminHeader
 title="Ediel message"
 subtitle="Live-meddelanden är låsta tills superadmin har aktiverat live."
 ...
 <pre className="mt-2 max-h-[360px] overflow-auto rounded-2xl bg-emerald-700 p-4 text-xs text-slate-100">
 ...
 <div className="rounded-2xl border border-slate-200 p-4">
 <div className="text-xs uppercase tracking-wide text-slate-700">Ack deadline</div>
 ...
 <div className="rounded-2xl border border-slate-200 p-4">
 <div className="text-xs uppercase tracking-wide text-slate-700">Checks</div>
 <div className="mt-2 space-y-1 text-sm text-slate-700">
 ...
 <div className="mt-4 text-sm text-slate-700">Ingen route runtime hittades.</div>
 )}
 {routeRuntime ? (
 <div className="mt-4">
