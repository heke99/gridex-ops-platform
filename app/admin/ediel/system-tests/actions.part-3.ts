...
import { createEdielTestRun, getEdielMessageById, listEdielTestRuns, updateEdielMessageStatus, updateEdielTestRunStatus } from "@/lib/ediel/db"
...
import { autoAttachImportedMessageToActiveTgtRun, runTgtAutopilotForRun } from "@/lib/ediel/testing/tgtAutopilot"
...
import { errorMessage, formFileText, formString, normalizeCode, revalidateSystemTests, safeInsert, safeUpsert } from './actions.part-1'
import { listRecentMessagesForSystemTest } from './actions.part-2'
...
export async function pollAndSyncTgtSystemTestMailboxAction(
  ...
  if (!companyId) throw new Error("Välj bolag innan systemtestets IMAP-poll körs");
  ...
  try {
    const existingRuns = await listEdielTestRuns({ scope: "tenant", companyId });
...
export function isSystemTestOutboundBusinessMessageSendable(
...
export async function findLinkedSystemTestOutboundBusinessMessage(params: {
  testRunId: string;
  preferredMessageId?: string | null;
}): Promise<EdielMessageRow | null> {
  if (params.preferredMessageId) {
    const preferred = await getEdielMessageById(params.preferredMessageId);
  ...
  const messageIds = Array.from(
    new Set(
      (links ?? [])
        .map((link: { ediel_message_id?: string | null }) => String(link.ediel_message_id ?? "").trim())
        .filter(Boolean),
    ),
  );
  ...
  const stepByMessageId = new Map<string, number>(
    (links ?? []).map((link: { ediel_message_id?: string | null; step_no?: number | null }) => [
...
export async function markSystemTestOutboundSendIntent(params: {
  actorUserId: string;
