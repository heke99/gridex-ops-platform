...
function validateSecretReference(value: string): string {
  ...
  if (!normalized.startsWith('env:')) {
    throw new Error('Secret reference måste peka på env, t.ex. env:GRIDEX_SHARED_EDIEL_IMAP_PASS.')
  }
...
export async function saveSharedMailboxProfileAction(formData: FormData) {
  ...
  const secretReference = validateSecretReference(requiredText(formData, 'secret_reference', 'Secret reference'))
  ...
  const query = existing.data?.id
    ? supabaseService.from('ediel_mailboxes').update(payload).eq('id', existing.data.id)
