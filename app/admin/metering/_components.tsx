...
export function meteringTone(status: string): string {
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 px-6 py-5">
        <h2 className="text-lg font-semibold text-slate-950">Requestkö mot nätägare</h2>
            ...
            <div key={request.id} className="rounded-2xl border p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex flex-wrap items-center gap-2">
                  <span
                    className={`rounded-full px-3 py-1 text-xs font-semibold ${meteringTone(
                      request.status
                    )}`}
                  >
                    {request.status}
              ...
              <div className="mt-3 grid gap-2 text-sm text-slate-600">
                ...
                <div>
                  Nätägare: <span className="font-medium">{request.grid_owner_id ?? '—'}</span>
                </div>
                <div>
                  Period:{' '}
...
export function MeteringValuesTable({
  values,
}: {
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-slate-50">
            <tr className="border-b border-slate-200 text-left">
              ...
              <th className="px-6 py-4 text-right font-semibold text-slate-600">Öppna</th>
            </tr>
          </thead>
          <tbody>
            {values.length === 0 ? (
              ...
              values.slice(0, 50).map((value) => (
                <tr key={value.id} className="border-b border-slate-100">
                  <td className="px-6 py-4 text-slate-700">
                    {value.read_at ? new Date(value.read_at).toLocaleString('sv-SE') : '—'}
                  </td>
