...
import { getCustomerGrowthSeries, getDeviationRows, getLatestForecastByBiddingZone, getLatestForecastByGridOwner, getMonthlyMetric } from '@/lib/analytics/db'
import { formatMwh, formatNumber, monthStart } from '@/lib/analytics/utils'
import { DeviationsTable, ForecastTable, GridOwnerForecastTable, SimpleBars } from '../../../../analytics/_components'
import { rebuildCompanyAnalyticsAction, runCompanyForecastAction } from '../../../analytics/actions'
...
export default async function PlatformCompanyAnalyticsPage({ params }: PageProps) {
  ...
  const [metric, growth, forecast, gridOwnerForecast, deviations] = await Promise.all([
    getMonthlyMetric(companyId, month),
    getCustomerGrowthSeries(companyId, month),
    getLatestForecastByBiddingZone(companyId, { month }),
    getLatestForecastByGridOwner(companyId, { month }),
    ...
    <div className="min-h-screen">
      ...
      <div className="space-y-6 p-4 sm:p-6 xl:p-8">
        <div className="flex flex-wrap gap-3">
          ...
          <form action={rebuildCompanyAnalyticsAction}>
            <input type="hidden" name="companyId" value={companyId} />
            <button className="rounded-2xl bg-emerald-700 px-4 py-2 text-sm font-black text-white hover:bg-emerald-800">Bygg om analytics</button>
          </form>
          <form action={runCompanyForecastAction}>
            <input type="hidden" name="companyId" value={companyId} />
            <button className="rounded-2xl border border-emerald-200 bg-white px-4 py-2 text-sm font-black text-emerald-800 hover:bg-emerald-50">Kör prognos</button>
          </form>
        </div>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5"><GridOwnerForecastTable rows={gridOwnerForecast} /></div>
        </section>
