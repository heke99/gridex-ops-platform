...
async function getProductionActorSetting(companyId: string): Promise<{
  ...
  if (!row || !edielId) {
    throw new Error(
      "Bolaget saknar aktivt production Ediel-ID i ediel_actor_settings. Lägg in Ediel-ID i Company → Ediel & Go-live innan route skapas.",
    );
  }
...
function validateProductionRoute(input: {
  ...
  if (!input.targetEmail)
    blockers.push(
      "Gridex shared production mailbox saknas. Lägg upp plattformens production-transport innan PRODAT aktiveras.",
    );
  if (
    ![
...
export async function createProductionRouteFromWizardAction(
  ...
  if (frontendSenderEdielId && frontendSenderEdielId !== actorSetting.edielId) {
    throw new Error(
      "Sender Ediel-ID får inte override:as i route-wizard. Ändra bolagets Ediel-ID i Company → Ediel & Go-live först.",
    );
  }
  ...
  const receiverSource: string = "selected_metering_point_grid_owner";
  const dynamicReceiverStrategy =
    defaultDynamicReceiverStrategy(receiverSource);
  ...
  const applicationReference = "PRODAT";
  const defaultMessageVersion = "26A";
  const encryptionMode = "smime";
  ...
  if (blockers.length > 0) {
    try {
      const { error: wizardRunError } = await supabaseService
      ...
      if (wizardRunError) {
        console.warn(
          "Production route wizard blocked run could not be logged",
          wizardRunError,
        );
      }
    } catch (error) {
  ...
  const { data: route, error: routeError } = await supabaseService
    ...
    .insert({
      ...
      auth_config: {},
      supported_payload_version: defaultMessageVersion,
      notes:
        text(formData, "notes") ??
        "Skapad via go-live. Receiver löses automatiskt från kundens nätägare/process. Separat från test/certifiering.",
      created_by: admin.userId,
  ...
  const { data: profile, error: profileError } = await supabaseService
    ...
    .insert({
      ...
      default_message_version: defaultMessageVersion,
      default_test_flag: 0,
      default_timezone: 1,
  ...
  try {
    ...
    if (wizardRunError) {
      console.warn(
        "Production route wizard created run could not be logged",
        wizardRunError,
      );
    }
  } catch (error) {
  ...
  const params = new URLSearchParams({
    status: "created",
    message:
      "Produktionsprofil för PRODAT skapades. Mottagare löses automatiskt från kundens nätägare/process och Gridex shared mailbox används som transport.",
  });
  redirect(
    `/admin/platform/go-live/${companyId}/route-wizard?${params.toString()}`,
  );
