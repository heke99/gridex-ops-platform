...
import { isPriceArea, type BasePriceComponent, type BillingUnderlayInput, type PriceComponent } from '@/lib/pricing/types'
...
function month(value: string): string {
  if (!/^\d{4}-\d{2}$/.test(value)) throw new Error('Månad måste anges som YYYY-MM.')
...
function price(value: string): number {
  ...
  if (!Number.isFinite(parsed) || parsed < 0 || parsed > 100) {
    throw new Error('Portföljpriset måste vara ett positivt tal i kr/kWh exklusive moms.')
  }
  return parsed
}

async function assertCompany(companyId: string) {
  const { data, error } = await supabaseService.from('companies').select('id,name').eq('id', companyId).maybeSingle()
...
export async function savePortfolioPriceAction(formData: FormData) {
  ...
  const priceExVat = price(text(formData, 'price_ex_vat_sek_per_kwh'))
  const notes = text(formData, 'notes') || null
  const requestedStatus = text(formData, 'status') === 'confirmed' ? 'confirmed' : 'draft'
  ...
  const { data: existing, error: existingError } = await supabaseService
    ...
    .maybeSingle()
  if (existingError && !['42703', 'PGRST204'].includes(String((existingError as { code?: string }).code ?? ''))) throw existingError

  const now = new Date().toISOString()
  if (existing) {
    if (existing.status === 'locked') throw new Error('Priset är låst. Skapa en korrigering i stället för att ändra den låsta versionen.')
    const payload = {
    ...
    const { data, error } = await supabaseService
      ...
      .single()
    if (error) throw error
    await audit({ companyId, actorUserId: actor.userId, entityId: String(existing.id), action: 'portfolio_price_updated', oldValues: existing, newValues: data })
  } else {
    const payload = {
    ...
    const { data, error } = await supabaseService.from('portfolio_monthly_prices').insert(payload).select('*').single()
    if (error) throw error
    await audit({ companyId, actorUserId: actor.userId, entityId: String(data.id), action: 'portfolio_price_created', newValues: data })
  }
  revalidate()
}
...
export async function transitionPortfolioPriceAction(formData: FormData) {
  ...
  if (!['confirm', 'lock'].includes(transition)) throw new Error('Ogiltig prisövergång.')
  ...
  if (transition === 'lock' && current.status !== 'confirmed') throw new Error('Priset måste bekräftas innan det kan låsas.')

  const now = new Date().toISOString()
  const payload = transition === 'confirm'
    ? { status: 'confirmed', confirmed_at: now, approved_at: now, approved_by: actor.userId, updated_by: actor.userId, updated_at: now }
    : { status: 'locked', locked_at: now, updated_by: actor.userId, updated_at: now }
  const { data, error } = await supabaseService
    .from('portfolio_monthly_prices')
    .update(payload)
    .eq('id', id)
    ...
    .single()
  if (error) throw error
  await audit({ companyId, actorUserId: actor.userId, entityId: id, action: transition === 'confirm' ? 'portfolio_price_confirmed' : 'portfolio_price_locked', oldValues: current, newValues: data })
  revalidate()
}
...
export async function createPortfolioPriceRevisionAction(formData: FormData) {
  ...
  const { data: current, error: readError } = await supabaseService
    ...
    .single()
  if (readError) throw readError
  if (!['confirmed', 'locked'].includes(String(current.status))) throw new Error('Endast bekräftade eller låsta priser behöver en ny korrigeringsversion.')

  const now = new Date().toISOString()
  const { error: supersedeError } = await supabaseService
    .from('portfolio_monthly_prices')
    .update({ status: 'superseded', superseded_at: now, updated_by: actor.userId, updated_at: now })
    .eq('id', id)
    .eq('company_id', companyId)
  ...
  const { data: revision, error: insertError } = await supabaseService
    ...
    .insert({
      ...
      status: 'draft',
      source: 'manual',
      notes: `Korrigering av version ${current.version_number ?? 1}. ${current.notes ?? ''}`.trim(),
  ...
  if (insertError) {
    await supabaseService
      .from('portfolio_monthly_prices')
      .update({ status: current.status, superseded_at: null, updated_by: actor.userId, updated_at: new Date().toISOString() })
      .eq('id', id)
      .eq('company_id', companyId)
    throw insertError
  }
  await audit({ companyId, actorUserId: actor.userId, entityId: String(revision.id), action: 'portfolio_price_revision_created', oldValues: current, newValues: revision, metadata: { supersedes_id: current.id } })
  revalidate()
}
...
export async function importPortfolioPricesAction(formData: FormData) {
  ...
  if (lines.length > 100) throw new Error('Max 100 rader kan importeras åt gången.')
  ...
  for (const [index, line] of lines.entries()) {
    ...
    data.set('price_ex_vat_sek_per_kwh', priceRaw ?? '')
    data.set('notes', notesRaw ?? `Importerad rad ${index + 1}`)
    data.set('status', 'draft')
    await savePortfolioPriceAction(data)
  }
  await audit({ companyId, actorUserId: actor.userId, entityId: companyId, action: 'portfolio_prices_imported', metadata: { row_count: lines.length } })
  revalidate()
}
...
export type PortfolioPricePreviewState = {
  ...
  annualIncVat?: number
  lines?: Array<{ description: string; amountExVat: number; amountIncVat: number }>
}
...
export async function previewPortfolioPriceAction(
  ...
  try {
    ...
    const portfolioPrice = price(text(formData, 'portfolio_price'))
    const markupOre = Number(text(formData, 'markup_ore').replace(',', '.') || '0')
    const monthlyFee = Number(text(formData, 'monthly_fee').replace(',', '.') || '0')
    if (!Number.isFinite(annualKwh) || annualKwh <= 0 || annualKwh > 10_000_000) {
      throw new Error('Årsförbrukningen måste vara större än 0 och högst 10 000 000 kWh.')
    }
    if (!Number.isFinite(markupOre) || markupOre < 0 || markupOre > 10_000) throw new Error('Påslaget är ogiltigt.')
    if (!Number.isFinite(monthlyFee) || monthlyFee < 0 || monthlyFee > 1_000_000) throw new Error('Månadsavgiften är ogiltig.')
    ...
    const priceComponents = snapshot.priceComponents.map((row) => ({
      ...
      periodizationMode: typeof row.periodization_mode === 'string' ? row.periodization_mode : 'none',
    ...
    if (preview.status === 'failed') throw new Error(preview.errors.join(' ') || 'Prisberäkningen misslyckades.')
    return {
      status: 'success',
    ...
    return { status: 'error', message: error instanceof Error ? error.message : 'Prisberäkningen misslyckades.' }
  }
}
