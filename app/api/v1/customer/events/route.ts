...
import { listPortalEventsPage, portalContextFromResolved } from '@/lib/customer-portal/apiData'
import {
  publicPageInput,
  publicPortalEvent,
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const page = await listPortalEventsPage(portalContext, publicPageInput(request.nextUrl.searchParams))
    ...
    return customerPortalJson({
      data: page.items.map((row) => publicPortalEvent(context.client.company_id, row)),
      page: page.page,
    })
  } catch (error) {
