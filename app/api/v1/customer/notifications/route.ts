...
import { listPortalNotificationsPage, portalContextFromResolved } from '@/lib/customer-portal/apiData'
import {
  publicPageInput,
...
export async function GET(request: NextRequest) {
  const context = await requireCustomerPortalApiContext(request, ['customer_notifications.read'])
  ...
  try {
    ...
    const page = await listPortalNotificationsPage(portalContext, publicPageInput(request.nextUrl.searchParams))
    ...
    return customerPortalJson({
      data: page.items.map((row) => publicPortalNotification(context.client.company_id, row)),
      page: page.page,
    })
  } catch (error) {
