...
import { listPortalNotifications, portalContextFromResolved } from '@/lib/customer-portal/apiData'
import {
  pagePublicItems,
  publicPageInput,
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const rows = await listPortalNotifications(portalContext)
    ...
    const page = pagePublicItems(
      rows.map((row) =>
        publicPortalNotification(context.client.company_id, row),
      ),
