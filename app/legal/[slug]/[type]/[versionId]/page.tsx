...
import { canonicalLegalModuleLabel } from "@/lib/legal/canonicalModules";
import {
  loadPublishedLegalVersion,
  urlSegmentToLegalType,
...
const POA_SCOPE_LABELS: Record<string, string> = {
  ...
  facility_information_lookup:
    "Inhämtning av anläggningsuppgifter från nätägare",
...
export async function generateMetadata({
  ...
  const brand =
    result.company.brand_name ?? result.company.name ?? "Elhandelsbolag";
  return {
...
export default async function PublicLegalDocumentPage({
  ...
  const typeLabel =
    ...
    canonicalLegalModuleLabel(version.type) ??
    version.title;
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-6 py-10 text-slate-950">
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <section className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-sm">
          <article className="whitespace-pre-wrap text-sm leading-6 text-slate-800">
            {version.body}
          </article>
        </section>
      </div>
