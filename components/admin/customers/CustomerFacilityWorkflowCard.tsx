...
function gridOwnerName(gridOwners: GridOwnerRow[], gridOwnerId: string | null): string | null {
...
function buildFacilityItems(input: {
  ...
  return input.sites.map((site) => {
    ...
    const hasMeteringPoint = points.some((point) => Boolean(point.meter_point_id?.trim() || point.ediel_reference?.trim()))
    const openRequest = input.infoRequests.some((request) => {
      const sameSite = request.site_id === site.id || (!request.site_id && request.customer_id === input.customerId)
    ...
    const nextAction = status === 'ready_for_switch'
...
export default function CustomerFacilityWorkflowCard(input: {
  ...
  const missingDataCount = items.filter((item) => item.missingFields.length > 0).length
  const waitingCount = items.filter((item) => item.status === 'awaiting_grid_owner').length
  const readyCount = items.filter((item) => item.status === 'ready_for_switch').length
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2 text-xs font-bold">
          <span className="rounded-full bg-amber-50 px-3 py-1 text-amber-800">{missingDataCount} behöver kompletteras</span>
          <span className="rounded-full bg-sky-50 px-3 py-1 text-sky-800">{waitingCount} väntar svar</span>
          <span className="rounded-full bg-emerald-50 px-3 py-1 text-emerald-800">{readyCount} redo</span>
        </div>
      </div>

      {items.length === 0 ? (
        <div className="mt-5 rounded-2xl border border-dashed border-slate-300 p-5 text-sm text-slate-700">
          Ingen anläggning finns ännu. Skapa anläggning eller använd kundintag/uppgiftsbegäran för att komplettera kunden.
        </div>
      ) : (
        <div className="mt-5 grid gap-3 xl:grid-cols-2">
          {items.map((item) => (
            <div key={item.siteId} className={`rounded-2xl border p-4 text-sm ${statusTone(item.status)}`}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="font-bold">{item.siteLabel}</div>
                  <div className="mt-1 text-xs opacity-80">{facilityStatusLabel(item.status)}</div>
                </div>
                <Link href={item.href} className="rounded-xl bg-white/80 px-3 py-1 text-xs font-bold text-slate-800 hover:bg-white">
                  {item.nextAction}
                </Link>
              </div>
              <p className="mt-3 leading-6">{item.statusDescription}</p>
              <div className="mt-3 grid gap-2 text-xs md:grid-cols-2">
                <div>Anläggnings-ID: <span className="font-bold">{item.facilityId ?? 'Saknas'}</span></div>
                <div>Mätpunkt: <span className="font-bold">{item.meteringPointId ?? 'Saknas'}</span></div>
                <div>Nätägare: <span className="font-bold">{item.gridOwnerName ?? 'Saknas'}</span></div>
                <div>Elområde: <span className="font-bold">{item.priceAreaCode ?? 'Saknas'}</span></div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <Link href="/admin/facility-requests" className="rounded-2xl bg-emerald-700 px-4 py-2.5 text-sm font-bold text-white hover:bg-emerald-800">
          Öppna anläggningskö
        </Link>
        <Link href={`/admin/customers/${input.customerId}?tab=data-requests`} className="rounded-2xl border border-slate-300 px-4 py-2.5 text-sm font-bold text-slate-800 hover:bg-slate-50">
          Begär uppgifter
        </Link>
      </div>
    </section>
