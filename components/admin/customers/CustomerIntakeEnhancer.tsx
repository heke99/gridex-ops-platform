...
function syncConditionalRequired(
  ...
  fields.forEach((field) => {
    ...
    if (wrapper) {
      wrapper.dataset.required =
        isRequired && !field.disabled ? "true" : "false";
    }
...
function syncDynamicLabels(attribute: string, selectedValue: string) {
  ...
  labels.forEach((label) => {
    const nextText =
      label.dataset[`label${selectedValue}` as keyof DOMStringMap];
...
function copySiteAddressToBillingAddress() {
  ...
  for (const [sourceName, targetName] of mapping) {
    const source = document.querySelector<HTMLInputElement | HTMLSelectElement>(
      `[name="${sourceName}"]`,
    );
    ...
    if (!target) continue;
    target.value =
      source?.value || (targetName === "billingCountry" ? "SE" : "");
...
export default function CustomerIntakeEnhancer({ offers, values = {} }: Props) {
  ...
  useEffect(() => {
    ...
    const sources = ["street", "postalCode", "city", "country"]
      ...
      .filter((field): field is HTMLInputElement | HTMLSelectElement =>
        Boolean(field),
      );
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 ">
        <div className="font-semibold text-slate-900 ">Intake-logik</div>
        <div className="mt-1">
          Kundtyp:{" "}
        ...
        <div className="mt-2 text-xs text-slate-700 ">{intakeSummary}</div>
      </div>
      ...
      {selectedOffer ? (
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800 ">
          ...
          <div className="mt-2 grid gap-2 md:grid-cols-2">
            <div>
              Avtalstyp: {formatContractTypeLabel(selectedOffer.contract_type)}
            </div>
            <div>
              Månadsavgift:{" "}
              {selectedOffer.monthly_fee_sek !== null
          ...
          <div className="mt-2 text-xs text-emerald-700 ">
            Fälten nedan fylls nu automatiskt med mallvärdena men kan
            fortfarande justeras manuellt innan kunden skapas.
          </div>
        </div>
      ) : (
