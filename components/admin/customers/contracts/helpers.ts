...
export function parseGreenFeeMode(
  value: FormDataEntryValue | null,
): GreenFeeMode {
...
export function getContractSituation(contract: CustomerContractRow): {
  ...
  if (lifecycle.terminationPending) {
    return {
      ...
      description:
        "Det finns en mottagen uppsägning som nu måste följas upp operativt.",
      tone: "warning",
    };
  }
  ...
  if (contract.status === "pending_signature") {
    return {
      ...
      description:
        "Det här avtalet är ännu inte färdigsignerat och bör följas upp.",
      tone: "warning",
    };
  }
...
function getRelevantSwitchRequests(
  ...
  const sameSite = switchRequests.filter(
    (row) => row.site_id === contract.site_id,
  );
...
export function getContractOpsStatus(
  ...
  const relevantSwitchRequests = getRelevantSwitchRequests(
    contract,
    opsContext.switchRequests,
  );
