...
function messageMatchesDefinition(
  ...
  if (definition.scenario === "portal_sends_actor_answers") {
    return (
      message.message_family === definition.messageFamily &&
      message.message_code === definition.messageCode
    );
  }
  ...
  if (
    ...
    const ref = String(
      message.external_reference ?? message.correlation_reference ?? "",
    ).toUpperCase();
    return (
      raw.includes(testMarker) ||
      subject.includes(testMarker) ||
      ref.includes(testMarker)
    );
  }
...
function CaseCard({
  ...
  const actorSendsFirst =
    definition.scenario === "actor_sends_and_receives_ack";
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <Badge tone={definition.suite === "PRODAT" ? "emerald" : "emerald"}>
              {definition.suite}
            </Badge>
            <Badge tone="emerald">{definition.testCaseCode}</Badge>
            {definition.messageVariant ? (
              <Badge>{definition.messageVariant}</Badge>
            ) : null}
            {activeRun ? (
              <Badge tone="amber">aktiv run</Badge>
            ) : (
              <Badge>inte startad i systemet</Badge>
            )}
          </div>
        ...
        {activeRun ? (
          <div className="text-right text-xs text-slate-700">
            ...
            <div>
              {formatDateTime(activeRun.started_at ?? activeRun.created_at)}
            </div>
          </div>
        ) : null}
      </div>
      ...
      <div className="mt-4 grid gap-2">
        {definition.expectedSteps.map((step) => (
          <div
            key={`${definition.testCaseCode}-${step.stepNo}`}
            className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-100 px-3 py-2 text-xs text-slate-700"
          >
            <Badge tone={step.actor === "actor" ? "emerald" : "emerald"}>
              Steg {step.stepNo}
            </Badge>
            <span className="font-medium">
              {step.actor === "actor"
                ? "Leverantören/systemet"
                : "Edielportalen"}
            </span>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <form
          action={createEdielAgtRunAction}
        ...
        >
          ...
          <p className="mt-2 text-xs leading-5 text-slate-700">
            Starta samma testfall i Edielportalen först eller direkt efter. Bara
            ett portaltest åt gången.
          </p>
        </form>

        {actorSendsFirst ? (
          <form
            action={createEdielAgtOutboundCommandAction}
          ...
          >
            ...
            <p className="mt-2 text-xs leading-5 text-emerald-800">
              Gäller {definition.messageFamily} {definition.messageCode}.
              Payload renderas vid skick och sparas därefter som audit/logg.
            </p>
          </form>
        ) : (
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3">
            <div className="text-sm font-semibold text-emerald-950">
              Vänta på inbound från portalen
            </div>
            <p className="mt-2 text-xs leading-5 text-emerald-800">
              Importera portalens {definition.messageFamily}{" "}
              {definition.messageCode} med motorläge AGT. Skapa sedan CONTRL +
              negativt svar från inbound-raden här.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="mt-4">
        <div className="text-sm font-semibold text-slate-900">
          Senaste matchande inbound
        </div>
        {candidates.length === 0 ? (
          <p className="mt-2 text-sm text-slate-700">
            Ingen matchande inbound hittad ännu.
          </p>
        ) : (
            ...
            {candidates.map((message) => (
              ...
              >
                ...
                <div className="mt-2 break-all text-xs text-slate-700">
                  FR {message.sender_ediel_id ?? "—"} → DO{" "}
                  {message.receiver_ediel_id ?? "—"} · ref{" "}
                ...
                {!actorSendsFirst ? (
                  <form
                    action={createEdielAgtResponsesForInboundAction}
                    className="mt-3"
                  >
                    <input
                      type="hidden"
...
export default function EdielAgtWorkbenchPanel({
  ...
  const activeAgtRuns = testRuns.filter(
    (run) =>
      isEdielAgtRunApprovalVersion(run.approval_version) &&
      (run.status === "running" || run.status === "draft"),
  );
    ...
    <section className="space-y-5">
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge tone="emerald">Portal från DB</Badge>
        </div>
        <h2 className="mt-3 text-lg font-semibold text-slate-950">
          Leverantörstest mot Edielportalen
        </h2>
        <p className="mt-2 text-sm leading-6 text-slate-700">
          Denna vy är separat från Gridcore/TGT. Leverantören skickar och
          kvitterar med aktivt Ediel-id från AGT-runtime. Motparten är
          Edielportalen från DB-konfigurerade AGT/systemtestinställningar.
          PRODAT använder sender-subadress från aktiv tenantprofil och
          receiver-subadress från systemtestinställningar; UTILTS använder ingen
          subadress.
        </p>
      ...
      <div>
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <span className="text-sm text-slate-700">
            L1 och L7 genereras och skickas direkt. L2-L5 besvarar portalens
            inbound.
          </span>
        </div>
      ...
      <div>
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <span className="text-sm text-slate-700">
            Inbound från portalen ska få positiv CONTRL och negativ
            UTILTS/UTILTS_ERR.
          </span>
        </div>
        <div className="grid gap-4 xl:grid-cols-2">
