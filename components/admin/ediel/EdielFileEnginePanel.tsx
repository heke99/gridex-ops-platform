  ...
  return (
    <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-4xl text-sm text-slate-700">
            Här importerar du svar från Edielportalen, till exempel CONTRL,
            APERAK eller PRODAT Z04. Klistra in ren EDIFACT från UNA/UNB till
            UNZ, inte JSON eller SQL-resultat.
          </p>
        </div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <form
          action={registerEdielFileAction}
        ...
        >
          <div className="mb-4 rounded-xl border border-emerald-100 bg-emerald-50 p-3 text-sm text-emerald-950">
            ...
            <p className="mt-1 text-xs text-emerald-900">
              Efter att du skickat en fil i Edielportalen hämtar du portalens
              svar och importerar det här som inbound/TGT. Systemet försöker
            ...
            <label className="block">
              ...
              >
                <option value="inbound">
                  Inbound · fil från portal/motpart
                </option>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            ...
            <label className="block md:col-span-2">
              ...
              <p className="mt-1 text-xs text-slate-700">
                Används bara i motorläge AGT när flera inbound-meddelanden har
                samma kod, exempelvis E66 för UL2/UL3.
              </p>
            </label>
          </div>
          ...
          <label className="mt-4 block">
            <span className="text-sm font-semibold text-slate-950">
              Klistra in EDIFACT/CSV från portalen
            </span>
            <textarea
              name="rawPayload"
              rows={8}
              placeholder={`Exempel: UNA:+.? '\nUNB+UNOC:3+<avsändare>+<mottagare>+...\nUNH+...\n...\nUNZ+...`}
              className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 font-mono text-xs text-slate-950 placeholder:text-slate-500"
            />
          </label>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <dl className="mt-3 grid gap-2 text-sm">
              ...
              <div className="flex justify-between gap-4">
                <dt className="text-slate-700">Application Reference</dt>
                <dd className="font-mono text-slate-900">
                  {EDIEL_TGT_PRODAT_APPLICATION_REFERENCE}
                </dd>
              </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            <h3 className="text-sm font-semibold text-slate-950">
              Första testflöden att stötta
            </h3>
            <div className="mt-3 space-y-2">
              {templates.map((template) => (
                <div
                  key={`${template.suite}-${template.code}`}
      ...
      <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
        <div className="flex items-center justify-between gap-4">
          <h3 className="text-sm font-semibold text-slate-950">
            Senaste filregistreringar
          </h3>
          <span className="text-xs text-slate-700">
            Visar transport_type = manual_upload
          </span>
        </div>
        <div className="mt-3 overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 text-left text-xs text-slate-700">
                    ...
                    >
                      ...
                      <td className="px-3 py-2">
                        ...
                        <div className="text-xs text-slate-700">
                          {row.direction} ·{" "}
                          {row.message_version ?? "utan version"} · {row.status}
                        </div>
                      </td>
                      <td className="px-3 py-2 text-xs text-slate-700">
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          <form action={cancelEdielMessageAction}>
                            ...
                            <input
                              type="hidden"
                              name="reason"
                              value="Dold från filimportlistan via admin cleanup."
                            />
