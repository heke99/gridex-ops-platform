...
function MessageCard({ row, action }: { row: EdielMessageRow; action?: 'safe_apply' }) {
 ...
 <div className="mt-1 text-xs text-slate-700">{formatDateTime(row.created_at)}</div>
 </div>
...
export default function EdielOperationalVerificationPanel({
 ...
 <p className="mt-1 text-xs text-slate-700">Skapa förslag innan masterdata uppdateras. Inga fält skrivs över automatiskt.</p>
 </div>
 <Badge tone={safeApplyCandidates.length > 0 ? 'amber' : 'emerald'}>{safeApplyCandidates.length}</Badge>
 </div>
 <div className="mt-3 space-y-3">
 {safeApplyCandidates.length === 0 ? (
 <div className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-700">Inga Z06/Z10 att granska i senaste urvalet.</div>
 ) : (
 ...
 <p className="mt-1 text-xs text-slate-700">Dessa ska kopplas innan de får styra switch/outbound-status.</p>
 </div>
 <Badge tone={unlinkedAcks.length > 0 ? 'amber' : 'emerald'}>{unlinkedAcks.length}</Badge>
 </div>
 <div className="mt-3 space-y-3">
 {unlinkedAcks.length === 0 ? (
 <div className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-700">Inga okopplade ACK i senaste urvalet.</div>
 ) : (
 ...
 <p className="mt-1 text-xs text-slate-700">PRODAT/UTILTS ska helst länkas till switch, data request, kund, anläggning eller mätpunkt.</p>
 </div>
 <Badge tone={unlinkedOperational.length > 0 ? 'amber' : 'emerald'}>{unlinkedOperational.length}</Badge>
 </div>
 <div className="mt-3 space-y-3">
 {unlinkedOperational.length === 0 ? (
 <div className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-700">Inga okopplade inbound PRODAT/UTILTS i senaste urvalet.</div>
 ) : (
