...
const PORTAL_FORM_CASE_CODES = new Set([
  "1.2.1",
  "1.2.2",
  "1.2.5",
  "2.5.1",
...
const PRODAT_PORTAL_GROUPS: ProdatCatalogGroupConfig[] = [
  ...
  },
  {
    ...
    description: "Positiva testfall för mätarbyte.",
    tone: "emerald",
    prefixes: ["2.3"],
  },
  {
  ...
  },
  {
    ...
    description: "Korrekta testfall för Z09, inklusive mikroproduktionsavtal.",
    tone: "emerald",
    prefixes: ["2.5"],
  },
  {
    ...
    description: "Positiva testfall för Z05L och Z05LK.",
    tone: "emerald",
    prefixes: ["3.1"],
  },
  {
  ...
  },
  {
    ...
    description: "Verifierar att felaktig Z14V avvisas med negativ APERAK.",
    tone: "red",
    prefixes: ["8.2"],
  },
  {
    ...
    description: "Avslutsflöden för aktivt tillstånd och rapportering.",
    tone: "emerald",
    prefixes: ["9.1"],
  },
  {
    ...
    description: "Verifierar att felaktig Z15V avvisas med negativ APERAK.",
    tone: "red",
    prefixes: ["9.2"],
  },
];
...
function isProdatCoreCase(testCase: EdielTgtTestCaseDefinition): boolean {
  return testCase.suite === "PRODAT" && testCase.scope === "core";
}

function isProdatSupplierCoreCase(
  ...
  return PRODAT_PORTAL_GROUPS.map((group) => ({
    ...
    testCases: allCases.filter((testCase) =>
      group.prefixes.some((prefix) =>
        matchesPrefix(testCase.testCaseCode, prefix),
      ),
    ),
  ...
  return (
    <label className="block text-xs font-semibold text-slate-700">
      ...
      <input
        ...
        type={type}
        className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs text-slate-950 placeholder:text-slate-500"
      />
    </label>
  );
}
...
function FormSection({
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
      ...
      {description ? (
        <p className="mt-1 text-[11px] text-slate-700">{description}</p>
      ) : null}
...
function PortalTestCatalogPanel() {
  ...
  const totalCases = groups.reduce(
    (sum, group) => sum + group.testCases.length,
    0,
  );
  ...
  return (
    <details className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4 open:ring-2 open:ring-slate-100">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 max-w-4xl text-xs text-slate-700">
              Här visas alla PRODAT-tester ni bör kunna köra, men i kompakt
              dropdown-format så sidan inte blir lång. 1.2.x och 2.5.x öppnas
              som riktiga kundformulär här nedan. Z09-testen är outbound
              aktör→portal: GridCore skapar och skickar PRODAT Z09, och portalen
      ...
      <div className="mt-4 rounded-2xl border border-emerald-100 bg-emerald-50 p-3 text-xs text-emerald-900">
        ...
        <p className="mt-1">
          <strong>Formulär här nedan</strong> används för verklig
          kund-/switchuppsättning i era riktiga tabeller. För Z09 betyder det:
          skapa testkund/ärende, klicka
          <strong> Skapa Z09-utkast</strong> och skicka PRODAT från GridCore
          till portalen.
          <strong> TGT guided mode</strong> används för övriga testfall som
          ...
          >
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Badge tone="slate">
                      {group.testCases.length} testfall
                    </Badge>
                    <Badge tone="emerald">
                      {
                        group.testCases.filter((testCase) =>
            ...
            <div className="mt-4 space-y-3">
              {group.testCases.map((testCase) => {
                const usesForm = PORTAL_FORM_CASE_CODES.has(
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-4xl text-xs text-emerald-800">
            Här fyller du in kunduppgifterna från Edielportalen manuellt.
            Testdataregistret kan hjälpa senare, men formuläret är källan till
            sanningen. När kunden skapas hamnar kund, fakturamottagare,
            anläggning, mätpunkt, route, fullmakt och switchärende i era riktiga
            tabeller.
          </p>
        </div>
      ...
      <div className="mt-4 space-y-4">
        {testCases.map((testCase) => (
          ...
          >
            ...
            <form
              action={createEdielPortalTestCustomerAction}
            ...
            >
              ...
              >
                ...
                <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-900 md:col-span-2">
                  För 1.2.1 Z03L med personnummer används normalt SE2. För 1.2.2
                  Z03LK med organisationsnummer 5560143041 ska kund-id typ vara
                  SE1.
                </div>
                <FormInput
                ...
                <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-900 md:col-span-2">
                  Minst en kontaktuppgift krävs eftersom outbound-valideringen
                  stoppar PRODAT om både kundens e-post och telefon saknas.
                </div>
                <FormInput
              ...
              >
                ...
                <FormSelect
                  ...
                  label="Transaktionstyp / fält 223"
                >
                  <option value="">Auto från testfall</option>
                  <option value="Z22">L = leverantörsbyte = Z22</option>
                  <option value="Z23">
                    LK = leverantörs- och kundbyte = Z23
                  </option>
                  <option value="E64">Z09F = E64</option>
                  <option value="E32">Z09G = E32</option>
                  <option value="Z70">Z09D = Z70</option>
                  <option value="E64">
                    Z09F = avtal om 15-minutersvärden = E64
                  </option>
                  <option value="E32">
                    Z09G = avtal om 15-minutersvärden upphör = E32
                  </option>
                  <option value="Z70">
                    Z09D = nytt avtal om mikroproduktion = Z70
                  </option>
                </FormSelect>
                <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-900 md:col-span-2">
                  Välj L/Z22 för vanligt leverantörsbyte, t.ex. 1.2.1 Z03L. Välj
                  LK/Z23 för leverantörs- och kundbyte, t.ex. 1.2.2 Z03LK. För
                  Z09-testerna används E64 = Z09F med mätmetod Z04, E32 = Z09G
                  med mätmetod Z03 och Z70 = Z09D.
                </div>
                <FormSelect
                ...
                >
                  <option value="signed">
                    Signerad - får användas för Ediel
                  </option>
                  <option value="draft">Utkast - spärrar Ediel</option>
                  <option value="sent">
                    Skickad - spärrar Ediel tills signerad
                  </option>
                ...
                <FormInput
                  ...
                  placeholder="Välj/ange BRP från DB-konfiguration"
                />
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
                Kontrollera uppgifterna innan du skapar. Om du har valt fel kund
                senare ska ej skickade utkast avbrytas/arkiveras, inte
                hårdraderas. Skickade meddelanden ska alltid ligga kvar för
                spårbarhet.
              </div>
...
function ProductionCandidateCard({
  ...
  const warningIssues = candidate.issues.filter(
    (issue) => issue.severity !== "error",
  );
    ...
    <details className="rounded-2xl border border-slate-200 bg-white shadow-sm open:ring-2 open:ring-emerald-100">
      <summary className="cursor-pointer list-none p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex flex-wrap gap-2">
              <Badge
                tone={candidate.readyForPortalOrProduction ? "emerald" : "red"}
              >
                {candidate.readyForPortalOrProduction
                  ? "redo för Ediel-utkast"
                  : "spärrad"}
              </Badge>
            ...
            <p className="mt-1 text-xs text-slate-700">
              {candidate.siteLabel} · Anläggning{" "}
              {candidate.facilityId ?? "saknas"} · Mätpunkt{" "}
      ...
      <div className="border-t border-slate-100 p-4">
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
          <Field
            label="Kund-ID/person/org"
            value={candidate.customerIdentifier}
          />
          ...
          <Field
            ...
            value={candidate.gridOwnerEdielId}
          />
          ...
          <Field
            label="Fullmaktsreferens"
            value={candidate.powerOfAttorneyReference}
          />
          <Field label="Mätmetod/typ" value={candidate.meteringMethod} />
          <Field
            label="Rapporteringsfrekvens"
            value={candidate.readingFrequency}
          />
        </div>
        ...
        {blockingIssues.length > 0 ? (
          ...
          <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-900">
            ...
            <p className="mt-1">
              Kund, person/orgnummer, anläggning, mätpunkt, nätägare, route,
              startdatum och fullmakt finns. Systemet tillåter filutkast.
            </p>
          </div>
        )}
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          >
            <input
              ...
              value={candidate.switchRequestId}
            />
            ...
            <p className="mt-1 text-[11px] leading-5 text-emerald-800">
              Använd detta när portalen säger att testdata inte matchar, utan
              att behöva skapa om kund/anläggning. När du sparar avbryts gamla
              oskickade PRODAT-utkast automatiskt. Skapa sedan nytt
              PRODAT-utkast.
            </p>
            ...
            <div className="mt-3 rounded-xl border border-emerald-100 bg-white px-3 py-2 text-[11px] leading-5 text-emerald-900">
              För 1.2.2 Z03LK ska du normalt ha: <strong>Z04</strong>,{" "}
              <strong>Z23</strong> och <strong>SE1</strong>. Raw PRODAT ska då
              innehålla <code>CCI++Z04&apos; CAV+Z04&apos;</code>,{" "}
              <code>CAV+Z23&apos;</code> och{" "}
          ...
          <form action={prepareSwitchZ04Action}>
            ...
            <button
              ...
              className="rounded-xl border border-emerald-300 bg-white px-3 py-2 text-xs font-semibold text-emerald-800 hover:bg-emerald-50 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-700"
            >
              Skapa Z04-utkast
            </button>
          </form>
          ...
          <form action={prepareSwitchZ09Action}>
            ...
            <button
              ...
              className="rounded-xl border border-emerald-300 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-800 hover:bg-emerald-100 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-50 disabled:text-slate-700"
            >
              Skapa Z09-utkast
            </button>
          </form>
                ...
                >
                  <div>
                    ...
                    <span className="ml-2 text-slate-700">
                      {message.status} ·{" "}
                      {message.external_reference ?? "utan extern ref"}
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Link
                    ...
                    {["draft", "queued", "prepared", "failed"].includes(
                      message.status,
                    ) ? (
                      <form
...
export default function EdielProductionProdatPanel({
  ...
  const ready = candidates.filter(
    (candidate) => candidate.readyForPortalOrProduction,
  ).length;
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-4xl text-sm text-slate-700">
            Välj ett riktigt switchärende. Systemet kontrollerar kund,
            anläggning, mätpunkt, nätägare, route, startdatum och fullmakt innan
            Z03/Z04 får skapas. Felaktiga utkast avbryts utan att historiken
            raderas.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Badge tone="emerald">redo: {ready}</Badge>
          <Badge tone={blocked > 0 ? "red" : "emerald"}>
            spärrade: {blocked}
          </Badge>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-900">
          ...
          <p className="mt-1">
            Utgå från ett switchärende så kund, avtal, anläggning och mätpunkt
            hänger ihop.
          </p>
        </div>
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-900">
          ...
          <p className="mt-1">
            Saknas personnummer, kontaktuppgift, fullmakt, route eller mätpunkt
            blockeras filskapande direkt.
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700">
          <div className="font-semibold">3. Skapa utkast</div>
          <p className="mt-1">
            Utkastet sparas som Ediel-meddelande. Skickade meddelanden raderas
            aldrig; fel hanteras via avbruten status och auditspår.
          </p>
        </div>
      </div>
      ...
      <div className="mt-5 space-y-3">
        {candidates.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-700">
            Inga switchärenden hittades. Skapa kund, anläggning, mätpunkt,
            fullmakt och switchärende först.
          </div>
        ) : (
