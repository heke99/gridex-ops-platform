...
function overlayClassName(open: boolean) {
 return open
 ? 'fixed inset-0 z-50 flex items-center justify-center bg-emerald-700/50 p-4'
 : 'hidden'
}
...
export default function EdielRuleTemplateModals({ hasProdatRule }: Props) {
 ...
 <p className="mt-1 text-xs text-slate-700">{template.subtitle}</p>
 ...
 <p className="mt-1 text-sm text-slate-700">{activeTemplate.subtitle}</p>
 </div>
 <button
 type="button"
