...
export function ProductionReadinessPanel({
  ...
  const canPause =
    canManageProduction && (readiness.status === "live" || s.productionEnabled);
    ...
    <section id="ediel-production" className="space-y-6">
      {readiness.status === "paused" ? (
        <div className="rounded-3xl border border-amber-200 bg-amber-50 p-5 text-sm font-semibold text-amber-900">
          Production sending är pausad. Inbound production-meddelanden kan
          fortfarande tas emot, loggas och granskas.
        </div>
      ) : null}
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Badge tone={toneForStatus(readiness.status)}>
              {statusLabel(readiness.status)}
            </Badge>
            <Badge
              tone={
                s.productionLockLocked
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <IssueList
          title="Detta saknas / blockerar"
          issues={readiness.blockingIssues}
          tone="red"
        />
        <IssueList
          title="Varningar att granska"
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 grid gap-3">
            <Info
              label="Test route profile"
              value={
                <span className="font-mono text-xs">
                  {s.activeTestRouteProfileId ?? "–"}
                </span>
              }
            />
            <Info
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 grid gap-3">
            <Info
              ...
              value={
                s.latestInbound
                  ? `${s.latestInbound.family} ${s.latestInbound.code} · ${s.latestInbound.status} · ${formatDate(s.latestInbound.createdAt)}`
                  : "–"
              }
            />
            <Info
              ...
              value={
                s.latestOutbound
                  ? `${s.latestOutbound.family} ${s.latestOutbound.code} · ${s.latestOutbound.status} · ${formatDate(s.latestOutbound.createdAt)}`
                  : "–"
              }
            />
            <Info
              label="Unresolved / Failed / Negative APERAK"
              value={`${s.unresolvedItems} / ${s.failedMessages} / ${s.negativeAperaks}`}
            />
            <Info
              label="Första live-send"
              value={
                s.firstLiveSendApprovedAt
                  ? `Godkänd ${formatDate(s.firstLiveSendApprovedAt)}`
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {readiness.latestDryRun.createdAt ? (
            <p className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-3 text-xs font-semibold text-slate-700">
              Senaste dry run: {readiness.latestDryRun.status ?? "okänd"} ·{" "}
              {formatDate(readiness.latestDryRun.createdAt)}
            </p>
          ) : null}
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-3">
          <form
            action={runProductionReadinessAction}
            className="rounded-2xl border border-slate-200 bg-slate-50 p-4"
          >
            <input
              type="hidden"
              name="company_id"
              value={readiness.companyId}
            />
            ...
            <p className="mt-2 text-sm leading-6 text-slate-700">
              Sparar en backend-kontroll i audit/go-live historiken.
            </p>
            <button
              disabled={!canManageProduction}
              className="mt-4 rounded-xl bg-slate-950 px-3 py-2 text-xs font-semibold text-white disabled:bg-slate-400"
            >
              Kör kontroll
            </button>
          </form>
          ...
          <form
            action={runProductionDryRunAction}
          ...
          <form
            action={approveFirstLiveSendAction}
          ...
          >
            ...
            <p className="mt-2 text-sm leading-6 text-amber-900">
              Kräver explicit superadmin-godkännande innan första outbound
              production-send.
            </p>
            <input
              name="confirmation"
              placeholder="APPROVE FIRST LIVE SEND"
              className="mt-3 w-full rounded-xl border border-amber-300 bg-white px-3 py-2 text-sm"
            />
            <button
              disabled={
                !canManageProduction || readiness.blockingIssues.length > 0
              }
              className="mt-3 rounded-xl bg-amber-700 px-3 py-2 text-xs font-semibold text-white disabled:bg-slate-400"
            >
              Godkänn första send
            </button>
          </form>
          ...
          <form
            action={activateLiveEdielAction}
            className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"
          >
            <input
              type="hidden"
            ...
            <input
              name="confirmation"
              placeholder="ACTIVATE PRODUCTION"
              className="mt-3 w-full rounded-xl border border-emerald-300 bg-white px-3 py-2 text-sm"
            />
            <button
          ...
          <form
            action={pauseProductionEdielAction}
          ...
          >
            ...
            <h4 className="font-semibold text-red-950">
              Pausa production sending
            </h4>
            <p className="mt-2 text-sm leading-6 text-red-800">
              Stoppar nya outbound production-send men behåller inbound logging.
            </p>
            <input
              name="reason"
              placeholder="Anledning krävs"
              className="mt-3 w-full rounded-xl border border-red-300 bg-white px-3 py-2 text-sm"
            />
            <button
              disabled={!canPause}
              className="mt-3 rounded-xl bg-red-700 px-3 py-2 text-xs font-semibold text-white disabled:bg-slate-400"
            >
              Pausa sending
            </button>
          </form>
          ...
          <form
            action={resumeProductionEdielAction}
            className="rounded-2xl border border-slate-200 bg-slate-50 p-4"
          >
            <input
              type="hidden"
            ...
            <input type="hidden" name="redirect_to" value={returnPath} />
            <h4 className="font-semibold text-slate-950">
              Återuppta production
            </h4>
            <p className="mt-2 text-sm leading-6 text-slate-700">
              Kör readiness igen och låser upp sending om allt fortfarande
              passerar.
            </p>
            <input
              name="confirmation"
              placeholder="RESUME PRODUCTION"
              className="mt-3 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm"
            />
            <button
              disabled={!canResume}
              className="mt-3 rounded-xl bg-slate-950 px-3 py-2 text-xs font-semibold text-white disabled:bg-slate-400"
            >
              Återuppta
            </button>
          </form>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 space-y-3">
          {readiness.auditEvents.length === 0 ? (
            <p className="text-sm text-slate-600">
              Ingen go-live historik ännu.
            </p>
          ) : null}
          {readiness.auditEvents.map((event) => (
            ...
            >
              ...
              {event.reason ? (
                <p className="mt-2 leading-6 text-slate-700">{event.reason}</p>
              ) : null}
            </article>
          ))}
        </div>
