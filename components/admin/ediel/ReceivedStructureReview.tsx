...
import {reviewReceivedStructureAction,reviewReceivedClosureAction} from '@/app/admin/ediel/structure-actions'

export default function ReceivedStructureReview({companyId,sourceMessageId,environment,closure=false}:{companyId:string;sourceMessageId:string;environment:'test'|'production';closure?:boolean}){
  const prefix=useId()
  ...
  return <section aria-labelledby={`${prefix}-title`} className="rounded-3xl border border-slate-200 bg-white p-6">
    ...
    <form className="mt-4 space-y-4" onSubmit={event=>{
      ...
      startTransition(async()=>{
        setResult(null)
        try{setResult(await (closure?reviewReceivedClosureAction:reviewReceivedStructureAction)(form))}
      ...
      <label className="flex items-start gap-3 text-sm text-slate-800">
        ...
        <span>{closure?'Jag har granskat hela originalets leveransavslut, inklusive orsak, samtliga objekt och exakt stopptid.':'Jag har granskat hela originalmeddelandet, inklusive ändringsorsak, giltighetsdatum, mätare och samtliga register.'}</span>
      </label>
