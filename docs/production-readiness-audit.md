...
| M9 | Public rate limit (`/api/public/energy-area`) is per-process in-memory — effective limit scales with serverless instance count. | `lib/http/publicRateLimit.ts` | **documented** — acceptable for the single public lookup endpoint; DB/edge-based limiting documented as post-launch improvement |
