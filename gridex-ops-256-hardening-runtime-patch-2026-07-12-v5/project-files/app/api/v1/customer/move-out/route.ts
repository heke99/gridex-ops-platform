...
import { readJsonObject, requireIsoDate } from '@/lib/api/strictRequest'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (siteId) {
      ...
      if (!update.data) return customerPortalJson({ error: 'Anläggningen hittades inte för kunden.', code: 'customer_site_not_found' }, { status: 404 })
    }
