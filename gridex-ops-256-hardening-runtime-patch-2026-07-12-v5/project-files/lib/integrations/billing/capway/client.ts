...
export class CapwayApticClient {
  ...
  private async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    const timeout = setTimeout(() => controller.abort(new Error('Capway request timeout')), 30_000)
  ...
  createInvoices(invoices: CapwayPutInvoice[], idempotencyKey: string) {
    if (!idempotencyKey.trim()) throw new Error('Provider-idempotensnyckel krävs.')
