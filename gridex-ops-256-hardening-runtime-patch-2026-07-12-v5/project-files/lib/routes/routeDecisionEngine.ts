...
async function findRoute(params: {
  ...
  const chooseTier = (rows: RouteRow[]): { route: RouteRow | null; ambiguous: boolean; matches: RouteRow[] } => {
    const tenantRows = params.companyId ? rows.filter((row) => row.company_id === params.companyId) : [];
    const globalRows = rows.filter((row) => row.company_id === null);
    const selected = tenantRows.length > 0 ? tenantRows : globalRows;
    return {
      route: selected.length === 1 ? selected[0] : null,
  ...
  if (params.preferredRouteId) {
    ...
    explicit = params.companyId
      ? explicit.or(`company_id.is.null,company_id.eq.${params.companyId}`)
      : explicit.is("company_id", null);
    const { data, error } = await explicit.limit(2);
    if (error) throw error;
    const selected = chooseTier((data ?? []) as RouteRow[]);
    if (selected.route || selected.ambiguous) return selected;
  }

  const load = async (gridOwnerId: string | null): Promise<RouteRow[]> => {
    ...
    query = params.companyId
      ? query.or(`company_id.is.null,company_id.eq.${params.companyId}`)
      : query.is("company_id", null);
    const result = await query.order("updated_at", { ascending: false }).limit(20);
    if (result.error) throw result.error;
    return (result.data ?? []) as RouteRow[];
  };

  if (params.gridOwnerId) {
    const exact = chooseTier(await load(params.gridOwnerId));
    if (exact.route || exact.ambiguous) return exact;
  }
  return chooseTier(await load(null));
}
...
async function findRouteProfile(
  ...
  const tenantRows = companyId ? rows.filter((row) => row.company_id === companyId) : [];
  const globalRows = rows.filter((row) => row.company_id === null);
  const tier = tenantRows.length > 0 ? tenantRows : globalRows;
  const enabled = tier.filter((row) => row.is_enabled === true);
  if (enabled.length > 1) {
    throw new Error(`route_profile_ambiguous:${routeId}:${environment ?? "any"}:${enabled.map((row) => row.id).join(",")}`);
  }
  if (enabled.length === 1) return { profile: enabled[0], status: "enabled" };
  if (tier.length > 1) {
    throw new Error(`route_profile_disabled_ambiguous:${routeId}:${environment ?? "any"}:${tier.map((row) => row.id).join(",")}`);
  }
  return { profile: tier[0] ?? null, status: tier[0] ? "disabled" : "missing" };
}
...
export async function logRouteDecision(
  ...
  const routingDecisionPayload = {
    ...
    transport_profile_id: uuidOrNull((decision.payload ?? {}).transport_profile_id),
    metering_point_id: input.meteringPointId ?? null,
    grid_owner_id: decision.resolvedGridOwnerId ?? input.gridOwnerId ?? null,
    validation_status:
...
export async function decideCommunicationRoute(
  ...
  const decision: RouteDecisionOutput = {
    ...
    payload: {
      ...
      transport_profile_id: uuidOrNull(profile?.transport_profile_id ?? profile?.mailbox_id),
  ...
  try {
    ...
    console.warn("[routeDecisionEngine] Route decision audit failed without blocking routing", error);
  }

  try {
    await createRouteAdminTasks(input, decision);
  } catch (error) {
    console.warn("[routeDecisionEngine] Route admin task creation failed without blocking routing", error);
  }
