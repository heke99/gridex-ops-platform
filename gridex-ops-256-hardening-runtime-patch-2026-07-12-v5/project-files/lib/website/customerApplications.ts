...
function mergeResolverWithExplicitInput(
  ...
  const gridOwnerDisagrees = Boolean(explicitOwnerId && resolution.gridOwnerId && explicitOwnerId !== resolution.gridOwnerId)
  const claimedOnly = !masterVerified && Boolean(explicitGridAreaCode || explicitPriceAreaCode || explicitOwnerId)

  return {
    ...resolution,
    gridAreaCode: masterVerified ? resolution.gridAreaCode : resolution.gridAreaCode ?? explicitGridAreaCode,
    priceArea: masterVerified
      ? resolution.priceArea
      : resolution.priceArea ?? (explicitPriceAreaCode as EnergyResolverResult['priceArea'] | null),
    gridOwnerId: masterVerified ? resolution.gridOwnerId : resolution.gridOwnerId ?? explicitOwnerId,
    automationAllowed: Boolean(resolution.automationAllowed && masterVerified && !gridAreaDisagrees && !priceAreaDisagrees && !gridOwnerDisagrees),
    nextRequiredAction: gridAreaDisagrees || priceAreaDisagrees || gridOwnerDisagrees || claimedOnly
    ...
    sourceChain: Array.from(new Set([
      ...
      ...(explicitGridAreaCode || explicitPriceAreaCode || explicitOwnerId ? ['input.claimed_energy_context'] : []),
    ])),
    warnings: Array.from(new Set([
      ...resolution.warnings,
      ...(explicitGridOwner?.warnings ?? []),
      ...(claimedOnly ? ['claimed_energy_context_not_master_verified'] : []),
      ...(gridAreaDisagrees ? ['resolver_grid_area_disagrees_with_claimed_input'] : []),
      ...(priceAreaDisagrees ? ['resolver_price_area_disagrees_with_claimed_input'] : []),
      ...(gridOwnerDisagrees ? ['resolver_grid_owner_disagrees_with_claimed_input'] : []),
    ])),
  }
}
...
export async function processWebsiteCustomerApplication(input: {
  ...
  try {
    ...
    if (site?.id && siteAddress?.street && siteAddress.postal_code && siteAddress.city) {
      ...
      void addressResult
      const currentSiteForCanonicalPatch = site as { id: string; facility_id?: string | null }
      const canonicalPatchFacilityId =
        normalizeFacilityId(currentSiteForCanonicalPatch.facility_id) ?? normalizeFacilityId(body.site?.facility_id)
