...
-- ---------------------------------------------------------------------------
-- 2b. Versioned market process policies (no business lead times in code)
-- ---------------------------------------------------------------------------
...
create unique index if not exists market_process_policies_unique_version_uidx
  on public.market_process_policies(coalesce(company_id,'00000000-0000-0000-0000-000000000000'::uuid),process_code,environment,valid_from,policy_version);
...
insert into public.market_process_policies(company_id,process_code,environment,valid_from,policy_version,policy,is_active)
select null,'supplier_switch','production','2026-01-01','SE-SWITCH-2026-01',
  '{"send_window_open_lead_days":45,"send_window_close_offset_days":0,"market_lead_days":14,"calendar":"Europe/Stockholm"}'::jsonb,true
where not exists (
  select 1 from public.market_process_policies
  where company_id is null and process_code='supplier_switch' and environment='production' and policy_version='SE-SWITCH-2026-01'
);
...
-- Verified timestamps are required for deterministic recipient selection.
alter table public.grid_owner_contact_channels
  add column if not exists verified_at timestamptz;
update public.grid_owner_contact_channels
...
create table if not exists public.customer_portal_write_idempotency (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique(company_id,api_client_id,customer_id,route,idempotency_key)
);
alter table public.customer_portal_write_idempotency add column if not exists customer_id uuid;
do $$ begin
  if exists(select 1 from public.customer_portal_write_idempotency where customer_id is null) then
    raise exception 'customer_portal_write_idempotency_customer_backfill_required';
  end if;
end $$;
alter table public.customer_portal_write_idempotency alter column customer_id set not null;
alter table public.customer_portal_write_idempotency drop constraint if exists customer_portal_write_idempotency_company_client_route_idempotency_key_key;
drop index if exists customer_portal_write_idempotency_company_client_route_key_uidx;
create unique index if not exists customer_portal_write_idempotency_company_client_customer_route_key_uidx
  on public.customer_portal_write_idempotency(company_id,api_client_id,customer_id,route,idempotency_key);
...
update public.invoice_provider_events e
 ...
 where e.environment is null and e.matched_invoice_export_item_id=i.id;
update public.billing_provider_webhook_events w
   set environment=coalesce(w.environment,'test')
...
create unique index invoice_provider_events_provider_idempotency_uidx
  on public.invoice_provider_events(company_id,provider,environment,idempotency_hash)
...
create unique index billing_provider_webhook_events_provider_idempotency_uidx
  on public.billing_provider_webhook_events(company_id,provider,environment,idempotency_key)
...
alter table public.invoice_provider_events alter column environment set not null;
alter table public.invoice_provider_events drop constraint if exists invoice_provider_events_environment_check;
alter table public.invoice_provider_events add constraint invoice_provider_events_environment_check check(environment in ('test','production'));
alter table public.billing_provider_webhook_events alter column environment set not null;
alter table public.billing_provider_webhook_events drop constraint if exists billing_provider_webhook_events_environment_check;
alter table public.billing_provider_webhook_events add constraint billing_provider_webhook_events_environment_check check(environment in ('test','production'));
alter table public.billing_provider_webhook_events drop constraint if exists billing_provider_webhook_events_connection_fkey;
alter table public.billing_provider_webhook_events add constraint billing_provider_webhook_events_connection_fkey foreign key(billing_provider_connection_id) references public.billing_provider_connections(id) on delete restrict;
