...
const RAW_PUBLIC_API_ROUTES: PublicApiRouteDefinition[] = [
  ...
  { method: 'POST', path: '/api/v1/website/quote', scopes: ['website_quotes.write'], description: 'Create an authoritative price quote from a published offer, selected price area, customer type, consumption and start date.', idempotencyRequired: true, rateLimitClass: 'expensive' },
  { method: 'POST', path: '/api/v1/website/quote/validate', scopes: ['website_quotes.validate'], description: 'Validate that a quote still matches the selected offer, customer type, price area, consumption and start date.', rateLimitClass: 'read' },
