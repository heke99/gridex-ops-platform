...
async function maybeOne(table: string, query: (builder: any) => Promise<{ data?: unknown; error?: unknown }>) {
  const result = await query(supabaseService.from(table))
  if (result.error) {
...
export async function evaluateCustomerIntake(input: {
  ...
  if (!facilityReady) {
    if (input.autoEnsureFacilityLookup) {
      const lookup = await ensureFacilityLookupAutomation({
      ...
      referencesBase.gridOwnerInformationRequestId = lookup.requestId
      referencesBase.communicationRouteId = lookup.routeId
      const state = lookup.status === 'waiting_response'
        ? 'facility_lookup_waiting_response'
        : lookup.status === 'ready_to_send'
          ? 'facility_lookup_ready_to_send'
          : lookup.status === 'blocked'
            ? 'needs_admin_review'
            : 'needs_facility_lookup'
      const nextAction = lookup.status === 'waiting_response'
        ? 'wait_for_grid_owner'
        : lookup.status === 'ready_to_send'
          ? 'send_facility_lookup'
          : lookup.status === 'blocked'
      ...
      const result = decision({
        state,
        nextAction,
        customerMessage: lookup.status === 'waiting_response'
          ? 'Vi väntar på anläggningsuppgifter från nätägaren.'
          : 'Vi hämtar anläggningsuppgifter från nätägaren.',
        adminMessage: lookup.nextStep,
        blockers: lookup.blockers,
        warnings: lookup.warnings.map((code) => ({ code, message: code, source: 'facility_lookup_automation' })),
        references: referencesBase,
      })
      if (input.apply) await persistDecision({ companyId: input.companyId, customerId: input.customerId, siteId, decision: result })
      return result
