...
// Swedish facility/metering identifiers are accepted only in canonical
// 18-digit form. Short aliases are never safe enough for automatic apply.
...
export function extractManualFacilityFields(bodyText: string): ManualFacilityExtractedPayload {
  ...
  return {
    facility_id: facilityDigits && facilityDigits.length === 18 ? facilityDigits : null,
    metering_point_id: meteringDigits && meteringDigits.length === 18 ? meteringDigits : null,
...
export async function applyManualFacilityResponse(input: {
  ...
  if (siteId) {
    const siteResult = await supabaseService
      .from('customer_sites')
      .select('id,company_id,customer_id,facility_id,grid_area_code,price_area_code,grid_owner_id,protected_identity')
      .eq('company_id', input.companyId)
      .eq('id', siteId)
      .maybeSingle()
    if (siteResult.error) throw siteResult.error
    site = (siteResult.data as JsonRecord | null) ?? null
  }
  ...
  let verifiedGridArea: JsonRecord | null = null
  if (effectiveGridAreaCode) {
    const areaResult = await supabaseService
      .from('platform_grid_areas')
      .select('id,grid_area_code,grid_owner_id,price_area,is_active')
      .eq('grid_area_code', effectiveGridAreaCode.toUpperCase())
      .eq('is_active', true)
      .limit(2)
    if (areaResult.error) throw areaResult.error
    const areas = (areaResult.data ?? []) as JsonRecord[]
    if (areas.length === 1) verifiedGridArea = areas[0]
    else reasons.push(areas.length === 0 ? 'grid_area_not_in_masterdata' : 'grid_area_masterdata_ambiguous')
  }
  const expectedGridOwnerId = clean(input.request.grid_owner_id) ?? clean(site?.grid_owner_id)
  if (verifiedGridArea && expectedGridOwnerId && clean(verifiedGridArea.grid_owner_id) !== expectedGridOwnerId) {
    reasons.push('grid_area_grid_owner_mismatch')
  }
  const replyPriceArea = clean(input.extracted.price_area_code)
  if (verifiedGridArea && replyPriceArea && clean(verifiedGridArea.price_area) !== replyPriceArea) {
    reasons.push('price_area_grid_area_mismatch')
  }
  ...
  if (site && clean(site.customer_id) !== customerId) reasons.push('request_site_customer_mismatch')
  ...
  if (!safe) {
    ...
    if (!reviewUpdate.data) throw new Error('Nätägarärendet kunde inte markeras för granskning.')
    ...
    return { outcome: 'needs_review', confidence: effectiveConfidence, extracted: input.extracted, reasons }
  }
  ...
  const evidenceUpdate = await supabaseService
    .from('grid_owner_information_requests')
    .update({
      parsed_payload: parsedPayload,
      confidence_score: effectiveConfidence,
  ...
  if (!evidenceUpdate.data) throw new Error('Nätägarärendets tolkningsbevis kunde inte sparas.')
  ...
  try {
    ...
    if (!failedUpdate.data) throw new Error('Nätägarärendet kunde inte säkert återföras till granskning.')
    return {
      outcome: 'needs_review',
      confidence,
      extracted: input.extracted,
  ...
  return { outcome: 'applied', confidence: effectiveConfidence, extracted: input.extracted, reasons: [], nextStepDecision }
}
