...
function looksLikePortalAgtUeE66Source(sourceMessage: EdielMessageRow): boolean {
  const family = String(sourceMessage.message_family ?? '').toUpperCase()
  const code = String(sourceMessage.message_code ?? '').toUpperCase()
  if (family !== 'UTILTS' || code !== 'E66') return false

  const sender = String(sourceMessage.sender_ediel_id ?? sourceMessage.unb_sender_id ?? '').trim()
  const appRef = String(sourceMessage.application_reference ?? '').toUpperCase()
  const raw = String(sourceMessage.raw_payload ?? '').toUpperCase()

  return (
    sender === '91100' &&
    (
      appRef.includes('23-DGI-E66-T') ||
      appRef.includes('23-DGI-E66-S') ||
...
function agtUeAllowedUtiltsErrCodeFor(sourceMessage: EdielMessageRow, code: string | null): string {
  const normalized = sanitizeEdifactToken(String(code ?? '').toUpperCase(), 8)
  if (normalized && AGT_UE_ALLOWED_UTILTS_ERR_CODES.has(normalized)) return normalized

  const raw = String(sourceMessage.raw_payload ?? '').toUpperCase()
  if (raw.includes('LOC+239:') && !raw.includes('LOC+172:')) return 'E49'

  // In AGT UE1/UE2 the portal test verifies that the actor can reject E66 data
  // that its production application cannot process. E87 is valid in live/TGT for
  // period/resolution count mismatches, but Edielportalen rejects E87 for UE1/UE2
  // rule 531 where allowed reason codes are E10|E14|E49|E55|E61.
  return 'E10'
}

function normalizeAgtUeUtiltsErrMessageText(sourceMessage: EdielMessageRow, messageText?: string | null): string | null {
  if (!looksLikePortalAgtUeE66Source(sourceMessage)) return messageText ?? null

  const raw = sanitizeSegmentText(messageText) || 'E10'
  const tokens = raw.split(/[|,;\s]+/).filter(Boolean)
  const mapped = tokens.map((token) => {
    const [rawCode, ...referenceParts] = token.split('@')
    const code = agtUeAllowedUtiltsErrCodeFor(sourceMessage, rawCode)
    const reference = referenceParts.join('@')
    return reference ? `${code}@${reference}` : code
  })

  const finalTokens = mapped.length > 0 ? mapped : ['E10']
...
function buildAckDraft(params: {
  ...
  const effectiveMessageText =
    params.ackFamily === 'UTILTS_ERR'
      ? normalizeAgtUeUtiltsErrMessageText(params.sourceMessage, params.messageText ?? null)
  ...
  const utiltsErrSequenceToken =
    params.ackFamily === 'UTILTS_ERR'
      ? sanitizeEdifactToken(
          sanitizeSegmentText(effectiveMessageText)
            .split(/[|,;\s]+/)
