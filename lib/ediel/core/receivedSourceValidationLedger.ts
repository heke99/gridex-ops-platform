...
export async function recordReceivedSourceValidation(input: {
  ...
  try {
    const evidence = buildReceivedSourceValidationEvidence(input)
    ...
    return { status: 'recorded', sourceDisposition: 'not_established', assessmentId: data.assessmentId, factsHash }
