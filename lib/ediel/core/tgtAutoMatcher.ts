...
export function messageCodePrefixesForTgtAutoMatch(
...
export function facilityIdsFromTgtTestData(
  testData: EdielTgtCaseTestData | null | undefined,
): string[] {
  return fieldValuesFromTgtTestData(testData, ["209", "233"]).filter((value) =>
    /^735\d{15}$/.test(value),
  );
}
...
function rawFacilityIds(rawText: string): string[] {
  return unique(
    Array.from(rawText.matchAll(/735\d{15}/g)).map((match) => match[0]),
  );
}
...
function hasRawFacilityMismatch(
  message: EdielMessageRow,
  rawText: string,
): boolean {
...
function meterNumbersForLine(
  line: ReturnType<typeof parseEdifactMessageFacts>["lineItems"][number],
): string[] {
  return unique(
    line.segments
...
function payloadHasSameMeterNumber(message: EdielMessageRow): boolean {
  ...
  return facts.lineItems.some((line) => {
    const rawMeterNumbers = line.segments
      ...
      .filter(Boolean);
    return (
      rawMeterNumbers.length >= 2 &&
      new Set(rawMeterNumbers).size < rawMeterNumbers.length
    );
  });
}
...
function rawLooksLikeZ10MeterChangeContext(
  rawText: string | null | undefined,
): boolean {
  const text = String(rawText ?? "");
  // Z10M meter-change TGT rows include meter-number context. Field 214 is the
  // meter constant itself; if that field is absent while the row clearly has
  // Z10M meter-change fields, the semantic issue is constant_missing.
  return (
...
export function tgtRawTextLooksLikeConstantMissing(
  rawText: string | null | undefined,
): boolean {
  const text = String(rawText ?? "");
  return rawLooksLikeZ10MeterChangeContext(text) && !rawHasField(text, "214");
}
...
function valuesByFieldCodeFromRawText(
  rawText: string,
  fieldCode: string,
): string[] {
  const values: string[] = [];
...
export function tgtRawTextHasSameNewAndOldMeterNumber(
  rawText: string | null | undefined,
): boolean {
  const text = String(rawText ?? "");
...
export function tgtTestDataHasSameNewAndOldMeterNumber(
  testData: EdielTgtCaseTestData | null | undefined,
): boolean {
  if (!testData) return false;
  for (const group of testData.groups) {
    const newField = group.fields.find(
      (field) => String(field.fieldCode).toUpperCase() === "224",
    );
    const oldField = group.fields.find(
      (field) => String(field.fieldCode).toUpperCase() === "225",
    );
    if (!newField || !oldField) continue;
    const columnNames = new Set([
      ...Object.keys(newField.values),
      ...Object.keys(oldField.values),
    ]);
...
function textLooksLikeConstantMissing(rawText: string): boolean {
  const normalized = normalize(rawText);
  return (
    normalized.includes("KONSTANT SAKNAS") ||
    normalized.includes("METER CONSTANT MISSING")
  );
}
...
function textLooksLikeZ09F(rawText: string): boolean {
  const normalized = normalize(rawText);
  return (
    normalized.includes("Z09F") ||
    normalized.includes("AVTAL OM TIMVARDEN") ||
    normalized.includes("AGREEMENT HOURLY")
  );
}
...
function textLooksLikeZ09G(rawText: string): boolean {
  const normalized = normalize(rawText);
  return (
    normalized.includes("Z09G") ||
    normalized.includes("TIMVARDEN UPPHOR") ||
    normalized.includes("AGREEMENT ENDS")
  );
}
...
function textLooksLikeZ09D(rawText: string): boolean {
  const normalized = normalize(rawText);
  return (
    normalized.includes("Z09D") ||
    normalized.includes("MIKROPRODUKTION") ||
    normalized.includes("MICROPRODUCTION")
  );
}
...
export function effectiveTgtTestCaseCodeForMessageRow(
  ...
  const explicitCase = rawText.match(
    /\b(?:U\d\.\d\.\d+b?|\d\.\d\.\d+B?)\b/i,
  )?.[0];
  ...
  if (family === "UTILTS") {
    return inferTgtTestCaseCodeForInboundTestData({
      message,
      rawText,
      fallback: row.testCaseCode,
    });
  }
...
function cciCavValue(
  lineSegments: ReturnType<typeof parseEdifactMessageFacts>["segments"],
  cciCode: string,
): string | null {
...
function segmentFirstValue(
  segments: ReturnType<typeof parseEdifactMessageFacts>["segments"],
  prefix: string,
): string | null {
...
function partyIdFromNad(
  segments: ReturnType<typeof parseEdifactMessageFacts>["segments"],
  qualifier: string,
): string | null {
  const segment = segments.find((item) =>
    item.raw.startsWith(`NAD+${qualifier}+`),
  );
...
function lineDateTimeValue(
  segments: ReturnType<typeof parseEdifactMessageFacts>["segments"],
  qualifiers: string[],
): string | null {
  for (const qualifier of qualifiers) {
    const segment = segments.find((item) =>
      item.raw.startsWith(`DTM+${qualifier}:`),
    );
    const value =
      segment?.raw.replace(`DTM+${qualifier}:`, "").split(":")[0]?.trim() ?? "";
    if (value) return value;
  }
  return null;
}

function lineActualValue(
  line: ReturnType<typeof parseEdifactMessageFacts>["lineItems"][number],
  fieldCode: string,
): string | null {
  ...
  switch (code) {
    ...
    case "227":
      return (
        partyIdFromNad(line.segments, "UD") ??
        partyIdFromNad(line.segments, "IV")
      );
    case "228":
    case "229":
    case "231":
...
function fieldErrorText(
  ...
  if (code === "209")
    return "Anläggningsid avviker från Edielportalens testdata";
  if (code === "218")
    return "Antal siffror saknas eller avviker från Edielportalens testdata";
  if (code === "214")
    return "Konstant saknas eller avviker från Edielportalens testdata";
  if (code === "224")
    return "Mätarnummer saknas eller avviker från Edielportalens testdata";
...
function comparableFieldCodesForMessage(messageCode: string): Set<string> {
  ...
  if (messageCode === "Z06")
    return new Set([...common, "210", "217", "218", "222", "223"]);
  if (messageCode === "Z10")
    return new Set([...common, "210", "214", "217", "218", "223", "224"]);
  if (messageCode === "Z05") return new Set([...common, "210", "217", "223"]);
  if (messageCode === "Z09") return new Set([...common, "210", "217", "223"]);
  if (messageCode === "Z04")
    return new Set([...common, "210", "213", "214", "217", "223"]);
  if (messageCode === "Z03")
    return new Set([...common, "210", "213", "217", "223"]);
  ...
  for (const group of testData.groups) {
    const columns = [...group.columns].sort((a, b) => {
      const sourceOrderDiff =
        Number(a.sourceOrder ?? a.index) - Number(b.sourceOrder ?? b.index);
...
function expectedFacilityIdsForObject(object: TgtObjectValues): string[] {
  return [object.fields["209"], object.fields["233"]].filter(
    (value): value is string => Boolean(value && /^735\d{15}$/.test(value)),
  );
}

function matchExpectedObjectForLine(
  objects: TgtObjectValues[],
  lineItemId: string | null,
): TgtObjectValues | null {
  if (objects.length === 0) return null;
  if (lineItemId) {
    const exact = objects.find((object) =>
      expectedFacilityIdsForObject(object).some(
        (id) => normalizeCompare(id) === normalizeCompare(lineItemId),
      ),
    );
...
export function compareInboundPayloadToTgtTestData(params: {
  ...
  for (const line of facts.lineItems) {
    ...
    if (
      expectedFacilities.length > 0 &&
      line.itemId &&
      !expectedFacilities.some(
        (id) => normalizeCompare(id) === normalizeCompare(line.itemId),
      )
    ) {
      issues.push({
        fieldCode: "105",
    ...
    for (const [fieldCode, expected] of Object.entries(object.fields)) {
      ...
      if (
        ["228", "229", "231", "232", "234", "235", "236", "237"].includes(
          fieldCode,
        )
      ) {
...
export function inferTgtTestCaseCodeForInboundTestData(params: {
  ...
  if (
    explicit &&
    explicit.toLowerCase() !== "auto" &&
    tgtCaseCodeMatchesMessage(message, explicit)
  )
  ...
  if (family === "PRODAT") {
    if (code === "Z06") {
      ...
      if (payloadHasMissingDigitCount(message) || rawHasField(rawText, "218"))
        return "2.2.2";
    ...
    if (code === "Z10") {
      if (
        payloadHasSameMeterNumber(message) ||
        textLooksLikeZ10MeterNumberInvalid(rawText) ||
        tgtRawTextHasSameNewAndOldMeterNumber(rawText)
      )
        return "2.4.1";
      if (
        payloadHasMissingConstant(message) ||
        textLooksLikeConstantMissing(rawText) ||
        tgtRawTextLooksLikeConstantMissing(rawText)
      )
        return "2.4.2";
  ...
  return messageCodePrefixesForTgtAutoMatch(message)[0]
    ? `${messageCodePrefixesForTgtAutoMatch(message)[0]}.1`
    : "AUTO";
}

function tgtCaseCodeMatchesMessage(
  message: EdielMessageRow,
  ...
  return prefixes.some(
    (prefix) =>
      ...
      code.startsWith(`${prefix}B`),
  );
}

export function scoreTgtTestDataForMessage(
  message: EdielMessageRow,
  row: EdielTgtDynamicTestDataSummary,
): number {
  const family = String(message.message_family ?? "").toUpperCase();
  const suite =
    family === "UTILTS" ? "UTILTS" : family === "PRODAT" ? "PRODAT" : null;
  if (!suite || row.testSuite !== suite || row.roleCode !== "supplier")
    return -1;
  ...
  if (
    prefixes.some(
      (prefix) =>
        row.testCaseCode === prefix ||
        row.testCaseCode.startsWith(`${prefix}.`) ||
        row.testCaseCode.startsWith(`${prefix}b`),
    )
  ) {
    score += 20;
  }
  ...
  const rowCode = effectiveTgtTestCaseCodeForMessageRow(
    message,
    row,
  ).toUpperCase();
  ...
  if (expectedFacilities.length > 0) {
    ...
    matchingFacilities = actualFacilities.filter((id) =>
      expected.has(id),
    ).length;
    mismatchingFacilities = actualFacilities.filter(
      (id) => !expected.has(id),
    ).length;
  ...
  const isObjectFailureCase =
    rowCode === "1.3.1" || rowCode === "2.2.1" || rowCode === "3.2.1";
  const isDigitCountCase = rowCode === "2.2.2";
  const rowText = [row.title, row.sourceNote, row.rawText]
    .filter(Boolean)
    .join(" ");
  ...
  if (
    rowCode.startsWith("2.2") ||
    rowCode.startsWith("2.4") ||
    rowCode.startsWith("3.2") ||
    rowCode.includes("U2.2") ||
    rowCode.includes("U1.2") ||
    rowCode.includes("U1.4")
  ) {
    score += 15;
  }
...
export function rawTextHasSourceMessageMarker(
  rawText: string | null | undefined,
...
export function findExactTgtTestDataForMessage(
  ...
  return (
    markedRows.find((row) =>
      tgtCaseCodeMatchesMessage(
        message,
        effectiveTgtTestCaseCodeForMessageRow(message, row),
      ),
    ) ?? null
  );
}
