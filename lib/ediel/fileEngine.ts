...
export function resolveEdielTgtProdatApplicationReference(params?: {
  ...
  if (
    ...
    if (/^(8|9)\./.test(testCaseCode))
      return EDIEL_TGT_PRODAT_ESCO_APPLICATION_REFERENCE;
  }

  return EDIEL_TGT_PRODAT_APPLICATION_REFERENCE;
}

export type EdielFileEngineMode =
  | "tgt"
  | "agt"
  | "internal_test"
  | "production_dry_run";
...
type RegisterFileParams = {
  ...
  testPortalReceiverSubAddress?: string | null;
  testPortalSenderSubAddress?: string | null;
...
function firstElementComponent(
...
function detectAiList(
  rawPayload: string,
  fileName?: string | null,
): ParsedEdielFile | null {
...
function buildValidation(params: {
  ...
  if (
    !params.rawPayload.includes("UNB+") &&
    params.parsed.messageStandard === "edifact"
  ) {
    errors.push(
      "UNB saknas. Filen ser inte ut som komplett EDIFACT-interchange.",
    );
  }

  if (
    !params.rawPayload.includes("UNH+") &&
    params.parsed.messageStandard === "edifact"
  ) {
  ...
  if (params.parsed.messageFamily === "OTHER") {
    warnings.push(
      "Meddelandefamilj kunde inte identifieras som PRODAT/UTILTS/APERAK/CONTRL.",
    );
  }
  ...
  if (params.parsed.messageFamily === "PRODAT") {
    if (
      ...
      (params.mode === "tgt" || params.mode === "agt") &&
      !params.parsed.receiverEdielId
    ) {
      warnings.push(
        `${params.mode.toUpperCase()} outbound PRODAT saknar mottagande Ediel-ID i payloaden. Runtime kommer kräva DB-konfigurerad systemtestportal innan registrering/skick.`,
      );
    }
    if (
      (params.mode === "tgt" || params.mode === "agt") &&
      !params.parsed.receiverSubAddress &&
      params.direction === "outbound"
    ) {
      warnings.push(
        `${params.mode.toUpperCase()} PRODAT saknar mottagarsubadress i payloaden. Subadress ska komma från DB-konfigurerade systemtestinställningar om portalen kräver det.`,
      );
    }
    if (
      params.mode === "tgt" &&
      ...
      ].includes(params.parsed.applicationReference ?? "")
    ) {
      warnings.push(
        `TGT PRODAT för elmarknad ska normalt ha Application Reference ${EDIEL_TGT_PRODAT_APPLICATION_REFERENCE}; ESCO/tillståndstest använder ${EDIEL_TGT_PRODAT_ESCO_APPLICATION_REFERENCE}.`,
      );
    }
  }

  if (
    (params.parsed.messageFamily === "PRODAT" ||
      params.parsed.messageFamily === "UTILTS") &&
    !params.parsed.messageVersion
  ) {
    warnings.push(
      "Version saknas i UNH. Runtime kan falla tillbaka på regelmotor men filen bör innehålla version.",
    );
  }

  if (params.rawPayload.length > 10 * 1024 * 1024) {
    warnings.push(
      "Filen är större än 10 MB. Ediel-anvisningen anger 10 MB som rekommenderad maxstorlek för meddelandefiler.",
    );
  }
  ...
  return {
    ...
    report: {
      ...
      systemTestIdentitySource:
        "ediel_actor_settings + ediel_system_test_settings",
      tgtProdatApplicationReference: EDIEL_TGT_PRODAT_APPLICATION_REFERENCE,
      tgtProdatEscoApplicationReference:
        EDIEL_TGT_PRODAT_ESCO_APPLICATION_REFERENCE,
    },
  };
}

export function parseEdielFile(
  rawPayload: string,
  fileName?: string | null,
): ParsedEdielFile {
  const normalized = normalizeRawPayload(rawPayload);
  ...
  const interchangeReference =
    trimOrNull(unb?.elements[4]) ?? trimOrNull(unb?.elements[3]);
  const transactionReference =
    parseRffValue(segments, "TN") ?? parseRffValue(segments, "ACW");
  const externalReference =
    firstElementComponent(bgm?.elements[1], 0) ?? interchangeReference;
  const applicationReference =
    trimOrNull(unb?.elements[6]) ?? parseRffValue(segments, "APP");
  const processType = firstElementComponent(bgm?.elements[2], 0);
  const ackOutcome = inferAckOutcome({ family, segments });
  const originalMessageId =
    parseRffValue(segments, "ACE") ?? parseRffValue(segments, "ACD");
  const originalTransactionId =
    parseRffValue(segments, "AGO") ?? parseRffValue(segments, "TN");
  ...
  const parsedWithoutValidation = {
    ...
    correlationReference:
      originalMessageId ?? transactionReference ?? externalReference,
...
async function withVersionValidation(params: {
  ...
  if (params.direction === "outbound") {
    ...
    if (
      params.parsed.messageVersion &&
      resolvedVersion &&
      params.parsed.messageVersion !== resolvedVersion
    ) {
      warnings.push(
        `Filens version ${params.parsed.messageVersion} skiljer sig från runtime-version ${resolvedVersion}.`,
      );
    }
  } else {
...
export async function registerEdielFile(
  params: RegisterFileParams,
): Promise<EdielFileEngineRegisterResult> {
  ...
  const requiresSystemTestContext = mode === "tgt" || mode === "agt";
  const ownActorEdielId = trimOrNull(params.ownActorEdielId);
  const ownActorName = trimOrNull(params.ownActorName);
  const testPortalEdielId = trimOrNull(params.testPortalEdielId);
  const testPortalName =
    trimOrNull(params.testPortalName) ??
    (mode === "agt"
      ? "Edielportalen AGT"
      : mode === "tgt"
        ? "Edielportalen TGT"
        : null);
  const testPortalEmail = trimOrNull(params.testPortalEmail);
  const testPortalReceiverSubAddress = trimOrNull(
    params.testPortalReceiverSubAddress,
  );
  const testPortalSenderSubAddress = trimOrNull(
    params.testPortalSenderSubAddress,
  );

  if (requiresSystemTestContext && !ownActorEdielId) {
    throw new Error(
      "Systemtest/TGT kräver att bolagets test-Ediel-ID hämtas från ediel_actor_settings. Ingen hårdkodad aktör får användas.",
    );
  }

  if (requiresSystemTestContext && !testPortalEdielId) {
    throw new Error(
      "Systemtest/TGT kräver DB-konfigurerad testportal i ediel_system_test_settings. Ingen hårdkodad testmottagare får användas.",
    );
  }
  ...
  const input: CreateEdielMessageInput = {
    ...
    senderEdielId,
    senderName:
      senderEdielId === ownActorEdielId
        ? ownActorName
        : senderEdielId === testPortalEdielId
          ? testPortalName
          : null,
    senderSubAddress:
      parsed.senderSubAddress ??
      (params.direction === "outbound" && parsed.messageFamily === "PRODAT"
        ? testPortalSenderSubAddress
        : null),
    receiverEdielId,
    receiverName:
      receiverEdielId === testPortalEdielId
        ? testPortalName
        : receiverEdielId === ownActorEdielId
          ? ownActorName
          : null,
    receiverSubAddress:
    ...
    receiverEmail:
      ...
      (params.direction === "outbound" && (mode === "tgt" || mode === "agt")
        ? testPortalEmail
        : null),
    subject: params.subject ?? null,
    fileName: params.fileName ?? inferFileName(parsed),
    mimeType:
      parsed.messageStandard === "ai_list"
    ...
    parsedPayload: {
      ...
      fileEngine: {
        ...
        testPortalEdielId,
        systemTestSettingsSource: requiresSystemTestContext
          ? "ediel_system_test_settings"
          : null,
      },
    },
    ...
    failureReason:
      (version.validationReport.errors as string[] | undefined)?.join(" | ") ||
      null,
    messageReceivedAt:
      params.direction === "inbound" ? new Date().toISOString() : null,
    messageSentAt: null,
    ackDueAt:
      ack.requiresContrl || ack.requiresAperak ? computeAckDueAt() : null,
  };
  ...
  await createEdielMessageEvent({
    ...
    eventStatus:
      ((version.validationReport.errors as string[] | undefined)?.length ?? 0) >
      0
        ? "warning"
        : "success",
    message:
    ...
    payload: {
      ...
      activeActorEdielId: ownActorEdielId,
      testPortalEdielId,
      systemTestIdentitySource: requiresSystemTestContext
        ? "ediel_system_test_settings"
        : null,
    ...
    try {
      ...
      if (synced) {
        await createEdielMessageEvent({
          ...
          eventStatus: "success",
          message:
            "Filimporten synkades automatiskt till Aktörstest & Produktionssättning.",
          payload: {
      ...
      await createEdielMessageEvent({
        ...
        eventStatus: "warning",
        message:
          "Aktörstest-synk kunde inte köras automatiskt efter filimport.",
        payload: {
          actorTestingGlobalHook: true,
          source: "file_engine_register",
  ...
  return [
    {
      ...
      focus:
        "Skapa Z03, ta emot CONTRL + APERAK + Z04, skicka CONTRL + APERAK.",
    },
    {
      suite: "PRODAT",
      role: "supplier",
      ...
      focus:
        "Verifierar att systemet hanterar negativ APERAK utan att skapa dubbel APERAK.",
    },
    {
      suite: "PRODAT",
      role: "supplier",
