...
export function prodatRegisterFieldState(fieldNumberOrKey: string, source: readonly ProdatRegisterSegment[], una = parseUna(null)): ProdatRegisterFieldState | null {
  ...
  const submitted = (t: EdifactTokenizedSegment, first = 1) =>
    Array.from({length: Math.max(0, segmentElementCount(t, una) - first + 1)}, (_, i) => segmentComposite(t, first + i, una)).flat()
  if (field.linElement !== undefined) {
    ...
    return { failureEvidence:lin?prodatComponentEvidence(lin.raw,field.segmentPath,submitted(lin,4)):undefined, present: exists, value, malformed: exists && (parts.length !== 2 || parts[0] !== '1' || !prodatPositiveSequence(value) || segmentElementCount(lin!, una) > 4) }
  }
  if (field.fieldNumber === '213') {
    ...
    return { failureEvidence:found.flatMap(t=>{const p=segmentComposite(t,1,una);return prodatComponentEvidence(t.raw,field.segmentPath,submitted(t),found.length===1&&segmentElementCount(t,una)===1&&p.length<=3?[...(!p[1]||!/^\d{1,15}$/.test(p[1])?[1]:[]),...(p[2]&&!['KWH','MTQ'].includes(p[2])?[2]:[])]:undefined)}), present: found.length > 0, value, malformed: found.length > 0 && (found.length !== 1 || !value || !/^\d{1,15}$/.test(value) || parts.length > 3 || (Boolean(parts[2]) && !['KWH','MTQ'].includes(parts[2])) || segmentElementCount(found[0], una) !== 1) }
  }
