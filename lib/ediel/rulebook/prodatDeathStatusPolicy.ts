...
export function evaluateProdatDeathStatus(input:DeathPolicyInput){
 ...
 for(const message of messages(tokens)){
  diagnosticInput={...input,rawSegments:message.map(t=>t.raw)}
  ...
  if(outbound&&message.some(t=>t.tag==='CCI'&&segmentComposite(t,2,una)[0]==='Z17'&&!first.some(g=>g.segments.includes(t))))fail('OCCURRENCE_INVALID','kundstatus ligger utanför eget första register',undefined,true,'invalid')
  ...
  for(const group of first){
   ...
   if(outbound&&condition===false){if(selected.length)fail('FORBIDDEN','fältet får inte skickas för eget villkor',occurrence,true,'invalid');continue}
   if(!selected.length){if(condition===true)fail('REQUIRED','Z41 saknas',occurrence,true,'missing');continue}
   if(selected.length!==1){fail('OCCURRENCE_INVALID','flera kundstatusförekomster',occurrence,true,'invalid');continue}
