...
const ESCO_UTILTS: RulebookTestCase[] = [
  {
    testCaseCode: "UE1",
    suite: "AGT_UTILTS_ESCO",
    title: "UTILTS E66-KVART",
    role: "energy_service_company",
    family: "UTILTS",
    code: "E66",
    subtype: "KVART",
    processGroup: "meter_values",
    expectedContrl: "positive",
    expectedAperak: "positive",
    expectedUtiltsErr: "not_required",
    mandatory: true,
  },
  {
...
const TGT_ESCO: RulebookTestCase[] = [
  ...
  testCaseCode,
  suite: "LEGACY_TGT_ESCO_REGRESSION",
  title,
  role: "energy_service_company",
  family,
  code,
  ...
  expectedUtiltsErr: String(testCaseCode).includes("U3.2.2")
    ? "required"
    : "not_required",
  mandatory: false,
}));
...
export function listRulebookTestCases(): RulebookTestCase[] {
  ...
  const supplierCases: RulebookTestCase[] = supplierCaseRows.map(
    ([testCaseCode, title, family, code, subtype]) => ({
      testCaseCode,
      suite: String(testCaseCode).startsWith("UL")
        ? "AGT_UTILTS_SUPPLIER"
        : "AGT_PRODAT_SUPPLIER",
      title,
      role: "supplier",
...
export function findRulebookTestCase(
  testCaseCode: string | null | undefined,
): RulebookTestCase | null {
  const normalized = String(testCaseCode ?? "")
    .trim()
    .toUpperCase();
  return (
    listRulebookTestCases().find(
      (testCase) => testCase.testCaseCode.toUpperCase() === normalized,
    ) ?? null
  );
}
