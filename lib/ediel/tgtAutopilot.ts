...
export type EdielTgtAutopilotResult = {
  testRunId: string
  action: 'created_gridex_draft' | 'waiting_for_portal_file' | 'linked_imported_message' | 'created_mock_portal_message' | 'complete' | 'blocked'
...
function buildUnb(params: {
  ...
  const senderSub = params.family === 'PRODAT' || params.family === 'APERAK' || params.family === 'UTILTS_ERR'
    ? EDIEL_TGT_PRODAT_RECEIVER_SUB_ADDRESS
    : null
  const receiverSub = 'GRIDEX'

  const sender = senderSub
    ? `${EDIEL_TGT_TESTSYSTEM_EDIEL_ID}:ZZ:${senderSub}`
    : `${EDIEL_TGT_TESTSYSTEM_EDIEL_ID}:ZZ`
...
function buildMockBody(step: EdielTgtExpectedStep, refs: { date: string; longDate: string; transactionRef: string; originalInterchangeRef: string }) {
  ...
  if (step.family === 'APERAK' || step.family === 'UTILTS_ERR') {
    ...
    return [
      ...
      isNegative ? 'FTX+AAO+++SIMULERAT FEL FRAN EDIELPORTALEN' : 'FTX+AAO+++OK',
    ]
  }
  ...
  if (step.family === 'PRODAT') {
    return [
      ...
      `NAD+MS+${EDIEL_TGT_TESTSYSTEM_EDIEL_ID}::9++EDIELPORTALEN`,
      `NAD+MR+${GRIDEX_EDIEL_ID}::9++GRIDEX`,
...
function buildMockPortalInput(params: {
  ...
  const interchangeRef = `PORTAL${safeCase}S${params.step.stepNo}${parts.compact}`.slice(0, 35)
  const messageRef = `P${safeCase}${params.step.stepNo}${parts.compact}`.slice(0, 14)
  ...
  const body = buildMockBody(params.step, {
    date: parts.yyMMdd,
    longDate: parts.yyyyMMdd,
    transactionRef,
    originalInterchangeRef: original?.interchange_reference ?? `GRIDEX-${safeCase}-S${Math.max(1, params.step.stepNo - 1)}`,
  })
  ...
  const isAck = params.step.family === 'CONTRL' || params.step.family === 'APERAK' || params.step.family === 'UTILTS_ERR'
  ...
  return {
    ...
    senderEdielId: EDIEL_TGT_TESTSYSTEM_EDIEL_ID,
    senderSubAddress: params.step.family === 'PRODAT' || params.step.family === 'APERAK' || params.step.family === 'UTILTS_ERR'
      ? EDIEL_TGT_PRODAT_RECEIVER_SUB_ADDRESS
      : null,
    receiverEdielId: GRIDEX_EDIEL_ID,
    receiverSubAddress: 'GRIDEX',
    senderEmail: '91100@ediel.se',
    receiverEmail: 'ediel@gridex.se',
    ...
    validationReport: {
      ok: true,
      mockOnly: true,
      warning: 'Internt simulerat portalsvar. Ska inte skickas till Edielportalen.',
    },
...
async function getRunEvaluation(testRunId: string): Promise<EdielTgtRunEvaluation> {
...
export async function runTgtAutopilotForRun(params: {
  ...
  const step = evaluation.definition.expectedSteps.find((candidate) => candidate.stepNo === nextAction.stepNo)
...
export async function createMockPortalMessageForNextStep(params: {
  ...
  if (step.actor !== 'portal') {
    return {
      ...
      messageId: null,
      stepNo: step.stepNo,
      description: 'Nästa steg ägs av Gridex. Kör autopilot för att skapa filutkast i stället.',
    }
  }

  const message = await createEdielMessage(buildMockPortalInput({
    actorUserId: params.actorUserId,
  ...
  return {
    testRunId: evaluation.testRun.id,
    action: afterMock.action === 'created_gridex_draft' ? 'created_mock_portal_message' : 'created_mock_portal_message',
    messageId: message.id,
    stepNo: step.stepNo,
    description: afterMock.action === 'created_gridex_draft'
...
export async function autoAttachImportedMessageToActiveTgtRun(params: {
  ...
  for (const run of activeRuns) {
    ...
    const matchingStep = evaluation.definition.expectedSteps.find((step) => {
      ...
      if (String(step.code).toUpperCase() !== String(params.edielMessage.message_code).toUpperCase()) return false
      if (step.outcome && params.edielMessage.ack_outcome && step.outcome !== params.edielMessage.ack_outcome) return false
      return true
    })
