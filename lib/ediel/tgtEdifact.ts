...
function requireSystemTestToken(
...
function testActorId(params: SystemTestContextParams): string {
...
function testPortalId(params: SystemTestContextParams): string {
  return requireSystemTestToken(
...
function testSenderSubaddress(params: SystemTestContextParams): string | null {
  return (
    params.systemTestContext.senderSubaddress?.trim().toUpperCase() || null
  );
}

function testReceiverSubaddress(
...
function testPortalEmail(params: SystemTestContextParams): string | null {
...
function sortColumnsBySourceOrder<T extends OrderedTgtColumn>(
  columns: readonly T[],
): T[] {
...
type TestDataLookupParams = Pick<
...
function inferCustomerIdCodeListQualifier(
...
function defaultPowerOfAttorneyReference(
...
function fallbackEscoPermissionMeteringPointId(
  params: TestDataLookupParams,
  step: EdielTgtExpectedStep,
): string {
...
function date102FromPortalDate(
  value: string | null | undefined,
  fallback: string,
): string {
  const token = firstToken(value);
...
function date203FromPortalDate(
  value: string | null | undefined,
  fallback: string,
): string {
  const token = firstToken(value);
...
function isZ09DTransaction(
  transactionType: string | null | undefined,
): boolean {
  return normalizeTgtCode(transactionType) === "Z09D";
}

function buildZ09FOrZ09GLineDateSegment(
  portalData: TgtPortalCustomerData,
  refs: DraftReferences,
): string {
  const dateSource =
...
function buildZ09DLineDateSegments(
  portalData: TgtPortalCustomerData,
  refs: DraftReferences,
): string[] {
...
function expectedZ09LineDateSegments(
  portalData: TgtPortalCustomerData,
  refs: DraftReferences,
): string[] {
...
function permissionPurposeForTransaction(
  transactionType: string,
...
function buildPortalProdatSegments(
  params: EdielTgtDraftBuildParams,
  step: EdielTgtExpectedStep,
  refs: DraftReferences,
): {
...
function buildUtiltsE31SchDraftBody(
  params: EdielTgtDraftBuildParams,
  refs: DraftReferences,
): string[] {
...
function prodatStepRequiresRegisterCoverage(
  step: EdielTgtExpectedStep,
): boolean {
...
function prodatStepRequiresMeteringMethod(
  step: EdielTgtExpectedStep,
  portalData: TgtPortalCustomerData,
): boolean {
  if (step.family !== "PRODAT") return false;
