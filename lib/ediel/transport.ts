...
import { getEdielRouteProfileByCommunicationRouteId, createEdielMessage } from '@/lib/ediel/db'
import type { EdielMessageRow } from '@/lib/ediel/types'
import { buildInboundUtiltsMessageInput } from '@/lib/ediel/utilts'
import { parseInboundProdat } from '@/lib/ediel/prodat'
import { inferEdielFamilyAndCodeFromRawPayload, inferEdielFileName } from '@/lib/ediel/classify'
...
export async function sendEdielMessageViaSmtp(message: EdielMessageRow): Promise<{
  ...
  const user = requireEnv('EDIEL_SMTP_USER', routeProfile?.mailbox ?? process.env.EDIEL_SMTP_USER ?? null)
  ...
  const result = await transporter.sendMail({
    from,
    to: message.receiver_email ?? undefined,
...
export async function pollEdielMailboxViaImap(params?: {
  ...
  const user = requireEnv('EDIEL_IMAP_USER', routeProfile?.mailbox ?? params?.mailbox ?? null)
        ...
        if (inferred.messageFamily === 'UTILTS') {
          createdMessage = await createEdielMessage(
            buildInboundUtiltsMessageInput({
              ...
              mailbox,
              mailboxMessageId: String(item.uid),
