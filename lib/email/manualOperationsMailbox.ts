...
export async function resolveManualOperationsMailbox(input: {
  ...
  const { data, error } = await supabaseService
    ...
    .select('id,company_id,mailbox_type,environment,from_email,reply_to_email,imap_host,imap_port,imap_username,imap_secret_reference,imap_folder,imap_secure,is_active,is_verified,metadata')
    .eq('is_active', true)
  ...
  candidates.sort(
    (a, b) =>
      scoreMailbox(b, { companyId: input.companyId, environment, preferredType }) -
