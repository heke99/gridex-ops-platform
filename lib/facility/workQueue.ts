...
type PowerOfAttorneyDocumentRow = {
  id: string;
  company_id: string | null;
  customer_id: string | null;
...
function isMissingSchemaError(error: unknown): boolean {
  ...
  return Boolean(
    ...
    (["42P01", "42703", "PGRST202", "PGRST204", "PGRST205"].includes(
      ...
      /does not exist|schema cache|relation .* does not exist|column .* does not exist|function .* not found/i.test(
        maybe.message ?? "",
      )),
  );
}
...
function rpcRows(data: unknown): RawRow[] {
  ...
  return data.filter(
    (row): row is RawRow => Boolean(row) && typeof row === "object",
  );
}
...
function normalizeStatus(value: unknown): FacilityWorkQueueStatus {
  ...
  if (normalized === "needs_grid_owner_review")
    return "needs_grid_owner_review";
...
function meteringPointLabel(
  point: MeteringPointRow | undefined,
): string | null {
  if (!point) return null;
  return (
    point.meter_point_id?.trim() || point.metering_point_id?.trim() || point.id
  );
}

function isSignedPowerOfAttorney(row: PowerOfAttorneyRow): boolean {
  const evidence =
    row.document_path?.trim() ||
    row.signed_at ||
    row.accepted_at ||
    row.reference ||
    row.fullmakt_snapshot;
  return row.status === "signed" && Boolean(evidence);
}

function isAvailablePowerOfAttorneyDocument(
  row: PowerOfAttorneyDocumentRow,
): boolean {
  return (
    row.document_type === "power_of_attorney" &&
    ["available", "uploaded", "signed", "active"].includes(
      String(row.status ?? "available").toLowerCase(),
    )
  );
}
...
function buildFallbackDescription(
  ...
  if (status === "awaiting_grid_owner")
    return `Uppgifter är begärda${gridOwnerName ? ` från ${gridOwnerName}` : " från nätägare"}. Följ upp svar och koppla inkommen Z02/manuellt svar till kundkortet.`;
  if (status === "missing_authorization")
    return "Kunden kan ligga kvar i OPS, men utskick till nätägare stoppas tills signerad fullmakt finns.";
  if (status === "needs_grid_owner_review")
    return "Adress/postnummer kan ge förslag, men verifierad nätägare eller nätområdeskod saknas. Kontrollera resolver-resultat innan switch.";
  if (missingFields.length === 0)
    return "Anläggningsdata ser komplett ut. Nästa steg är leverantörsbyte eller aktiv kundprocess.";
...
function mapRpcRow(row: RawRow): FacilityWorkQueueRow {
  ...
  return {
    ...
    href:
      stringOrNull(row.href) ??
      `/admin/customers/${String(row.customer_id)}?tab=data-requests`,
  };
}
...
async function fallbackFacilityQueue(
  ...
  let siteQuery = supabase
    .from("customer_sites")
    .select(
      "id,company_id,customer_id,site_name,facility_id,grid_owner_id,price_area_code,status,street,postal_code,city,created_at,updated_at",
    )
    ...
    customerIds.length
      ? safeSelect<CustomerRow>(
          supabase
            .from("customers")
            .select(
              "id,company_id,customer_number,full_name,first_name,last_name,company_name,email",
            )
            .in("id", customerIds),
        )
      : Promise.resolve([]),
    siteIds.length
      ? safeSelect<MeteringPointRow>(
          supabase
            ...
            .select(
              "id,company_id,customer_id,site_id,meter_point_id,metering_point_id,grid_owner_id,price_area_code,status,created_at,updated_at",
            )
            .in("site_id", siteIds),
        )
      : Promise.resolve([]),
    customerIds.length
      ? safeSelect<RequestRow>(
          supabase
            ...
            .select(
              "id,customer_id,site_id,metering_point_id,status,target_party_type,request_type,created_at,updated_at",
            )
    ...
    customerIds.length
      ? safeSelect<RequestRow>(
          supabase
            ...
            .select(
              "id,customer_id,site_id,metering_point_id,status,request_scope,created_at,updated_at",
            )
            .in("customer_id", customerIds)
    ...
    customerIds.length
      ? safeSelect<PowerOfAttorneyRow>(
          supabase
            ...
            .select(
              "id,company_id,customer_id,site_id,status,scope,document_path,signed_at,accepted_at,reference,fullmakt_snapshot",
            )
            .in("customer_id", customerIds),
        )
      : Promise.resolve([]),
    customerIds.length
      ? safeSelect<PowerOfAttorneyDocumentRow>(
          supabase
            .from("customer_documents")
            .select(
              "id,company_id,customer_id,power_of_attorney_id,document_type,status",
            )
            .in("customer_id", customerIds)
  ...
  for (const power of powersOfAttorney) {
    ...
    if (isSignedPowerOfAttorney(power))
      signedPowerByCustomerId.set(power.customer_id, true);
  }
  for (const document of powerOfAttorneyDocuments) {
    if (!document.customer_id) continue;
    if (isAvailablePowerOfAttorneyDocument(document))
      signedPowerByCustomerId.set(document.customer_id, true);
  }
  ...
  for (const site of sites) {
    ...
    const missingFields = [
      ...
      points.some(
        (point) =>
          point.meter_point_id?.trim() || point.metering_point_id?.trim(),
      )
        ? null
        : "metering_point_id",
      signedPowerByCustomerId.get(site.customer_id)
        ? null
        : "power_of_attorney",
    ].filter((value): value is string => Boolean(value));
    const activeRequests = [
      ...(requestsBySiteId.get(site.id) ?? []),
      ...(requestsBySiteId.get(`customer:${site.customer_id}`) ?? []),
    ];
    ...
    const status: FacilityWorkQueueStatus = waitingForGridOwner
      ...
      : missingFields.includes("power_of_attorney")
        ...
        : missingFields.includes("grid_owner")
          ? "needs_grid_owner_review"
          : "needs_facility_data";
    const priority: FacilityWorkQueuePriority =
      status === "missing_authorization" || status === "needs_grid_owner_review"
        ? "high"
        : "normal";
    const customer = customersById.get(site.customer_id);
    const gridOwner = effectiveGridOwnerId
      ? gridOwnersById.get(effectiveGridOwnerId)
      : undefined;
