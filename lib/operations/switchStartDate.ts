...
function noticeFloorFromPeriod(
  ...
  if (typeof period === "number" && Number.isFinite(period) && period > 0) {
    return addDays(from, Math.ceil(period));
  }
  ...
  return addDays(from, Math.ceil(amount));
}
...
export function calculateEarliestSwitchStartDate(
  ...
  if (requested) {
    if (requested >= earliest) {
      ...
      // Honor the requested date object but flag it; do not silently move it.
      effective = requested;
      requestedHonored = true;
      requestedTooEarly = true;
      reason =
        "Begärt startdatum är tidigare än tidigaste giltiga datum. Kräver granskning innan utskick.";
    }
  } else {
    effective = earliest;
    reason = isMove
