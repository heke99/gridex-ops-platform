...
export const REQUIRED_LEGAL_TEXT_TYPES = ['terms', 'privacy_policy', 'withdrawal', 'power_of_attorney', 'price_terms'] as const
...
export type CustomerOpsBlocker = {
  ...
  label: string
  action: string
  tab: 'legal-readiness' | 'authorization-documents' | 'data-requests' | 'contracts' | 'sites' | 'metering-points' | 'switch-operations' | 'communication' | 'overview'
}
...
export type CustomerOpsReadiness = {
  ...
  nextAction: { label: string; hrefTab: CustomerOpsBlocker['tab']; description: string }
}
...
export async function listCompanyLegalTextVersions(companyId: string): Promise<LegalTextVersion[]> {
...
export async function getTenantWebsiteReadiness(companyId: string): Promise<TenantWebsiteReadiness | null> {
  const { data, error } = await supabaseService
...
export async function listCustomerLegalAcceptances(companyId: string, customerId: string): Promise<CustomerLegalAcceptance[]> {
  const { data, error } = await supabaseService
    .from('customer_legal_acceptances')
    .select('id,company_id,customer_id,contract_id,contract_application_id,acceptance_type,legal_text_version_id,accepted_at,accepted_ip,accepted_user_agent,source,snapshot,metadata,reason')
    .eq('company_id', companyId)
...
export async function listCustomerDocuments(companyId: string, customerId: string): Promise<CustomerDocument[]> {
  const { data, error } = await supabaseService
    .from('customer_documents')
    .select('id,company_id,customer_id,contract_id,document_type,document_version,title,file_name,storage_key,file_path,source,status,created_at,metadata')
    .eq('company_id', companyId)
    .eq('customer_id', customerId)
    .order('created_at', { ascending: false })
    .limit(100)
  ...
  return ((data ?? []) as CustomerDocument[]).map((row) => ({ ...row, metadata: jsonObj(row.metadata) }))
}

export async function listCustomerOpsTimeline(companyId: string, customerId: string): Promise<CustomerOpsTimelineEvent[]> {
  const { data, error } = await supabaseService
  ...
  return ((data ?? []) as CustomerOpsTimelineEvent[]).map((row) => ({ ...row, metadata: jsonObj(row.metadata) }))
}

function hasAcceptance(rows: CustomerLegalAcceptance[], type: string): boolean {
  return rows.some((row) => row.acceptance_type === type)
}

function recordDateActive(row: Record<string, unknown>): boolean {
  const status = String(row.status ?? '').toLowerCase()
  const validStatus = ['signed', 'accepted', 'active', 'completed'].includes(status)
...
function poaAllows(row: Record<string, unknown>, scope: string): boolean {
  ...
  if (directScope === 'all' || directScope === 'supplier_switch') return ['supplier_switch', 'facility_data_request', 'metering_point_lookup', 'ediel_communication'].includes(scope)
  const summary = jsonObj(row.scope_summary)
  if (boolish(summary[scope])) return true
  const metadata = jsonObj(row.metadata)
  const scopes = Array.isArray(metadata.scopes) ? metadata.scopes.map(String) : []
...
export function evaluateCustomerOpsMasterReadiness(input: {
  ...
  const hasPriceSnapshot = hasAcceptance(acceptances, 'price_snapshot') || contracts.some(hasSnapshot)
  const hasPowerOfAttorneyAcceptance = hasAcceptance(acceptances, 'power_of_attorney') || activePoaRows.length > 0
  const hasActivePowerOfAttorney = activePoaRows.some((row) => poaAllows(row, 'supplier_switch'))
  const hasFacilityPoa = activePoaRows.some((row) => poaAllows(row, 'facility_data_request') || poaAllows(row, 'metering_point_lookup'))
  const hasContractSnapshot = contracts.some(hasSnapshot)
  const hasFacility = sites.length > 0
  const hasMeteringPoint = meteringPoints.some((row) => Boolean(str(row, 'meter_point_id', 'metering_point_id', 'ediel_metering_point_id')))
  const hasGridOwner = sites.some((row) => Boolean(str(row, 'grid_owner_id', 'grid_owner_code'))) || meteringPoints.some((row) => Boolean(str(row, 'grid_owner_id')))
  const hasGridArea = sites.some((row) => Boolean(str(row, 'grid_area_code'))) || meteringPoints.some((row) => Boolean(str(row, 'grid_area_code')))
  ...
  if (!hasPriceSnapshot) add('price_snapshot_missing', 'Prissnapshot saknas', 'Spara exakt avtal/prisversion/pristext som kunden tecknade.', 'contracts')
  ...
  if (!hasActivePowerOfAttorney) add('poa_missing', 'Aktiv fullmakt saknas', 'Lägg in eller begär fullmakt med rätt scope för leverantörsbyte.', 'authorization-documents')
  if (!hasContractSnapshot) add('contract_snapshot_missing', 'Avtalssnapshot saknas', 'Spara avtalssnapshot innan leverantörsbyte eller fakturering används.', 'contracts')
  ...
  if (!hasEdielRoute) add('ediel_route_missing', 'Ediel-route saknas', 'Kontrollera route, subadress och certifikat innan PRODAT skickas.', 'switch-operations')

  const canRequestFacilityData = hasFacilityPoa && (hasGridOwner || hasGridArea)
  ...
  const nextAction = first
    ...
    : { label: 'Starta eller fortsätt leverantörsbyte', hrefTab: 'switch-operations' as const, description: 'Alla juridiska och operativa grundkrav är uppfyllda.' }
