...
function canonicalPricingModel(
  contractType?: string | null,
  billingModel?: string | null,
): CanonicalContractSnapshot["pricingModel"] {
  const raw = `${billingModel ?? ""} ${contractType ?? ""}`
    .trim()
    .toLowerCase();
...
function normalizedWeights(
  input: ContractSnapshotOfferInput,
  pricingModel: CanonicalContractSnapshot["pricingModel"],
) {
  const explicit = {
  ...
  if (pricingModel === "mixed") {
    if (Math.abs(explicitTotal - 100) > 0.0001) {
      throw new Error(
        `Mixpris måste summera till 100 %. Nuvarande summa är ${explicitTotal} %.`,
      );
    }
    return explicit;
  }

  if (pricingModel === "portfolio")
    return { spot: 0, portfolio: 100, fixed: 0 };
  if (pricingModel === "fixed") return { spot: 0, portfolio: 0, fixed: 100 };
  if (pricingModel === "manual_override")
    return { spot: 0, portfolio: 0, fixed: 0 };
...
function componentRow(component: PriceComponent): Record<string, unknown> {
  return {
    ...
    calculation_base: component.calculationBase ?? null,
...
export function buildCanonicalContractSnapshot(
  input: ContractSnapshotOfferInput,
): CanonicalContractSnapshot {
  const pricingModel = canonicalPricingModel(
    input.contractType,
    input.billingModel,
  );
  ...
  if (weights.spot > 0) {
    basePriceComponents.push(
      baseRow({
        sourceType: "spot",
        weightPercent: weights.spot,
        label:
          pricingModel === "mixed" ? "Rörlig spotandel" : "Rörligt spotpris",
  ...
  if (weights.fixed > 0) {
    if (fixedPriceOre === null)
      throw new Error("Fastpris saknas för fast eller mixad prisandel.");
    basePriceComponents.push(
      baseRow({
        sourceType: "fixed",
  ...
  const spotMarkup =
    numeric(input.spotMarkupOrePerKwh) ?? numeric(input.markupOrePerKwh);
  if (spotMarkup !== null)
    push({
...
export function assertCanonicalSnapshot(
  input: CanonicalContractSnapshot,
): void {
  if (input.basePriceComponents.length === 0)
    throw new Error("Avtalets prisbas saknas.");
  const totalWeight = input.basePriceComponents.reduce(
    (sum, row) => sum + (numeric(row.weight_percent) ?? 0),
    0,
  );
  if (Math.abs(totalWeight - 100) > 0.0001)
    throw new Error(
      `Avtalets prisandelar måste summera till 100 %. Nuvarande summa är ${totalWeight} %.`,
    );
}
