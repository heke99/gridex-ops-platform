...
function addIssue(target: RouteDecisionIssue[], issue: Omit<RouteDecisionIssue, 'severity'> & { severity?: 'warning' | 'blocking' }) {
  target.push({ ...issue, severity: issue.severity ?? 'blocking' })
}

function addTrace(target: RouteDecisionTraceEntry[], entry: RouteDecisionTraceEntry) {
  target.push(entry)
}
...
async function findRoute(params: {
...
}): Promise<{ route: RouteRow | null; ambiguous: boolean; matches: RouteRow[] }> {
  if (params.preferredRouteId) {
    ...
    if (params.companyId) explicit = explicit.or(`company_id.is.null,company_id.eq.${params.companyId}`)

    const { data, error } = await explicit.limit(2)
    if (error) throw error
    const rows = (data ?? []) as RouteRow[]
    if (rows.length === 1) return { route: rows[0], ambiguous: false, matches: rows }
  }
  ...
  if (params.companyId) exact = exact.or(`company_id.is.null,company_id.eq.${params.companyId}`)

  const exactResult = await exact.order('company_id', { ascending: false }).order('updated_at', { ascending: false }).limit(3)
  if (exactResult.error) throw exactResult.error
  const exactRows = (exactResult.data ?? []) as RouteRow[]
  if (exactRows.length === 1) return { route: exactRows[0], ambiguous: false, matches: exactRows }
  if (exactRows.length > 1) return { route: null, ambiguous: true, matches: exactRows }
  ...
  if (params.companyId) fallback = fallback.or(`company_id.is.null,company_id.eq.${params.companyId}`)

  const fallbackResult = await fallback.order('company_id', { ascending: false }).order('updated_at', { ascending: false }).limit(3)
  if (fallbackResult.error) throw fallbackResult.error
  const fallbackRows = (fallbackResult.data ?? []) as RouteRow[]
  if (fallbackRows.length === 1) return { route: fallbackRows[0], ambiguous: false, matches: fallbackRows }
  return { route: null, ambiguous: fallbackRows.length > 1, matches: fallbackRows }
}

async function findRouteProfile(routeId: string, companyId?: string | null): Promise<RouteProfileRow | null> {
  ...
  const { data, error } = await query.order('company_id', { ascending: false }).order('updated_at', { ascending: false }).limit(1).maybeSingle()
  if (error) throw error
...
function compactPayload(decision: RouteDecisionOutput): Record<string, unknown> {
  return {
...
export async function createRouteAdminTasks(input: RouteDecisionInput, decision: RouteDecisionOutput): Promise<void> {
  ...
  const { error } = await supabaseService.from('customer_operation_tasks').insert(tasks)
  if (error) console.warn('[routeDecisionEngine] Kunde inte skapa admin tasks', error)
}

export async function decideCommunicationRoute(input: RouteDecisionInput): Promise<RouteDecisionOutput> {
  ...
  if (!input.companyId) {
    addIssue(blockingReasons, {
      ...
      message: 'company_id saknas. Systemet får inte skicka eller uppdatera kritisk data utan tenant.',
      source: 'tenant_guard',
    })
    requiredAdminActions.push('Välj eller reparera bolagskoppling.')
  }

  if (['supplier_switch', 'metering_access', 'customer_masterdata'].includes(input.businessProcess) && !input.customerId) {
    addIssue(blockingReasons, {
  ...
  if (['supplier_switch', 'metering_access', 'meter_values', 'customer_masterdata'].includes(input.businessProcess) && !input.gridOwnerId) {
    addIssue(blockingReasons, {
      code: 'missing_grid_owner_id',
      message: 'grid_owner_id saknas. Systemet kan inte välja mottagande nätägare.',
      source: 'preflight',
    })
    requiredAdminActions.push('Koppla nätägare på anläggning/mätpunkt.')
  }

  if (input.businessProcess === 'metering_access' && routeScope !== 'metering_access') {
    addIssue(blockingReasons, {
  ...
  const agreementDecision = await resolveGridOwnerAgreementReference({
    ...
    routeScope,
    requireActiveAgreement: requiresGridOwnerAgreement(input.businessProcess, messageCode),
  })
  ...
  if (agreementDecision.status === 'ambiguous') {
    addIssue(blockingReasons, {
      ...
      message: agreementDecision.reasons[0] ?? 'Flera aktiva nätägaravtal matchar.',
      source: 'agreement_resolver',
    })
  ...
  if (input.businessProcess === 'metering_access' && !agreementDecision.agreementReference) {
    addIssue(blockingReasons, {
  ...
  const routeResult = await findRoute({
    ...
    routeScope,
    gridOwnerId: input.gridOwnerId ?? null,
    preferredRouteId: agreementDecision.preferredRouteId ?? input.preferredRouteId ?? null,
  })
  ...
  if (routeResult.ambiguous) {
    ...
    requiredAdminActions.push('Välj route manuellt eller inaktivera dubbletter.')
  }
  ...
  if (routeResult.route) {
    addTrace(trace, {
      ...
      metadata: { routeId: routeResult.route.id, routeType: routeResult.route.route_type },
    })
  }
  ...
  if (routeResult.route?.route_type === 'email_manual' && input.businessProcess === 'supplier_switch') {
    addIssue(blockingReasons, {
      ...
      message: 'Z03 får inte skickas via vanlig mail som huvudflöde. Skapa Ediel/PRODAT-route för supplier_switch.',
      source: 'route_guard',
    })
  }

  if (['PRODAT', 'UTILTS', 'APERAK', 'CONTRL', 'UTILTS_ERR'].includes(messageFamily)) {
    if (!senderEdielId) {
      addIssue(blockingReasons, {
    ...
    if (!receiverEdielId) {
      addIssue(blockingReasons, {
        ...
        message: 'receiver Ediel-id saknas på route profile/nätägare.',
        source: 'route_profile_resolver',
      })
    }
  }
  ...
  if (guardIssues.length > 0) requiredAdminActions.push('Byt till riktig produktionsroute och mottagare.')

  const subtype = input.businessProcess === 'supplier_switch'
    ? supplierSwitchSubtype({
...
export function routeDecisionPayload(decision: RouteDecisionOutput): Record<string, unknown> {
