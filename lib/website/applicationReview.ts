...
function isControlledFacilityStatus(
  value: string | null,
): value is ControlledFacilityStatus {
...
function humanFacilityIssue(status: ControlledFacilityStatus): ReviewIssue {
  ...
  if (status === "duplicate_facility_id") {
    return {
      ...
      severity: "blocking",
      message:
        "Anläggnings-ID finns redan hos samma bolag. Systemet skapar inte en dubblett.",
      action:
        "Granska befintlig kund/anläggning och länka eller rätta uppgiften innan ny readiness-check.",
    };
  }
  if (status === "protected_identity") {
    return {
      ...
      action:
        "Flytta ärendet till behörig handläggare och följ skyddad-identitet-processen.",
    };
  }
  if (GRID_OWNER_FOLLOWUP_STATUSES.has(status)) {
    return {
      ...
      severity: "blocking",
      message:
        "Nätägarens svar måste tolkas affärsmässigt. Leverantörsbyte får inte fortsätta automatiskt.",
      action:
        "Kontrollera nätägarens svar, begär rätt uppgifter vid behov och kör ny readiness-check efter korrigering.",
    };
  }
  return {
    field: status,
    ...
    severity: "blocking",
    message:
      "Anläggnings-ID, mätpunkt, kundidentitet eller nätområde kunde inte verifieras säkert.",
    action:
      "Kontrollera uppgifterna med kunden, begär rätt uppgifter från nätägare eller ladda upp elnätsfaktura. Kör ny readiness-check efter rättning.",
  };
}

export function customerIntakeStatusForReadiness(
  readiness: Pick<
    WebsiteApplicationReadiness,
    "status" | "missingFields" | "blockingReasons" | "canStartSwitch"
  >,
  ...
  if (
    ...
    return "blocked";
  if (readiness.status === "duplicate_facility_id")
    return "pending_duplicate_review";
  if (
    [
      "facility_data_invalid",
      "customer_information_mismatch",
  ...
  if (
    ["pending_validation", "facility_data_received"].includes(readiness.status)
  )
  ...
  if (blockingFields.length === 0 && readiness.canStartSwitch)
    return "ready_for_operations";
  if (blockingFields.length === 1 && missing.has("power_of_attorney"))
    return "pending_power_of_attorney";
  if (
    missing.has("grid_area_code") ||
    missing.has("price_area") ||
    missing.has("grid_owner") ||
    missing.has("metering_point_id") ||
    missing.has("facility_verified") ||
    missing.has("site")
  )
    return "pending_information";
...
function addIssue(
  issues: ReviewIssue[],
  input: Omit<ReviewIssue, "severity"> & { severity?: ReviewIssue["severity"] },
) {
...
export function readRequestedStartMode(
  input: unknown,
): "earliest_possible" | "specific_date" {
  const mode = firstText(input, [
    "requested_start_mode",
    "requestedStartMode",
    "contract.requested_start_mode",
    "contract.requestedStartMode",
  ])?.toLowerCase();
...
function readCalculatedEarliestStartDate(input: unknown): string | null {
  return firstText(input, [
    ...
    "contract.calculatedEarliestStartDate",
  ]);
}

export function assessWebsiteApplicationReadiness(
  input: unknown,
): WebsiteApplicationReadiness {
  ...
  const customerIdentity = firstText(input, [
    ...
    "customer.personal_identity_number",
  ...
  const postalCode = firstText(input, [
    "site.postal_code",
    "site.postalCode",
    "postal_code",
    "postalCode",
  ...
  const gridAreaCode = normaliseGridAreaCode(
    firstText(input, [
      "grid_area_code",
      "gridAreaCode",
      "site.grid_area_code",
      "site.gridAreaCode",
      "metadata.grid_area_code",
      "energy_resolution.gridAreaCode",
    ]),
  );
  const priceArea = normalisePriceArea(
    firstText(input, [
      "price_area_code",
      "priceAreaCode",
      "price_area",
      ...
      "site.priceAreaCode",
      "metering_point.price_area_code",
      "energy_resolution.priceArea",
    ]),
  );
  const resolutionStatus = firstText(input, [
    "resolution_status",
    "resolutionStatus",
    "site.resolution_status",
    "energy_resolution.resolutionStatus",
  ]);
  const gridOwnerVerificationStatus = firstText(input, [
    "grid_owner_verification_status",
    "gridOwnerVerificationStatus",
    "site.grid_owner_verification_status",
    "energy_resolution.gridOwnerVerificationStatus",
  ]);
  ...
  const informationRequestId = firstText(input, [
    "grid_owner_information_request_id",
    "gridOwnerInformationRequestId",
    "metadata.grid_owner_information_request_id",
  ]);
  const informationRequestStatus = firstText(input, [
    "grid_owner_information_request_status",
    "gridOwnerInformationRequestStatus",
    "metadata.grid_owner_information_request_status",
  ]);
  const hasVerifiedGridOwner =
    gridOwnerVerificationStatus === "verified" &&
    (isUuid(gridOwnerId) ||
      isEdielId(gridOwnerEdielId) ||
      Boolean(
        gridAreaCode &&
        priceArea &&
        [
  ...
  const facilityHasControlledError =
    isControlledFacilityStatus(facilityDataStatus);
  const facilityVerified =
    !facilityHasControlledError &&
    (firstBoolean(input, [
  ...
  const confirmedStartDate = firstText(input, [
    "confirmed_start_date",
    "confirmedStartDate",
    "contract.confirmed_start_date",
    "contract.confirmedStartDate",
  ]);
  const actualStartDate = firstText(input, [
    "actual_start_date",
    "actualStartDate",
  ...
  if (!customerEmail) {
    ...
    addIssue(blockingReasons, {
      ...
      label: "E-post saknas",
      message:
        "Kunden behöver en giltig e-postadress innan kundmail och portalflöde kan hanteras.",
  ...
  if (!customerIdentity) {
    ...
    addIssue(blockingReasons, {
      ...
      message:
        "Identitet saknas och bör kontrolleras innan switch eller aktiv kundstatus.",
  ...
  if (siteAddress && (!postalCode || !city)) {
    ...
    addIssue(blockingReasons, {
      field: "site.address",
      label: "Adress ej komplett",
      message: "Full adress krävs för säker nätområdesmatchning.",
      action: "Komplettera gata, postnummer och ort.",
    });
  }
  if (!gridAreaCode) {
    ...
    addIssue(blockingReasons, {
      ...
      label: "Nätområdeskod saknas",
      message:
        "Systemet behöver hitta eller bekräfta nätområde innan nästa steg.",
      action:
        "Låt systemet matcha adressen eller skicka till manuell granskning.",
    });
  }
  if (!priceArea) {
    ...
    addIssue(blockingReasons, {
      field: "price_area",
      label: "Elområde saknas",
      message: "Prissättning och fakturering behöver verifierat SE-område.",
      action:
        "Låt systemet kontrollera elområde eller komplettera anläggningsuppgiften.",
    });
  }
  if (!meteringPointId) {
    ...
    addIssue(blockingReasons, {
      ...
      message: "Leverantörsbyte får inte startas utan mätpunkt/anläggnings-ID.",
      action: "Begär anläggningsuppgifter från nätägare.",
    });
  }
  if (!hasVerifiedGridOwner) {
    missingFields.push("grid_owner");
    const verificationDetail =
      gridOwnerVerificationStatus && gridOwnerVerificationStatus !== "verified"
        ? ` Status: ${gridOwnerVerificationStatus}.`
        : "";
    addIssue(blockingReasons, {
      field: "grid_owner",
      label: "Nätägare ej verifierad",
      message: `Nätägare måste verifieras innan leverantörsbyte kan skickas.`,
      action:
        "Låt systemet matcha nätägare automatiskt eller skicka ärendet till granskning.",
    });
  }
  if (!facilityVerified) {
    missingFields.push("facility_verified");
    addIssue(blockingReasons, {
      field: "facility_verified",
      label: "Anläggningsuppgifter ej verifierade",
      message:
        "Geodata räcker för att begära uppgifter, men switch kräver verifierad anläggning/mätpunkt.",
      action:
        "Begär uppgifter från nätägare eller markera mottagna uppgifter efter kontroll.",
    });
  }
  if (!hasValidPricePlan) {
    ...
    addIssue(blockingReasons, {
      ...
      message: "Avtal finns inte komplett för nästa steg.",
      action: "Koppla prisplan/prisversion eller kontrollera avtalet.",
    });
  }
  if (!powerOfAttorneyAccepted) {
    ...
    addIssue(blockingReasons, {
      ...
      message: "Leverantörsbyte kan inte begäras utan giltig fullmakt.",
      action: "Kontrollera fullmakten eller skapa fullmaktsdokument.",
    });
  }
  if (!termsAccepted) {
    ...
    addIssue(blockingReasons, {
      ...
      message:
        "Kundens juridiska godkännanden behöver kontrolleras innan processen fortsätter.",
      action: "Kontrollera juridiska godkännanden.",
    });
  }
  if (requestedStartMode === "specific_date" && !requestedStartDate) {
    ...
    addIssue(blockingReasons, {
      ...
      label: "Startdatum saknas",
      message: "Kunden har valt specifikt startdatum men datum saknas.",
      action: "Ange önskat startdatum eller ändra till snarast möjligt.",
    });
  }
  if (facilityHasControlledError && facilityDataStatus) {
    ...
    if (CUSTOMER_CORRECTION_STATUSES.has(facilityDataStatus))
      warnings.push("customer_correction_required");
    if (GRID_OWNER_FOLLOWUP_STATUSES.has(facilityDataStatus))
      warnings.push("grid_owner_followup_required");
  }
  if (
    requestedStartMode === "earliest_possible" &&
    !calculatedEarliestStartDate
  )
    warnings.push("calculated_earliest_start_date_pending");
  ...
  if (resolutionStatus === "needs_review")
    warnings.push("energy_resolution_needs_review");
  if (gridOwnerVerificationStatus && gridOwnerVerificationStatus !== "verified")
    warnings.push(`grid_owner_${gridOwnerVerificationStatus}`);
  if (informationRequestId || informationRequestStatus)
    warnings.push("grid_owner_information_request_open");
  if (gridOwnerId && !isUuid(gridOwnerId))
    warnings.push("grid_owner_id_not_verified_uuid");
  if (pricePlanId && !isUuid(pricePlanId) && !pricePlanDefinition)
    warnings.push("price_plan_id_not_verified_uuid");

  const blocking = blockingReasons.filter(
  ...
  const canRequestGridOwnerInformation = Boolean(
    gridAreaCode && priceArea && hasVerifiedGridOwner && !facilityVerified,
  );
  const hasValidStartMode =
    requestedStartMode === "earliest_possible" || Boolean(requestedStartDate);
  ...
  if (facilityHasControlledError && facilityDataStatus)
  ...
  else if (canRequestGridOwnerInformation && informationRequestId)
    ...
    status = blocking.length > 0 ? "needs_information" : "pending_validation";

  const nextStep =
    facilityHasControlledError && facilityDataStatus
  ...
  const qualityScore = Math.max(
    ...
    Math.min(
      ...
      100 -
        blocking.length * 12 -
        (blockingReasons.length - blocking.length) * 5 -
        warnings.length * 3,
    ),
  );
