...
## BUG-001 — Customer portal sync converts controlled client errors to HTTP 500
...
- Symbol: `POST`
- Lines: 365–375 at commit `3eb8445cb840d38af6068d49266ce0881a8e0157`
- Evidence: `readJsonObject` throws exported `ApiInputError` values with 400/413 and stable codes for missing, invalid, non-object or oversized JSON. The route imports only `readJsonObject`; its final catch logs and returns status 500 for every exception.
- Actual function: malformed or oversized client input is reported as an internal server failure.
...
- Reproduction path:
  ...
  3. Observe HTTP 500 instead of 400/413.
...
- Security impact: low; malformed-input traffic is misclassified.
...
- Safest solution: import `ApiInputError`; mirror the canonical controlled-error pattern already used by `app/api/v1/events/route.ts`.
- Test to add: static regression plus runtime tests for invalid JSON, missing object, array and oversized body.
...
- Lines: multiple.
- Evidence: memory documents refer to `apps/ops`, while the current application is rooted in `app/`, `components/` and `lib/`. The root README is a narrow historical hotfix document rather than the current platform overview.
...
- Likely production impact: wasted time or edits in wrong locations.
- Safest solution: update architecture memory and README without deleting historical decisions.
- Test to add: documentation path/link check where practical.
- Changed files: `quality/CODEBASE.md`, `quality/ARCHITECTURE.md`; canonical memory update pending.
- Verification command: manual path review.
- Verification result: documented, not fixed.
- Fix commit: pending.
