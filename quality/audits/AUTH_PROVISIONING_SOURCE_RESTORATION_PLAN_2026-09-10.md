...
### Task 22: Whole customer-alignment characterization and private boundary

# Task22 — implement whole customer-alignment characterization and private boundary

## Prerequisites and binding authorities
Read this first as the exact task requirements. Read approved quality/audits/USER_RBAC_CUSTOMER_ALIGNMENT_SOURCE_EFFECTS_2026-09-11.md and USER_RBAC_ALIGNMENT_RUNTIME_OWNERSHIP_2026-09-11.md, with task-19-review.md and task-21-review.md in this SDD directory. Do not restart their741line effects inventory. Task21 design5bdee189 is independently spec/quality APPROVED with no findings.

Task18 currentpublished6c9e05d2dd5b93f31e031b513c5ff9862761d5c2/tree4957ebcd63dce52a48f9dddcf56daeef9cb094b0 full native continuation job103253167529 in OPS34596437761 PASSED: standalone, actual63, fresh repeats, all22 rejection/fault modes, child exit, controller SIGKILL after D2/F2/X, final privacy and exact cleanup; final12:10:31UTC. Full10218/25/28/31+finalprivacy and original16/actual52/actual56/quality/Ediel passed same head. Historical dedupe19 reduced controller cleanup failed despite exact unchanged code/job from passing54ae6759; a single targeted retry is now executing. INITIAL PHASE READ-ONLY: inspect the exact reusable interfaces and prepare ignored task-22-report.md only until root confirms this predecessor gate. No tracked edits/index/commit until root's message. This does not require user permission.

## Actual identities and scope
Task20 actualCLI2.101.0 artifact10261507270 at54ae6759/job103250143578 verified ZIP484 SHA2567d1841830464e313509f55021ec23d91151b7e5bae02a94e67d98ce4db7a89f4. Reuse retained original empty regular files here, copying each to supabase/migrations before authoring:
P20260911114442_canonical_user_rbac_customer_alignment_prerequisites.sql
W20260911114443_canonical_user_rbac_customer_alignment_boundary.sql
Both originally0bytes/e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855. Their actual filenames start with2026, not literal P/W. No regeneration/renaming/invented timestamps.

Implement complete standalone actual63-derived characterization and immediate private P/A/B/C/W transaction only; historical A/B/C source selection and actual staged-loop insertion are a subsequent task after hosted acceptance. Early owner-private W is not final runtime readiness. Future application TGT/role/count/Ediel convergence remains separate with exact later canonical authority prerequisites; do not introduce those broad app changes here.

## Owned files after root gate
- Exact two actual P/W migrations above.
- New scripts/canonical-user-rbac-customer-alignment-batch.py, scripts/canonical-user-rbac-customer-alignment-selftest.py and safely separated focused support/oracle modules when required to stay below2000lines; name them in report before creating. Reuse approved existing private catalog/reader/lifecycle helpers rather than clone full older suites.
- New scripts/sql/canonical-user-rbac-customer-alignment-{admission,assertions,catalog}.sql only if this is the narrowest compatible format.
- .github/workflows/ops-hardening.yml: one dedicated bounded private ownedPG17 characterization job with exact cleanup; preserve all10existing jobs and original19/102/continuation commands. No temporary skeleton job needed; Task20 removed it already.
- scripts/migration-history-manifest.additions.json for new P/W pins; directly coupled accounting/focused-family/integrity regression files only for these new inputs. No historical A/B/C selection, foundation-order change or originalSQL/hash change. Existing600-input expectation is only a prediction: inspect actual accounting and notify root before equality-dependent tests. Prior598=544/23/27/4, focused344=301/20/19/4, foundation104 endpoint63 and50total/39focused unresolved. P/W registration alone should not reduce the50unresolved historic inputs.
- Reuse existing ownedPostgres/replay/fixed runtime unchanged where possible. If exact current APIs cannot safely construct an independent actual63-derived reference and private control target, report the concrete seam needed before changing another tracked file; no unowned/duck-typed handle, published-handle rearm or broad refactor.
- Ignored task-22-report.md. Root owns all plan/memory/PR/publication/production.

## Complete executable contract
Use actual accepted compatible bootstrap+first63 including complete once-only fixed continuation, not reduced schema substituted as actual prefix. Bind independent reference and current actual catalog before mutation. Keep source inputs whole and exact separately. P adds only evidenced billing_export_run_items.contract_id uuid after exact prefix admission establishes need, preserving existing data/default/nullability/FKs; no broad1Bsource admission. A/B/C immutable pins/lengths/order are Task19 authority.

Execute admission, P, A, B, C, W and independent assertions in one outer transaction, with fresh READ COMMITTED admission, existing mutex before catalog reads, deterministic target/parent/dependency/trigger-write locks. W requires private exact context/transaction/stage/source hashes and rejects standalone/mismatched/repeated misuse. A/B/C contain no COMMIT. Transactional row/catalog effects roll back together; sequence/nontransactional change makes target terminal and requires fresh-prefix reconstruction, not sequence reset.

W restores selected15-name diagnostic view exact body/owner/ACL/options. It preserves C role TABLE shape but keeps it owner-private at this early prefix; diagnostic view is private/security_invoker, column helper/permission overrides/counts/Ediel RPCs owner-private with effective inherited/direct/column paths denied. Do not grant based on a later canonical helper not present here or assume runtime row governance from company tags. Retain exact historical source column/default/index effects including guard absence/alternatives and preserved existing definitions, unless a concrete independently reviewed forward correction owns a conflict. A6 loses rows globally: unsafe populated cases are rollback-only characterization or rejected admission, never accepted cleanup or inferred ownership. No historical PK replacement/data deletion/relabel or creator-membership guess.

Independent full-row/PK/catalog/index/function/view/ACL/sequence expected state must come from source-backed oracle, not actual output copied as expectation or Wusedasoracle. Prove all19DMLtargets/parents/trigger-written/outgoing and incomingFK effects in Task19 map;57declaredindex identities include guard skips/alternatives, not a required57objects. Compare exact rows bidirectionally, attributes/constraints/index shapes, function/view bodies and dependencies/options/owners/effective ACL, RLS/policies and sequence last_value/is_called/log_cnt as applicable. Unknown native catalog shapes/dependents/guard omissions fail closed with exact source evidence; do not expand admission speculatively.

Meaningful whole-source native lanes from Task19 and Task21: populated A payload/NULLroles/timestampUUIDties/activity/company collisions and baseunique/FK/trigger effects; eachB/C branch dirty and already-populated outcomes including conflicting parents and Cmeteringoverwrite; absent/missingguarded and unguardedcolumns, wrongtypes/indexshapes/read_at/created_atalternatives; exact role/override actor/status semantics in private characterization, counts total/duplicate-none/unknown/reservedall/tie/filter cases; Ediel window/direction/case/code/defaultNULL/order/ties and canonicalbypass preservation; first/exactrepeat, failureaftereachinput/W/context/hash/owner mismatch, nativebackenddeath, realwriter/index/dependency55P03, rollback/terminalfreshretry, unchanged canary and complete artifact privacy/cleanup. Do not duplicate original19/102 matrices. Application design cases remain later explicit gates; source characterization cannot claim those implementations.

## Verification and delivery
No localPG/CLI/container/socket install/probes: unavailable already established. Targeted meaningful RED/GREEN constructors for actual source/ownership/context/oracle/rejection seams; existing relevant preservation/accounting checks and Python/SQLstatic where reliable, whitespace. Do not create doc/YAML-mirroring tests. NativePG17 executableproof is mandatory hosted after independent review/publication; do not claim SQLsuccess locally. Keep diagnostics closed and no raw SQL/exception/private path/identities; use existing private writer authorization and scanner, no extra whole-output exemptions.

Report full exact files/BASE/HEAD, source/PW pins, actual counts/deltas, tested commands/output and limitations, independent oracle coverage, selfreview and concrete interface blockers. Commit only coherent ownedfiles once rootgateopen. No push, production/provider/customer/credential actions, runtime deployment or artifact/type regen. User explicitly authorizes continuous efficient work; no unnecessary review loops, repeats or confirmation pauses.
...
## Root implementation gate update — 2026-09-11 12:19UTC

Root authorizes tracked Task22 implementation now. Complete new actual63/lossless/fault/death/privacy native predecessor passed; historical19's separate retry runs unchanged code/job previously passed at54ae6759. That retry remains a required aggregate acceptance/publication gate, not a reason to idle independent code authoring. No Task22 native acceptance/publication until historical19 is resolved. This supersedes the initial read-only phase only; all owned-file/security/immutability/no-localSQL/no-shared-runtime-edit boundaries remain.

Characterization-only subclass and existing selftest-reader reuse belong strictly in selftest/test-support; trusted batch must not import selftest as authority. Bind exact fixed._RELEASES[h] is fixed._RUNS[h] is bound Reservation, fixed._REFERENCES[h] is Reservation.reference, actual fixed scope/SUCCEEDED and source-backed s1 equality before separate clones. Never mutate/rearm original. Add a test-only separate-input memory reader if existing stdin-only method cannot execute whole separate -f sources atomically; unchanged OwnedPostgres.command and existing privacy rules remain authority. Capture BASE immediately before tracked work and report actual new registration counts before equality tests.
...
### Task 23: Five unresolved DB2 whole-source effects

# Task23 — complete effects of the five unresolved DB2 reconciliation sources

Initial phase read-only. This is independent next-source preparation while Task22 owns alignment implementation. Write only ignored task-23-draft.md and task-23-report.md in this SDD directory. No tracked edits/index/commit/SQL/tests/native or CLI probes/network/live/provider/customer/credential action. Do not inspect or modify current Task22 work, rerun original19/102/actual63, or broaden to all remaining sources. No delegated agents. Later root may authorize one tracked design document quality/audits/DB2_CONTROLLED_RECONCILIATION_SOURCE_EFFECTS_2026-09-11.md and independent review; no source selection from this map.

## Exact whole-source scope
All five paths under supabase/migrations, total1506lines:
- 01_db2_full_view_preflight_schema_and_functions.sql:1079lines SHA2564de50050384d6892612c16484de8b198785c59cbb5d2ff03e7cea7e600d36cc9
- 01_db2b_preflight_views.sql:106lines SHA256884cc115dee1b169eae11ef37a2b319d894a85cff5971ebf050ad2be026e39d7
- 02_db2_execute_controlled_reconciliation.sql:31lines SHA256fcdc75e660f157a58e742f64b3e8f7a1c6801565ef16023bd0c9a317982744c9
- 03_db2_validation_and_finish.sql:127lines SHA25674579e9fb883c1aba60933b1c82460164534d6bf171a7fb094b528f5bbb7224e
- 03_db2b_validation_views.sql:163lines SHA256f84d355c54d20bd0fcd3f86426bcd0e56435a1e8713fad3ec7a239f480a2b5e5

Read all five complete sources in bounded nontruncated chunks. Verify pins and manifest/classification metadata. Do not execute SQL. The neighboring02_db2b_apply_superadmin_and_membership.sql is independently reviewed EXPLICITLY_EXCLUDED operational-only repair, SHA64671e13a4390e0d464a24198cd6ad27a38908c9816e3c597dc5119afc95dbc4, authority quality/audits/OPERATIONAL_REPAIR_CLASSIFICATION_2026-09-06.md and scripts/gridex-aud-003-noncanonical-artifacts.json. Do not reopen/reclassify or run that excluded source; inspect its reviewed dependency summary only if a concrete reference in these five requires it. No blanket extension of its exclusion to schema-bearing preflight/validation/function sources.

## Required evidence and deliverable
Map every complete-source top-level statement/function body/dynamic SQL branch to prerequisite, selected-prefix availability, row/catalog/index/helper/view/security/default/constraint/trigger/sequence/provider/session/durable-side effects and later winner where concretely relevant. Exact statements that only emit diagnostics must be distinguished from assertions or mutation, especially procedures invoked by the31line execution source. Trace nested helper/call effects and complete transactional/internalCOMMIT behavior, lock/concurrency/race implications, ambiguity/idempotency, NULL/case/default tie semantics and fixed-target lifecycle. Read named prerequisite/caller/trigger/authority bodies only when needed for a concrete branch; don't recrawl accepted sources or broad inventory.

Selected actual63/foundation104 is now bounded native accepted at published6c9e05d2 with full19/full102/actual63 fault/death/repeat/privacy. Current accounting before Task22 new P/W is598=544full/23substituted/27unclassified/4excluded,50unresolved, focused344=301/20/19/4. These are context, not counts to update; Task22 may change new-input counts independently and none of its code/native outcomes are your authority. The entire later chain/nativegenesis/types/live parity remain OPEN. Identify concrete prerequisites from accepted foundation order or immutable source definitions, and mark unknown actual catalog/data/dependencies explicitly; never invent accepted shapes.

Preserve historical SQL/checksums, existing invitation/actor/tenant/creator/PK/session invariants and provider/credential separation. Historical unsafe behavior is characterization evidence, not a required production feature. Never print/copy source credential or personal-identity literals; use path/line/effect and synthetic symbolic slots. No password generation/AuthAPI/provider calls/realrows. Distinguish schema-bearing sources from operational data repair using whole effects. Every source needing restoration must have a coherent whole-file native characterization boundary, not excerpts or broadexclusions. ADR006 prohibits replay/massledger marking in production; proposed later forward convergence must preserve rows and PKs, with authoritative ownership evidence.

Deliver concise line-backed complete unit/effect/dependency/security map, confirmed source defects vs native/live unknowns, minimal justified source grouping/order/prerequisite/boundary and independent before/after oracle. Specify meaningful synthetic dirty/populated/fixed-target/role/tenant/trigger/FK/default/index/dependency/race/rollback/repeat/privacy/cleanup tests needed by actual semantics; don't invent a huge matrix by keywords. Identify exact app/lib consumers only for material function/view contracts by bounded symbol search, no broadappaudit. Give minimum safe runtime/private boundary, caller gaps and data/UX decisions if relevant; do not infer authority from labels or arbitrary fixed actors/companies. No invented migration timestamp/CLI skeleton/source count/selection or finalruntimeaccessgrant.

Static hash/line/reference/whitespace checks only; no doc-mirroring tests. Report all five whole files read, exact evidence limits and scoped checks/outputs. Record active skill routing relevant to source/security/PG/FP analysis without broad scanners or implementations; instructions here scope out broader loops. Return concise readiness/concerns and ignored draft/report paths, no tracked commit. Root/independent reviewer will decide actual next implementation after evidence.

Task22 working-tree registration IMPLEMENTED_NOT_VERIFIED: actual P20260911114442/W20260911114443,600=546full/23substituted/27unclassified/4excluded; focused346=303/20/19/4 after exact customer-alignment family RED/GREEN16tests. Foundation104/actual63 unchanged, timestamp510;50total/39focused historic unresolved unchanged. P/W alone registered, no A/B/C selection or native acceptance. Root exact active markers synchronized before equality-dependent tests. Published6c9e05d2 remains prior598/344 accepted Task18 baseline.

## Root bounded next-source catalog receipt addition

In the new native selftest only, before any alignment P/A/B/C/W mutation, query existing actual63 pg_class/to_regclass through the private memory reader for exactly public.customer_profiles, public.customer_delivery_points, public.contract_agreements and public.document_ai_extractions. Validate exact fixed labels/result shape and emit only actual relkind or missing in one closed safe receipt. No customer rows/raw SQL/result dump, no added suite/job/source execution, no presence assertion or readiness claim. Constructor-only runs must not emit a native receipt. This resolves a concrete next-source prerequisite uncertainty while reusing the already-planned owned actual63 session. No shared-runtime scope expansion.
...
### Task24 preparation — finite DB2 operational execution disposition

Initial phase read-only and independent of Task22 nativefailure diagnosis. Accepted Task23 source-effects mapcfbc9f46 now closes its static review. Determine whether exact31line02_db2_execute_controlled_reconciliation.sql (SHAfcdc75e660f157a58e742f64b3e8f7a1c6801565ef16023bd0c9a317982744c9) qualifies for the existing finite historical_operational_data_repair disposition through complete named helper/trigger/allocation/event/queue effect closure and immutable pins. No predetermined exclusion, broadnamepattern or schema/reference effect omission. The four schema-bearing DB2 companion inputs and independently excludedDB2B repair retain separate classifications. No SQL/provider/customer/native actions or appgrant. Only ignored assessment/report authorized during Task22trackedfix; later exact implementation/independentreview remains separate. Current600/346 and50historical unresolved unchanged.
