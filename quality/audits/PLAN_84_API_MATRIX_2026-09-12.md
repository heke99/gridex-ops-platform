...
## Follow-up: remaining default/explicit-company variants and GET write inventory

2026-09-12, base4d4fe075. Root accepted independently probed application findings in API_DEFAULT_COMPANY_AUTHORITY_FINDING_2026-09-12.md (64source cases,37source hashes verified) and API_INVOICE_GET_COMPANY_AUTHORITY_FINDING_2026-09-12.md (31source cases,21source hashes verified). Seven remaining route files/eight HTTP handlers can use company A permissions with independently chosen B membership. Five selected-company billing/pricing routes use the no-cookie fallback; invoice-export GET and invoice provider-status GET also accept explicit B aliases under cookie A. Same/reverse/cookie/permission/membership/lifecycle/data guards distinguish the missing company authority from arbitrary nonmember or unauthenticated access. Native database commits and production reachability remain unverified. Task10b2auth is queued for the canonical binding fix; no source change yet.

Inventory correction: invoices/[id]/provider-status GET is an additional mutating GET outside the21worker route/20GET-alias inventory. It reads provider configuration, invokes four provider invoice GET boundaries (OAuth may obtain tokens), updates local invoice_export_items provider_status/purchase_status/status_payload/updated_at, and returns provider data. Therefore it must be included in point84 effect-based write/authorization review. The original57route/58declaredPOST-or-DELETE count remains the correct lexical count, but it did not establish a complete inventory of mutating GET behavior. No provider invoice creation/purchase/dispute command is called by this route. Invoice-export GET is a pure service read. Spot import/lock shared-market routes are refuted for this particular company-target variant.
