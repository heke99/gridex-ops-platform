...
## Fix round 5 — isolate the legacy-global permission fixture

At base `73970bdcccb275f5b9ab8e95ea690f6101e50700`, genuine OPS35941188514/native107449194254 passes native **224/4**, case1/browser2/post-browser1 and reconciled types, then fails the unchanged tenant invariant: **F-3 user_permissions holds one row with no company_id**. The downloaded replay log confirms the legacy-global positive at18137, native224 at18153 and F-3 at18506-18507. Schema stage remains unreached; successful earlier stages do not constitute a green pipeline. Parent artifact10784818129 SHA-256 `eedd9adca08a0d496a08c412d01c23323209c61dddf8eaae88fe27bc5c5571ac` identifies this run.

Diagnosis traced the one row to the compatibility fixture's committed null-company direct grant. Real HTTP RPCs need its committed visibility, but the test previously performed no cleanup. The fixture now supplies a fresh owned grant UUID and wraps the original selected-company/global/successful capture assertions in try/finally. Cleanup targets only that UUID plus actor/null-company/canonical key. It asserts the UUID is absent and restores the entire ordered pre-test null-grant snapshot. Original assertion errors propagate; a simultaneous cleanup failure retains both errors and the original cause. No unrelated row, source, concern or witness is deleted.

Named residual-state inspection found the registry/ACL/global-platform-role probes already roll back; all other new direct grants and company role assignments are tenant-bound. Explicit platform admin/audit rows have no company column and use existing platform-shared classification. No other new persistent null-company tenant fixture was found. Production resolver semantics, published SQL/checksums, table classification and tenant invariant remain unchanged.

Local final checks under pinned Node22.23.2: covering **71/4 PASS**, scripts typecheck **PASS**, native-file ESLint **PASS**, `git diff --check` **PASS**, production SQL/invariant/generated-file diff **empty**. Native224 count is unchanged; new cleanup assertions and complete native/tenant/parity/schema replay remain **pending parent CI**. No broad tests, hosted operations or parent-file edits occurred. This is round5/5, with no automatic sixth. Task2 remains source-only DONE_WITH_CONCERNS; Task2b/3/4 are not implemented.
