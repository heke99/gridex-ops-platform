...
### Five changed-view runtime witness implementation (2026-09-15)

Added `canonical_changed_view_witness.py` and dedicated controls. The retained source register is `PR310_CHANGED_VIEW_DISPOSITIONS_2026-09-15.json`, exact SHA256 `adaee88b661a56b89592ecd706f9305a2ed06cefccade188c2ea375981138bb1`. Its five original reference hashes and five observed replay hashes remain unchanged. Every selected migration query is extracted from pinned source bytes; the actor view uses the existing pinned historical outer-projection transition, and the unresolved-message view requires both exact guarded source declarations. The actual comparator source and invoker-hardening source are pinned separately.

The executable PostgreSQL witness creates five fresh temporary invoker views inside a rollback transaction on the admitted full owned replay. It captures the actual eight-field comparator rows for public and temporary views, normalizes only temporary namespace/name, and requires both rows to match the independently recorded observed replay hash. Both portable and native paths retain sources before staging, execute after the added-view witness, and require the exact changed-view receipt before schema comparison. Native execution retains the existing real Runner and ledger admission before and after SQL. Catalog, business rows, source bytes and ledger must remain unchanged. Derived SQL stays in memory on the portable transport. Diagnostic output is restricted to fixed ordinals and SHA256 values.

This implements executable source-definition witnesses, not actor-access or business-fixture acceptance. Historical inner `SELECT *` projections are retained exactly from source. If final-schema expansion differs from creation-time expansion, the fixed observed hash must fail and the actual run must guide a separately source-backed reconstruction; no new expected hash is permitted merely to pass the witness. Actual PostgreSQL execution of this new witness is still outstanding. Schema acceptance and generated-type flags remain false.

Offline controls pass: changed-view witness 8, portable full-schema observer 26, owned timestamp 19, native schema comparison 4, native lifecycle integration 12, and both existing/changed native Runner witness wrappers. The full timestamp-runtime suite passed 21 tests before the new wrapper test name was made distinct; the two wrapper tests were then rerun explicitly. These controls verify implementation admission/preservation and do not claim a PostgreSQL pass.

### Exact ninth-forward binding

After the separately qualified auth-action and access-capability sources, removed-policy qualification now requires the exact original first seven receipts followed by `20260915174610_expand_auth_email_event_action_domain.sql` (SHA256 `69b8d5693f586209f37950be866e3f3dd8e5d910408c34ed9cb075daab85407c`) and `20260915174614_restrict_access_table_capabilities.sql` (SHA256 `ddee41e3266948eef7f9082b331f873823602266448efe7cabcb03fdb3566683`). The receipt is `fullNineForwardReceiptsVerified`; all 267 policy identities/hashes and formula obligations remain unchanged. Ten qualification controls pass, including missing, reordered, wrongly hashed and non-repeated suffix receipts in both runtime modes.

The separate actual F14 diagnosis identified 24 inert policies on inbound_ediel_match_attempts, inbound_ediel_parse_results and inbound_email_attachments. None belongs to this module's 267-policy source selection. Their independently qualified removal therefore needs no exception or replacement hash in the 267-policy set; a later promoted forward will still need exact receipt admission.
