...
| O-005 | CODE_REMEDIATED | BL-006 migration isolates `platform_actor_contacts` SELECT to platform admin + service role (`20260807154500`) |
| O-006 | CODE_REMEDIATED | BL-006 migration isolates address/energy lookup-cache SELECT to platform admin + service role |
| O-007 | CODE_REMEDIATED | `/admin/network-owners` import history now reads via `supabaseService` after `requirePlatformAdminAccess()` |
