...
def selection():
    ...
    assert len(order) == 118 and order.count(SOURCE) == additions['foundation'].count(SOURCE) == 1
    ...
    assert counts == {'FULL_FILE_SELECTED':311,'SUBSTITUTED':20,'UNCLASSIFIED':10,'EXPLICITLY_EXCLUDED':5}, counts
    return order[:30]
