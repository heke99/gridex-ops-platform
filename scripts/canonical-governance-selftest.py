...
def selection():
    ...
    assert len(order) == 144 and order.count(SOURCE) == additions['foundation'].count(SOURCE) == 1
    ...
    assert data['totalMigrations'] == 600 and data['counts'] == {'FULL_FILE_SELECTED':588,'SUBSTITUTED':2,'UNCLASSIFIED':5,'EXPLICITLY_EXCLUDED':5}, data['counts']
    ...
    assert counts == {'FULL_FILE_SELECTED':334,'SUBSTITUTED':2,'UNCLASSIFIED':5,'EXPLICITLY_EXCLUDED':5}, counts
    return order[:30]
