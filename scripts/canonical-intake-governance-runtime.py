"""Owned immutable R/D/I continuation from the linked actual74 commit."""
...
spec=importlib.util.spec_from_file_location('intake_runtime_loader',ROOT/'scripts/canonical-auth-provisioning-replay.py')
loader=importlib.util.module_from_spec(spec);spec.loader.exec_module(loader)
replay=loader.controller();batch=replay.load_intake();prior=replay.load_readiness_runtime()
...
def base_owned(target):
    ...
    check(ref.prior.alignment.fixed.dedupe.scope in ('intake77','full'),'INTAKE_SCOPE_REQUIRED')
    return ref
...
def owned(target,state=None):
    ...
    check(type(ref) is Reference and ref.prior is predecessor and ref.inputs is predecessor.inputs
          and ref.name==target.name and ref.directory==target.directory.name,'INTAKE_REFERENCE_REQUIRED')
    if state is not None:check(dedupe._STATES.get(target)==state,'INTAKE_STATE_REQUIRED')
    return ref
...
def prepare_reference(target):
    ...
    check(frame.f_code is dedupe.prepare_reference.__code__ and frame.f_locals.get('target') is target
          and target not in dedupe._STATES and target not in _REFERENCES,'INTAKE_PREPARATION_REQUIRED')
...
def check_program(target,sql):
    ref=owned(target,'INTAKE_NATIVE');run=_RUNS.get(target)
    check(type(run) is Release and target not in _RELEASES and run.reference is ref
          ...
          and run.prior_completion is prior.completion(target),'INTAKE_PROGRAM_BINDING_REQUIRED')
    check(batch.validate_sources(reviewed_paths(),run.staging)==ref.sources,'INTAKE_STAGED_SOURCE_CHANGED')
...
def check_receipt(output):
    markers=[line for line in output.splitlines() if line.startswith('INTAKE_')]
    check(markers==['INTAKE_SOURCE_R','INTAKE_SOURCE_D','INTAKE_SOURCE_I','INTAKE_COMPLETE'],
          'INTAKE_COMPLETION_RECEIPT_REQUIRED')


def execute(target,database,paths,staging):
    ref=owned(target,'INTAKE_NATIVE');frame=sys._getframe(1)
    check(frame.f_code is dedupe.continue_intake.__code__ and frame.f_locals.get('target') is target,'INTAKE_CONTROLLER_CALL_REQUIRED')
    check(database==DATABASE and type(staging) is legacy.StagedSources and ref.inputs.staging is staging
          and target not in _RUNS,'INTAKE_ONCE_STAGED_REQUIRED')
    sources=batch.validate_sources(paths,staging)
    check(sources==ref.sources,'INTAKE_FROZEN_SOURCES_REQUIRED')
    predecessor=prior.assert_final(target);before=snapshot(target)
    check(encoded(before)==predecessor.after,'INTAKE_ACCEPTED_BASELINE_REQUIRED')
    rows=batch.expected_rows(before[1],before[0])
    run=Release(ref,predecessor,staging,encoded(before),b'',uuid.uuid4().hex);_RUNS[target]=run
    def expected_catalog():
        check(encoded(snapshot(target,ORACLE))==run.before,'INTAKE_ORACLE_BASELINE_REQUIRED')
    ...
    expected=prior.prior.clone(target,DATABASE,origin,expected_catalog)
    check(encoded(snapshot(target))==run.before,'INTAKE_PREIMAGE_CHANGED')
    ...
    check(batch.alignment.catalog.final_equal(before[0],after[0],expected,batch.new_index_keys(sources,before[0],before[1]))
          and alignment.rows_equal(after[1],rows),'INTAKE_POST_COMMIT_MISMATCH')
    check(batch.validate_sources(paths,staging)==sources,'INTAKE_STAGED_SOURCE_CHANGED')
...
def assert_final(target):
    ...
    check(encoded(snapshot(target))==release.after,'INTAKE_FINAL_STATE_CHANGED')
    return release


def release_checks(target,full=False):
    ref=owned(target,'INTAKE_COMPLETE');frame=sys._getframe(2)
    ...
    check(type(full) is bool and full==(ref.prior.prior.alignment.fixed.dedupe.scope=='full'),'INTAKE_RELEASE_SCOPE_REQUIRED')
    release=completion(target) if full else assert_final(target)
    check(replay.originals_snapshot()==ref.prior.prior.alignment.fixed.originals,'INTAKE_ORIGINALS_RESTORATION_REQUIRED')
