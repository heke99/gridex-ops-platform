...
ERRORS = frozenset(('FIXED_SOURCE_REQUIRED', 'EXACT_LOCK_REQUIRED', 'CLI_REQUIRED',
    ...
    'EXACT_FAILURE_AND_ROLLBACK_REQUIRED', 'EXACT_SUCCESS_REPEAT_REQUIRED', 'INTERRUPTED'))
...
def run():
    ...
    report = dict(scope='SYNTHETIC_FORWARD10_LOCK_CONTEXT_NOT_RELEASE_ACCEPTANCE', sourceSha256=DIGEST,
                  productionModified=False, schemaAccepted=False, generatedTypesVerified=False, cases=[])
    ...
    with tempfile.TemporaryDirectory(prefix=project + '-') as directory:
        ...
        try:
            ...
            adapted = plain.replace(LOCK.encode(), ('DO $forward_lock$ BEGIN\n' + LOCK + '\nEND $forward_lock$;').encode())
