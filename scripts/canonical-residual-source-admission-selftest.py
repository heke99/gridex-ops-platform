...
class AdmissionTests(unittest.TestCase):
    ...
    def test_complete_plan_accounts_610_and_retains_601_without_claiming_sql_or_types(self):
        report = self.complete.account(ROOT)
        self.assertEqual(report['totalMigrations'], 610)
        self.assertEqual(report['rawSelectedInputCounts'], {'foundation':144, 'timestamp':523})
        self.assertEqual(report['additionalForwardSourceCount'], 9)
        self.assertEqual(len(report['additionalForwardSources']), 9)
    ...
    def test_original_selector_still_reports_its_own_seven_unresolved_sources(self):
        self.assertEqual(self.selected['counts'], {'FULL_FILE_SELECTED':589 + 9,'SUBSTITUTED':2,
    ...
    def test_forward_partition_reports_exact_registered_sources_without_execution_claims(self):
        ...
        self.assertEqual(report['baseSelectorCounts']['FULL_FILE_SELECTED'], 598)
        self.assertEqual(report['canonicalCounts']['additionalForwardSources'], 9)
        for key in ('sqlExecutionVerified','ledgerProvenanceVerified','completeReplayVerified','generatedTypesVerified'):
            self.assertIs(report[key], False)
