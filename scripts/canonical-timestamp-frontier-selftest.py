...
class TimestampTests(unittest.TestCase):
    ...
    def test_twelve_registered_forwards_do_not_enter_the_historical_compiler_input(self):
        forward = load('canonical_forward_sources')
        selected, _ = tail.load_inputs(ROOT, self.report, self.foundation)
        self.assertEqual(self.report['totalMigrations'], 613)
        self.assertEqual(len(forward.FORWARD_SOURCES), 12)
        self.assertEqual(self.report['selectedInputCounts']['timestamp'], 526)
        self.assertEqual(len(selected), 514)
