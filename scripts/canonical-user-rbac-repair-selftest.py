...
def integration_constructors(b):
    ...
    assert data['counts']=={'FULL_FILE_SELECTED':588,'SUBSTITUTED':2,'UNCLASSIFIED':5,'EXPLICITLY_EXCLUDED':5}
    ...
    with tempfile.TemporaryDirectory(prefix='repair-staged-constructor-') as directory:
        ...
        try:
            ...
            # These recorder-only checks retain full144 dispatch construction.
            with contextlib.ExitStack() as constructor_patches:
                ...
                restored=replay.trusted_module('canonical_residual_source_restoration','canonical-residual-source-restoration.py')
                def record_residual(target,db,relative,ordinal,*,rulebook_bytes):
                    assert target is h and db==replay.DATABASE
                    assert relative in restored.FOUNDATION_SOURCES and restored.FOUNDATION_SOURCES[relative][1]==ordinal
                    assert observed[-1]==(db,'replay_foundation_'+str(ordinal),False)
                    restored.checks(relative,rulebook_bytes=rulebook_bytes)
                    native_callbacks.append((relative,ordinal))
                def record_conversion(target,db,relative,ordinal,*,rulebook_bytes):
                    assert target is h and db==replay.DATABASE and ordinal==88
                    assert relative=='migrations/20260529_batch_2_rulebook_hardening_sql_fix_v4.sql'
                    assert observed[-1]==(db,'replay_foundation_88',False)
                    restored.rulebook_seed_check('ediel_field_rules',True,source_bytes=rulebook_bytes)
                    native_callbacks.append((relative,ordinal))
                ...
                for scope in _record_scopes:
                    ...
                    if scope=='full':expected+=['replay_foundation_'+str(i) for i in range(78,145)]
                    assert [stage for _,stage,_ in observed]==expected
                    assert native_callbacks==([(restored.RULEBOOK_COMPLETION,87),
            ...
            for scope in ('legacy52','repair56'):
                ...
                try:
                    ...
                    else:raise AssertionError('whole144 validation omitted repair source in named scope')
    ...
    print('PASS repair integration constructors: exact144, shared trusted loader, retained dependencies, nine scopes and once-only same-target dispatch; NO SQL claim')
