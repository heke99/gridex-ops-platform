...
def integration_constructors(b):
    ...
    assert len(order)==112 and order[52:56]==['migrations/'+p.name for p in b.reviewed_paths()]
    ...
    assert data['counts']=={'FULL_FILE_SELECTED':552,'SUBSTITUTED':23,'UNCLASSIFIED':20,'EXPLICITLY_EXCLUDED':5}
    ...
    with tempfile.TemporaryDirectory(prefix='repair-staged-constructor-') as directory:
        ...
        try:
            ...
            # These recorder-only checks retain full112 dispatch construction.
            with contextlib.ExitStack() as constructor_patches:
                ...
                _record_scopes=('legacy52','repair56','dedupe57','fixed-target','alignment68','operations71','full')
                for scope in _record_scopes:
                    dedupe._REFERENCES[h]=dedupe._Reference(directory,h.name,h.reference,b.REFERENCES[h],{},{},[],scope in ('fixed-target','alignment68','operations71','full'),scope)
                    ...
                    if scope in ('dedupe57','fixed-target','alignment68','operations71','full'):expected+=['dedupe']
                    if scope in ('fixed-target','alignment68','operations71','full'):expected+=['fixed']
                    if scope in ('alignment68','operations71','full'):expected+=['alignment']
                    if scope in ('operations71','full'):expected+=['operations']
                    if scope=='full':expected+=['replay_foundation_'+str(i) for i in range(72,113)]
            ...
            for scope in ('legacy52','repair56'):
                ...
                try:
                    ...
                    else:raise AssertionError('whole112 validation omitted repair source in named scope')
    ...
    print('PASS repair integration constructors: exact112, shared trusted loader, retained dependencies, seven scopes and once-only same-target dispatch; NO SQL claim')
