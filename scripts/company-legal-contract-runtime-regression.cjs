...
assert(/function getNullableInt[\s\S]*\/\^-\?\\d\+\$\//.test(internalContractActions), 'Internal contract integer validation is not full-match strict')
assert(/function intValue[\s\S]*\/\^-\?\\d\+\$\//.test(publicContractActions), 'Public contract integer validation is not full-match strict')
