...
it('a historical raw Z08 with stale metadata stays a wildcard but a sealed unrelated point does not',async()=>{
 ...
 sql(`BEGIN; SET LOCAL session_replication_role=replica;
  UPDATE public.ediel_messages SET raw_payload=${literal(wire)},immutable_payload_hash=${literal(createHash('sha256').update(wire).digest('hex'))},
   immutable_rendered_at=clock_timestamp() WHERE id=${literal(f.messageId)};
  COMMIT;`)
 const open=()=>sql<{readsetText:string}>(`SET ROLE service_role; SELECT public.gridex_correction_combined_snapshot_v1(${literal(f.companyId)},'test',${literal(f.sourceMessageId)},clock_timestamp())`)
 expect((JSON.parse(open().readsetText) as {outbound:{originalCount:number}}).outbound.originalCount).toBe(0)
 sql(`BEGIN; SET LOCAL session_replication_role=replica; UPDATE public.ediel_messages SET message_code='Z05',rule_profile_key='PRODAT:Z05:C:26.A:r3',
  immutable_payload_hash=NULL,immutable_rendered_at=NULL WHERE id=${literal(f.messageId)}; COMMIT;`)
 expect(sql(`SELECT jsonb_build_object('code',message_code,'profile',rule_profile_key,'hash',immutable_payload_hash,'rendered',immutable_rendered_at)
  FROM public.ediel_messages WHERE id=${literal(f.messageId)}`)).toEqual({code:'Z05',profile:'PRODAT:Z05:C:26.A:r3',hash:null,rendered:null})
 const body=JSON.parse(open().readsetText) as {outbound:{originalCount:number;originals:{messageId:string;scope:unknown}[]}}
 expect(body.outbound.originalCount).toBe(1)
