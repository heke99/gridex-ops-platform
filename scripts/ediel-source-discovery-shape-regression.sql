...
BEGIN
 ...
 -- Repeated physical registers keep both ordinals in one exact tuple.
 altered:=jsonb_set(jsonb_set(inv,'{sources,0,occurrences,1,objectId}','"MP-A"'),'{sources,0,occurrences,1,identityAgency}','"9"');
 altered:=jsonb_set(altered,'{sources,0,objects}','[{"messageIndex":0,"objectId":"MP-A","identityAgency":"9","occurrenceOrdinals":[1,2]}]');
 PERFORM pg_temp.shape_case('valid/typed-register-repetitions',snap,altered,false);
 altered:=jsonb_set(altered,'{sources,0,occurrences,1,messageIndex}','1');
 altered:=jsonb_set(altered,'{sources,0,objects}','[{"messageIndex":0,"objectId":"MP-A","identityAgency":"9","occurrenceOrdinals":[1]},{"messageIndex":1,"objectId":"MP-A","identityAgency":"9","occurrenceOrdinals":[2]}]');
 PERFORM pg_temp.shape_case('valid/typed-distinct-message-tuples',snap,altered,false);
