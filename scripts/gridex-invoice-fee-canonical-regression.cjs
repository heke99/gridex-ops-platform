...
must(/Fakturaavgift, kr per faktura/.test(adminUi) && /Publicera canonical avtal på hemsidan/.test(tenantUi), 'pricing is edited only in canonical admin and tenant page is channel-only')
must(/Avgiften används alltid i offert, avtal och fakturering/.test(adminUi) && !/name="invoice_fee_sek"/.test(tenantUi), 'calculation is separated from visibility without duplicate tenant fields')
...
must(openapi.info.version === '2026-07-22.2', 'OpenAPI contract version is 2026-07-22.2')
must(openapi.paths['/api/v1/website/quote'].post.deprecated === true && openapi.paths['/api/v1/website/quote'].post.responses['410'], 'external quote endpoint is documented as removed')
