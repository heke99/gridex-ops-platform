...
function exists(relativePath) { if (!fs.existsSync(path.join(root, relativePath))) fail(`Missing required file: ${relativePath}`) }
...
function scanFiles(files, pattern, message, allow = () => false) {
  for (const file of files) {
    ...
    if (pattern.test(read(file))) fail(`${message}: ${file}`)
  }
}
...
if (fs.existsSync(path.join(root, 'lib/website/customerApplicationPersistence.ts'))) {
  ...
  if (!/from\(['"]external_contract_intakes['"]\)/.test(content)) fail('Website intake flow does not write external_contract_intakes')
}
...
for (const needle of [
  'alter table public.admin_users enable row level security', 'revoke all on table public.admin_users from anon',
  'gridex_audit_admin_users_change', 'security_invoker = true', 'platform_actor_contacts', 'gridex_route_readiness_v',
  'gridex_company_operations_statistics_v', 'gridex_launch_error_summary_v', 'external_contract_intakes',
  'integration_api_clients', 'domain_events', 'event_outbox', 'webhook_deliveries',
...
for (const needle of [
  'integration_api_rate_limit_events', 'platform_actor_contact_import_runs', 'gridex_launch_db_security_warnings_v',
  'gridex_company_launch_blocker_reasons_v', 'gridex_billing_launch_readiness_v', 'gridex_data_retention_policies',
  'gridex_run_launch_retention_cleanup', 'revoke all on function', 'revoke all on table',
...
if (fs.existsSync(path.join(root, 'lib/integrations/apiAuth.ts'))) {
  ...
  if (!apiAuth.includes("code === 'rate_limited'") || !apiAuth.includes('status: 429') && !apiAuth.includes("if (code === 'rate_limited') return 429"))
    fail('API auth does not normalize real quota exhaustion to HTTP 429')
  if (!apiAuth.includes('integration_api_rate_limit_events')) fail('API auth does not log rate limit events')
  if (!apiAuth.includes('The API client rate limit has been exceeded.')) fail('API auth lacks a precise public message for real quota exhaustion')
  if (!apiAuth.includes('api_rate_limiter_unavailable') || !apiAuth.includes('return 503')) fail('API auth still conflates limiter infrastructure failure with HTTP 429')
  if (!apiAuth.includes('retryAfterSeconds') || !apiAuth.includes('rateLimit: status === 429 ? rateLimit : undefined')) fail('API auth does not expose deterministic retry/rate-limit response context for 429')
}

const packageJson = JSON.parse(read('package.json'))
for (const script of [
  'gridex:launch-security-regression', 'gridex:db-ui-compatibility-regression', 'gridex:website-api-webhook-regression',
  'gridex:external-contract-intake-regression', 'gridex:company-statistics-regression', 'gridex:pricing-flow-regression',
  'gridex:billing-readiness-regression', 'gridex:route-readiness-regression', 'gridex:supplier-contact-regression',
  'gridex:customer-intake-regression', 'gridex:ui-db-mismatch-regression', 'gridex:rate-limit-regression',
  'gridex:system-health-regression', 'gridex:launch-smoke',
