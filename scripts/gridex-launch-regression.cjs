...
const appLibFiles = sourceFiles.filter(
...
scanFiles(
  appLibFiles,
  /\.from\(['"]api_keys['"]\)/,
  "Legacy api_keys table access remains",
);
scanFiles(
  appLibFiles,
  /\.from\(['"]api_audit_logs['"]\)/,
  "Legacy api_audit_logs table access remains",
);
scanFiles(
  appLibFiles,
  /\.from\(['"]customer_applications['"]\)/,
  "Legacy customer_applications table access remains",
);
scanFiles(
  appLibFiles,
  /\.from\(['"]webhook_events['"]\)/,
  "Legacy webhook_events table access remains",
);
scanFiles(
  appLibFiles,
  /\.from\(['"]webhook_endpoints['"]\)/,
  "Legacy webhook_endpoints table access remains",
);
scanFiles(
  appLibFiles,
  /\.from\(['"]contracts['"]\)/,
  "Legacy contracts table access remains",
);
scanFiles(
  appLibFiles,
  /customer_sites\.price_area(?!_code)/,
  "Invalid customer_sites.price_area reference remains",
);
scanFiles(
  appLibFiles,
  /platform_actor_roles[\s\S]{0,180}role_key/,
  "platform_actor_roles.role_key mismatch remains",
);
scanFiles(
  appLibFiles,
  /ediel_business_errors[\s\S]{0,180}severity/,
  "ediel_business_errors.severity assumption remains",
);

if (fs.existsSync(path.join(root, "app/admin/ediel/routes/page.tsx"))) {
  const routePage = read("app/admin/ediel/routes/page.tsx");
  if (!routePage.includes("from('platform_actor_routes')"))
    fail("/admin/ediel/routes is not backed by platform_actor_routes");
  if (!routePage.includes("from('platform_actor_roles')"))
    fail("/admin/ediel/routes is not backed by platform_actor_roles");
  if (routePage.includes("from('communication_routes')"))
    fail("/admin/ediel/routes still uses legacy communication_routes");
  if (!routePage.includes("actor_role"))
    fail("/admin/ediel/routes does not use actor_role");
}

if (
  fs.existsSync(path.join(root, "app/admin/ediel/route-readiness/page.tsx"))
) {
  const page = read("app/admin/ediel/route-readiness/page.tsx");
  if (!page.includes("from('gridex_route_readiness_v')"))
    fail("Route-readiness page is not backed by gridex_route_readiness_v");
  if (!page.includes("ready_verified_manual_send"))
    fail("Route-readiness page misses manual-send ready state");
  if (!page.includes("ready_auto_send_allowed"))
    fail("Route-readiness page misses auto-send ready state");
  if (!page.includes("bulkRouteReadinessByStatusAction"))
    fail("Route-readiness page misses bulk actions");
  if (!page.includes("importSupplierContactsCsvAction"))
    fail("Route-readiness page misses supplier contact CSV import");
  if (!page.includes("/api/admin/ediel/supplier-contacts/export"))
    fail("Route-readiness page misses supplier contact CSV export");
}
...
if (fs.existsSync(path.join(root, "lib/admin/websiteIntegrationOps.ts"))) {
  ...
  if (!/from\(['"]external_contract_intakes['"]\)/.test(content))
    fail(
      "Website application admin listing does not read external_contract_intakes",
    );
  if (!content.includes("listLegacyWebsiteApplications"))
...
if (fs.existsSync(path.join(root, "lib/website/customerApplicationPersistence.ts"))) {
  ...
  if (!/from\([\'\"]external_contract_intakes[\'\"]\)/.test(content))
    fail("Website intake flow does not write external_contract_intakes");
}
...
if (fs.existsSync(path.join(root, "lib/customer-intake/intakeEngine.ts"))) {
  ...
  if (!content.includes("missingSchema") || !content.includes("PGRST204"))
    fail("Customer intake does not classify schema-cache mismatches");
  if (!content.includes("minimalIntakeValues"))
    fail(
      "Customer intake lacks minimal fallback payloads for site/metering creation",
    );
  if (
    !content.includes("createOrUpdateSiteTenantSafe") ||
    !content.includes("createOrUpdateMeteringPointTenantSafe")
  )
    fail("Customer intake core create helpers are missing");
}
...
if (
  ...
  for (const needle of requiredSql) {
    if (!migration.includes(needle))
      fail(`Launch migration missing: ${needle}`);
  }
  if (/auto_send_allowed\s*=\s*true/i.test(migration))
    fail("Migration must not enable auto_send_allowed automatically");
}
...
if (
  ...
  for (const needle of requiredCompletionSql) {
    if (!completionMigration.includes(needle))
      fail(`Launch completion migration missing: ${needle}`);
  }
  if (/auto_send_allowed\s*=\s*true/i.test(completionMigration))
    fail(
      "Launch completion migration must not enable auto_send_allowed automatically",
    );
}

if (fs.existsSync(path.join(root, "lib/integrations/apiAuth.ts"))) {
  const apiAuth = read("lib/integrations/apiAuth.ts");
  if (!apiAuth.includes("rate_limited"))
