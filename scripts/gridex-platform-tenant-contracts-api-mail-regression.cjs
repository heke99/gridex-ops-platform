...
contains('app/admin/platform/api-clients/CreateApiClientForm.tsx', /INTEGRATION_API_PERMISSION_GROUPS\.map[\s\S]*group\.label[\s\S]*group\.description/, 'API UI visar grupper i vanliga ord')
...
contains('lib/website/customerApplicationOnboarding.ts', /public_contract_offer_id[\s\S]*spot_weight_percent[\s\S]*portfolio_weight_percent/, 'Kundteckning sparar juridiskt snapshot av avtal/mix')
contains('lib/website/customerApplicationProcess.ts', /resolvePublicContractOffer\(\{[\s\S]*offerReference:\s*selectedOfferReference/, 'Kundteckning kan lösa publicerat avtal via offer_reference')
