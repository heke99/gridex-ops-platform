...
assert(/sendInvoiceExportRun/.test(monthly), 'monthly automation sends through canonical invoice export service')
const blockerIndex = monthly.indexOf('if (underlayResult.needsReview > 0)')
const exportIndex = monthly.indexOf('const exportRun = await createInvoiceExportRun')
assert(blockerIndex >= 0 && exportIndex > blockerIndex, 'monthly automation evaluates measurement blockers before invoice export')
const blockerReturnIndex = monthly.indexOf('return {', blockerIndex)
assert(blockerReturnIndex > blockerIndex && blockerReturnIndex < exportIndex, 'monthly automation stops before invoice generation when measurements are blocked')
