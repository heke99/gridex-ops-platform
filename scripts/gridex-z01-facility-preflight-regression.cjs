...
assert(renderIndex !== -1, 'prodat flow renders via the sanctioned customer-masterdata RenderGateway')
assert(preflightIndex < preparedIndex, 'preflight runs before outbound is marked prepared')
assert(preparedIndex < renderIndex, 'EDIFACT render still happens after prepared decision in the valid path')
const gateway = read('lib/ediel/intent/renderGateway.ts')
assert(gateway.includes('buildCustomerMasterdataZ01Draft'), 'RenderGateway builds the current customer-masterdata Z01 draft')
assert(gateway.includes('renderAndQueueCustomerMasterdataZ01'), 'RenderGateway exposes the sanctioned customer-masterdata queue bridge')
assert(/requestType:\s*['"]customer_masterdata['"]/.test(gateway), 'customer-masterdata gateway finalizes under the customer_masterdata transport scope')
