...
assert(
  /transport_profile_id:\s*uuidOrNull/.test(routeEngine) && /transport_mode:\s*text\(profile\?\.transport_mode\)/.test(routeEngine),
  'routeDecisionEngine.ts: keeps transport_profile_id UUID-only and stores smtp_imap as transport_mode text',
)
assert(
  /tryUpdateGridOwnerDataRequestStatus/.test(prodat),
  'prodatCustomerMasterdata.ts: controlled blocker path does not depend on grid_owner_data_requests update success',
)
