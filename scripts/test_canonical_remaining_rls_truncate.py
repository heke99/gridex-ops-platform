...
SCRIPT = ROOT / 'scripts/canonical-remaining-rls-truncate-selftest.py'


def load():
    if not SCRIPT.is_file():
        raise AssertionError('remaining RLS TRUNCATE fixture is not implemented')
    spec = importlib.util.spec_from_file_location('remaining_rls_truncate', SCRIPT)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class RemainingRlsTruncateTests(unittest.TestCase):
    def test_selection_runs_without_database_tools(self):
        self.assertTrue(SCRIPT.is_file(), 'remaining RLS TRUNCATE fixture is not implemented')
    ...
    def test_exact_oracle_rejects_other_privilege_role_row_and_catalog_changes(self):
        ...
        before = dict(acl=[
            ['public', 'customer_case_events', 'postgres', 'authenticated', 'TRUNCATE', False],
            ['public', 'customer_case_events', 'postgres', 'authenticated', 'SELECT', False],
            ['public', 'customer_case_events', 'postgres', 'service_role', 'TRUNCATE', False],
            ['public', 'outside_targets', 'postgres', 'authenticated', 'TRUNCATE', False],
            ['control', 'customer_case_events', 'postgres', 'authenticated', 'TRUNCATE', False],
        ], rows={'public.customer_case_events': [{'id': 1, 'marker': 'retained'}]},
        ...
        for field, mutation in [
            ...
            ('row_value', lambda x: x['rows']['public.customer_case_events'][0].__setitem__('marker', 'changed')),
