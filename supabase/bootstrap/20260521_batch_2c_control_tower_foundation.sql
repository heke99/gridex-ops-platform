...
begin
  if to_regclass('public.billing_export_run_items') is not null then
    ...
    create unique index if not exists billing_export_run_items_company_idempotency_uidx on public.billing_export_run_items(company_id, idempotency_key) where idempotency_key is not null;
    create index if not exists billing_export_run_items_company_export_status_idx on public.billing_export_run_items(company_id, export_status, updated_at desc);
  end if;
end $$;
...
create index if not exists metering_period_gaps_company_status_idx on public.metering_period_gaps(company_id, status, period_month desc);
create index if not exists metering_period_gaps_company_point_idx on public.metering_period_gaps(company_id, metering_point_id, period_month desc);
...
select 'billing_export_item'::text as queue_type, bei.company_id, bei.id as source_id, bei.customer_id, bei.site_id, bei.metering_point_id,
  case when bei.status='blocked' then 'critical' else 'info' end as severity, bei.status,
  ...
  bei.created_at, coalesce(bei.updated_at, bei.created_at) as updated_at,
  jsonb_build_object('billing_export_run_id',bei.billing_export_run_id,'billing_underlay_id',bei.billing_underlay_id,'export_status',bei.export_status,'blocker_reasons',bei.blocker_reasons) as payload
from public.billing_export_run_items bei where bei.status='blocked'
union all
select 'metering_period_gap'::text, mpg.company_id, mpg.id, mpg.customer_id, mpg.site_id, mpg.metering_point_id, mpg.severity, mpg.status,
  'Saknade mätvärden'::text, 'Perioden saknar mätvärden och behöver begäras eller kompletteras.'::text,
  mpg.created_at, mpg.updated_at,
  jsonb_build_object('period_month',mpg.period_month,'period_start',mpg.period_start,'period_end',mpg.period_end,'outbound_request_id',mpg.outbound_request_id,'customer_case_id',mpg.customer_case_id)
from public.metering_period_gaps mpg where mpg.status in ('open','request_queued','waiting_for_data','failed')
union all
select 'customer_case'::text, cc.company_id, cc.id, cc.customer_id, cc.site_id, cc.metering_point_id,
  case when cc.priority in ('high','urgent') then 'critical' else 'warning' end, cc.status,
  coalesce(cc.title,'Kundärende'), coalesce(cc.description,'Kundärende kräver handläggning.'), cc.created_at, cc.updated_at,
  jsonb_build_object('case_type',cc.case_type,'reason_category',cc.reason_category,'billing_blocked',cc.billing_blocked)
from public.customer_cases cc where cc.status not in ('resolved','closed','cancelled')
union all
select 'partner_export'::text, pe.company_id, pe.id, pe.customer_id, pe.site_id, pe.metering_point_id,
  case when pe.status='failed' then 'critical' else 'warning' end, pe.status,
  'Partnerexport kräver uppföljning'::text, coalesce(pe.failure_reason,'Exporten har inte kvitterats färdigt.'), pe.created_at, pe.updated_at,
  jsonb_build_object('target_system',pe.target_system,'external_reference',pe.external_reference,'export_batch_key',pe.export_batch_key)
from public.partner_exports pe where pe.status in ('queued','failed')
union all
select 'external_contract_intake'::text, eci.company_id, eci.id, eci.created_customer_id, eci.created_site_id, eci.created_metering_point_id,
  case when eci.status='failed' then 'critical' else 'warning' end, eci.status,
  'Externt avtal kräver kontroll'::text,
  'Ett avtal från extern ingång behöver granskas, kompletteras eller kopplas vidare.'::text,
  eci.created_at, eci.updated_at,
  jsonb_build_object('email',eci.email,'facility_id',eci.facility_id,'issues',eci.issues)
from public.external_contract_intakes eci where eci.status in ('received','needs_review','failed');
