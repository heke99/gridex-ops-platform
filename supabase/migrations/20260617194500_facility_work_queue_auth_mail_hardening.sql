-- Follow-up: align facility work queue with backfilled power-of-attorney documents
-- and mask stored auth-email secrets in completed/dead-lettered outbox rows.
    ...
    (
      exists (
        ...
        where poa.customer_id = cs.customer_id
          ...
          and (
            nullif(poa.document_path, '') is not null
            or poa.signed_at is not null
            or poa.accepted_at is not null
            or nullif(poa.reference, '') is not null
            or poa.fullmakt_snapshot is not null
          )
      )
      or exists (
        select 1
        from public.customer_documents d
        where d.customer_id = cs.customer_id
          and coalesce(d.company_id, cs.company_id) = cs.company_id
          and d.document_type = 'power_of_attorney'
          and coalesce(d.status, 'available') in ('available','uploaded','signed','active')
      )
...
-- Auth emails may contain recovery tokens or temporary passwords. Keep delivery/audit
-- metadata, but mask raw bodies for already terminal rows so admin/debug views never
-- expose secrets after the send attempt is over.
update public.tenant_email_outbox
set
  html_body = '<p>Innehållet är maskat efter utskick/fel för att inte lagra återställningslänkar, tokens eller temporära lösenord i klartext.</p>',
  text_body = 'Innehållet är maskat efter utskick/fel för att inte lagra återställningslänkar, tokens eller temporära lösenord i klartext.',
  redirect_url = null,
  branding_snapshot = coalesce(branding_snapshot, '{}'::jsonb) || jsonb_build_object(
...
where email_type in ('password_reset','company_invite')
  ...
  and (
    coalesce(html_body, '') ~* '(token=|temporärt lösenord|temporary password|type=recovery|verify\?)'
    or coalesce(text_body, '') ~* '(token=|temporärt lösenord|temporary password|type=recovery|verify\?)'
    or coalesce(redirect_url, '') ~* '(token=|type=recovery|verify\?)'
  );
