-- Gridex OPS external API correctness/performance remediation foundation.
--
-- Forward-only. Do not edit historical migrations.
-- This migration keeps the existing schema and routes, removes verified exact
-- duplicate indexes, aligns portal-write idempotency with tenant/customer
-- semantics, adds keyset indexes for the existing Customer Portal tables and
-- exposes one service-role-only read RPC for the existing two-source document
-- feed.
...
-- ---------------------------------------------------------------------------
-- Preconditions: fail closed rather than silently applying to a drifted schema.
-- ---------------------------------------------------------------------------
do $$
declare
  v_table text;
...
-- Existing linked-customer uniqueness already exists in the current schema.
-- Add the pre-resolution claim key first, then remove the legacy global key
-- that incorrectly prevents two different customers from reusing the same
...
-- ---------------------------------------------------------------------------
-- Keyset-pagination indexes for the existing hot Customer Portal tables.
-- Tie-breaker id is required so rows sharing the same timestamp remain stable.
-- ---------------------------------------------------------------------------
...
create index if not exists normalized_metering_values_portal_keyset_idx
  on public.normalized_metering_values
    (company_id, customer_id, period_start desc, id desc);
...
-- ---------------------------------------------------------------------------
-- Correct database-level pagination for the existing merged document feed.
-- Customer documents retain precedence over authorization documents when both
-- represent the same power of attorney, matching the existing API behaviour.
-- Internal identifiers remain internal; the route maps them to opaque public
...
revoke all on function public.portal_customer_documents_page_v1(
  uuid, uuid, timestamptz, integer, uuid, integer
) from public, anon, authenticated;
grant execute on function public.portal_customer_documents_page_v1(
  uuid, uuid, timestamptz, integer, uuid, integer
) to service_role;

-- ---------------------------------------------------------------------------
-- Verified exact duplicate-index cleanup.
-- Constraint/FK-bearing indexes are deliberately retained. Only copies with
-- no constraint dependency are removed.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if to_regprocedure(
    'public.portal_customer_documents_page_v1(uuid,uuid,timestamp with time zone,integer,uuid,integer)'
  ) is null then
