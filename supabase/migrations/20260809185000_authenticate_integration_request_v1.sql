-- Collapse external API credential lookup, tenant-state validation, custom
-- Gridex scopes, IP/origin policy and the existing atomic route rate limiter
-- into one service-role-only database roundtrip. This does not introduce a new
-- auth model: it orchestrates the existing integration_api_clients + companies
-- + integration_api_rate_limit_check contracts.

begin;
set local search_path = public, pg_catalog;

create or replace function public.integration_api_scope_present_v1(
  p_scopes text[],
...
as $$
  select
    '*' = any(coalesce(p_scopes, array[]::text[]))
    or p_required_scope = any(coalesce(p_scopes, array[]::text[]))
    or (
      'customer_portal.read' = any(coalesce(p_scopes, array[]::text[]))
      and p_required_scope = any(array[
        'customer_profile.read','customer_sites.read','customer_contracts.read',
        'customer_invoices.read','customer_metering.read','customer_legal.read',
        'customer_events.read','customer_documents.read',
        'customer_notifications.read','customer_power_of_attorney.read'
      ]::text[])
    )
    or (
      'customer_portal.write' = any(coalesce(p_scopes, array[]::text[]))
      and p_required_scope = any(array[
        'customer_sync.write','customer_contact.write',
        'customer_facility_data.write','customer_power_of_attorney.write',
        'customer_notifications.write','customer_documents.write'
      ]::text[])
    );
$$;

revoke all on function public.integration_api_scope_present_v1(text[],text)
  from public, anon, authenticated;
grant execute on function public.integration_api_scope_present_v1(text[],text)
  to service_role;
...
create or replace function public.authenticate_integration_request_v1(
  ...
  p_route_limit integer default 60,
...
begin
  ...
  if v_client.status <> 'active' or v_client.revoked_at is not null then
    return query select
      'denied','api_client_inactive',v_tenant_status,
      ...
      v_client.expires_at,null::integer,p_route_limit,null::timestamptz;
    return;
  end if;

  if v_client.expires_at is not null and v_client.expires_at <= clock_timestamp() then
    return query select
      'denied','api_token_expired',v_tenant_status,
      v_client.id,v_client.company_id,v_client.name,v_client.status,
      v_client.key_prefix,v_client.secret_hash,v_client.scopes,v_client.allowed_ips,
      v_client.allowed_origins,v_client.metadata,v_client.rate_limit_per_minute,
      v_client.expires_at,null::integer,p_route_limit,null::timestamptz;
  ...
  if v_tenant_status is null then
    return query select
      'unavailable','tenant_status_unavailable',null::text,
      v_client.id,v_client.company_id,v_client.name,v_client.status,
      v_client.key_prefix,v_client.secret_hash,v_client.scopes,v_client.allowed_ips,
      v_client.allowed_origins,v_client.metadata,v_client.rate_limit_per_minute,
      v_client.expires_at,null::integer,p_route_limit,null::timestamptz;
  ...
  if v_tenant_status <> 'active' then
    return query select
      'denied','tenant_' || v_tenant_status,v_tenant_status,
      v_client.id,v_client.company_id,v_client.name,v_client.status,
      v_client.key_prefix,v_client.secret_hash,v_client.scopes,v_client.allowed_ips,
      v_client.allowed_origins,v_client.metadata,v_client.rate_limit_per_minute,
      v_client.expires_at,null::integer,p_route_limit,null::timestamptz;
  ...
  foreach v_required in array coalesce(p_required_all,array[]::text[]) loop
    if not public.integration_api_scope_present_v1(v_client.scopes,v_required) then
      return query select
        'denied','api_scope_missing',v_tenant_status,
        v_client.id,v_client.company_id,v_client.name,v_client.status,
        v_client.key_prefix,v_client.secret_hash,v_client.scopes,v_client.allowed_ips,
        v_client.allowed_origins,v_client.metadata,v_client.rate_limit_per_minute,
        v_client.expires_at,null::integer,p_route_limit,null::timestamptz;
  ...
  if cardinality(coalesce(v_client.allowed_ips,array[]::text[])) > 0 then
    ...
    if not v_ip_ok then
      return query select
        'denied','api_ip_not_allowed',v_tenant_status,
        v_client.id,v_client.company_id,v_client.name,v_client.status,
        v_client.key_prefix,v_client.secret_hash,v_client.scopes,v_client.allowed_ips,
        v_client.allowed_origins,v_client.metadata,v_client.rate_limit_per_minute,
        v_client.expires_at,null::integer,p_route_limit,null::timestamptz;
  ...
  if cardinality(coalesce(v_client.allowed_origins,array[]::text[])) > 0
    ...
    return query select
      'denied','api_origin_not_allowed',v_tenant_status,
      v_client.id,v_client.company_id,v_client.name,v_client.status,
      v_client.key_prefix,v_client.secret_hash,v_client.scopes,v_client.allowed_ips,
      v_client.allowed_origins,v_client.metadata,v_client.rate_limit_per_minute,
      v_client.expires_at,null::integer,p_route_limit,null::timestamptz;
  ...
  if p_route_limit is null or p_route_limit <= 0 then
    return query select
      'unavailable','api_rate_limit_invalid',v_tenant_status,
      v_client.id,v_client.company_id,v_client.name,v_client.status,
      v_client.key_prefix,v_client.secret_hash,v_client.scopes,v_client.allowed_ips,
      v_client.allowed_origins,v_client.metadata,v_client.rate_limit_per_minute,
      v_client.expires_at,null::integer,p_route_limit,null::timestamptz;
