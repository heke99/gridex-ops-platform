-- Remaining Gridex OPS masterpoint convergence.
-- Extends the existing canonical runtime; no parallel ownership model is introduced.

begin;

-- -----------------------------------------------------------------------------
-- Manual-review lifecycle: make existing inbound work actionable and auditable.
-- -----------------------------------------------------------------------------
...
set review_owner = coalesce(nullif(review_owner, ''), 'tenant_operations'),
    ...
    review_reason = coalesce(
      ...
      'legacy_manual_review_unclassified'
    ),
...
do $$
begin
  if to_regclass('public.inbound_processing_jobs') is not null
     and not exists (
       select 1 from pg_constraint
       where conrelid = 'public.inbound_processing_jobs'::regclass
         and conname = 'inbound_processing_jobs_review_priority_check'
     ) then
    alter table public.inbound_processing_jobs
      add constraint inbound_processing_jobs_review_priority_check
      check (review_priority is null or review_priority in ('low', 'normal', 'high', 'critical'))
      not valid;
    alter table public.inbound_processing_jobs
...
create index if not exists inbound_processing_jobs_manual_review_queue_idx
  on public.inbound_processing_jobs(company_id, review_sla_due_at, created_at)
  where status = 'manual_review' and review_resolved_at is null;

comment on column public.inbound_processing_jobs.review_owner is
  'Operational owner for manual-review work. This is an operational responsibility, not a fabricated user identity.';
comment on column public.inbound_processing_jobs.review_sla_due_at is
  'Explicit SLA deadline for unresolved manual-review work.';

-- -----------------------------------------------------------------------------
-- Account anonymization: keep the existing entry point but make authorization
-- cleanup atomic with profile anonymization and revoke live sessions.
-- -----------------------------------------------------------------------------
...
begin
  ...
  if to_regclass('public.user_permission_overrides') is not null then
    execute 'delete from public.user_permission_overrides where user_id = $1'
      using target_user_id;
  end if;
  ...
  if to_regclass('public.company_memberships') is not null then
    execute 'delete from public.company_memberships where user_id = $1'
      using target_user_id;
  end if;
  ...
  if to_regclass('public.company_invitations') is not null
     and exists (
    ...
    if exists (
      ...
      execute 'update public.company_invitations set invited_user_id = null where invited_user_id = $1'
        using target_user_id;
    else
      execute 'delete from public.company_invitations where invited_user_id = $1'
        using target_user_id;
    end if;
  end if;

  -- Revoke existing authenticated sessions without deleting the auth identity.
  -- The identity itself remains so the product can retain its established
  -- account-closure semantics while authorization state is removed atomically.
...
-- -----------------------------------------------------------------------------
-- Organisation number: org_number is the stored source of truth. The legacy
-- organization_number field remains a compatibility projection only.
-- -----------------------------------------------------------------------------
...
begin
  if tg_op = 'INSERT' then
    new.org_number := coalesce(nullif(btrim(new.org_number), ''), nullif(btrim(new.organization_number), ''));
  elsif new.org_number is distinct from old.org_number then
    new.org_number := nullif(btrim(new.org_number), '');
  elsif new.organization_number is distinct from old.organization_number then
    -- Legacy writers are accepted as an input alias, then immediately projected
    -- into the canonical org_number field.
    new.org_number := nullif(btrim(new.organization_number), '');
  end if;
...
drop trigger if exists companies_canonical_org_number on public.companies;
create trigger companies_canonical_org_number
before insert or update of org_number, organization_number on public.companies
for each row execute function public.gridex_canonicalize_company_org_number();

update public.companies
set org_number = coalesce(nullif(btrim(org_number), ''), nullif(btrim(organization_number), '')),
    organization_number = coalesce(nullif(btrim(org_number), ''), nullif(btrim(organization_number), ''))
where org_number is distinct from organization_number
   or (org_number is null and organization_number is not null);

comment on column public.companies.org_number is
  'Canonical stored organization number. normalized_org_number is derived from this value.';
comment on column public.companies.organization_number is
  'Deprecated compatibility projection of org_number; kept synchronized by companies_canonical_org_number.';

-- -----------------------------------------------------------------------------
-- Release identity: extend the existing receipt rather than creating another
-- release-tracking system. Historical receipts remain valid; new releases can
-- persist complete immutable evidence.
-- -----------------------------------------------------------------------------

alter table if exists public.platform_release_receipts
...
create index if not exists platform_release_receipts_environment_recorded_idx
  on public.platform_release_receipts(environment, recorded_at desc);

comment on column public.platform_release_receipts.database_schema_fingerprint is
  'Stable schema fingerprint captured by the release process for post-deploy parity verification.';
comment on column public.platform_release_receipts.reconciliation_result is
  'Tenant-scoped reconciliation result captured for this exact release identity.';

-- -----------------------------------------------------------------------------
-- Reconciliation: extend the existing canonical function and findings table.
-- Every check remains tenant-scoped and writes through canonical_set_architecture_finding.
-- Optional/legacy relations are guarded so clean replay remains deterministic.
-- -----------------------------------------------------------------------------
...
revoke all on function public.canonical_run_architecture_reconciliation(uuid) from public;
