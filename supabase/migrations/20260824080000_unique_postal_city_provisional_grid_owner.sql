...
begin
  -- Canonical ownership and explicit prior selection always win. This trigger
  -- only provides a provisional recipient for facility-information routing.
  ...
  -- Re-check live masterdata instead of trusting metadata. Any ambiguity keeps
  -- the site unresolved and requires the existing review/resolution path.
  ...
  select upper(regexp_replace(m.grid_area_code, '\s+', '', 'g')),
         ...
         pgo.name,
         m.confidence
    into v_grid_area_code, v_grid_area_name, v_price_area,
         v_ops_grid_owner_id, v_grid_owner_name, v_mapping_confidence
  from public.platform_postal_code_grid_mappings m
  ...
  if v_grid_area_code is null
     ...
     or upper(coalesce(v_resolution ->> 'priceArea', v_price_area)) <> upper(v_price_area)
...
-- Repair active unresolved sites through the same fail-closed trigger.
...
'Selects a provisional OPS grid owner for facility-information routing only when exact postcode+city resolver evidence and live masterdata both contain one conflict-free candidate. Canonical grid_owner_id and grid_area_code remain unresolved until facility data verifies them.';
