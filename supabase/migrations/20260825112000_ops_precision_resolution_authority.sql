...
begin
  ...
  set status='completed',dispatch_status='completed',received_at=v_now,completed_at=v_now,facility_id=v_facility_id,metering_point_id=v_meter_external,grid_area_code=v_grid_area,price_area=v_price_area,resolution_id=v_resolution_id,received_payload=v_received,metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('completed_by',p_source,'completed_at',v_now,'matched_ediel_message_id',p_ediel_message_id,'atomic_completion',true,'resolution_id',v_resolution_id),updated_at=v_now,updated_by=p_actor_user_user_id
  where id=p_request_id and company_id=p_company_id;
