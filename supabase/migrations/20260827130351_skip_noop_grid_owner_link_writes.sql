...
begin
  ...
  -- Link existing rows to registry/resolver records, but skip rows whose persisted link values are already identical.
  ...
  where v.grid_owner_id = g.id
    and (
      g.platform_market_actor_id is distinct from coalesce(g.platform_market_actor_id, v.platform_market_actor_id)
      or g.platform_grid_owner_id is distinct from coalesce(g.platform_grid_owner_id, v.platform_grid_owner_id)
      or g.ediel_id is distinct from coalesce(nullif(g.ediel_id, ''), v.ediel_id)
      or g.org_number is distinct from coalesce(nullif(g.org_number, ''), v.org_number)
    );
