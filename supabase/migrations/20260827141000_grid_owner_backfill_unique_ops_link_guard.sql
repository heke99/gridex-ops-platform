-- Harden grid-owner verification backfill so platform↔OPS linking and
-- verification writes only apply when match sets are unique.
-- Aligns with gridex_reconcile_grid_owner_mappings_v1 (candidate_count = 1,
-- target_count = 1) and prevents ambiguous UNION matches from flipping
-- ops_grid_owner_id or persisting nondeterministic verification rows.
...
begin
  ...
  -- Link existing rows only when the verified view yields a single row per owner.
  ...
  if to_regclass('public.platform_grid_owners') is not null then
    ...
    classified as (
      select
        ...
        case
          when count(distinct m.grid_owner_id) = 1
            then min(m.grid_owner_id::text)::uuid
          else null::uuid
        end as grid_owner_id
      from matched m
      group by m.pgo_id
    ),
    target_counts as (
    ...
    unique_matched as (
      ...
      where c.match_count = 1
        and c.grid_owner_id is not null
        and tc.target_count = 1
    )
  ...
  where uv.verification_status <> 'verified'
    and not exists (
      ...
      where r.grid_owner_id = uv.grid_owner_id
        and r.issue_type = uv.verification_status
        and r.status = 'open'
    );
...
-- Preserve service-role-only execute semantics after CREATE OR REPLACE.
