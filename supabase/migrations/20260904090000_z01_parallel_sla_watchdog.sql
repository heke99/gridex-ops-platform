-- PRODAT Z01 parallel SLA watchdog
--
-- Canonical behaviour:
--   * technical CONTRL and business Z02/negative APERAK are monitored independently
--   * both clocks start from the actual persisted message_sent_at
--   * overdue Z01 never auto-resends; it creates typed operator escalation instead
--   * customer business state waits for Z02 while technical ACK state remains on ediel_messages
--
-- Swedish Elmarknadshandbok 26A section 10.2.1: Z02 L/LK within 30 minutes.

begin;

alter table public.ediel_messages
  add column if not exists contrl_due_at timestamptz,
  add column if not exists business_response_due_at timestamptz,
  add column if not exists response_overdue_at timestamptz;

comment on column public.ediel_messages.contrl_due_at is
  'Canonical technical CONTRL deadline projected from actual message_sent_at.';
comment on column public.ediel_messages.business_response_due_at is
  'Canonical business-response deadline. For outbound PRODAT Z01 this is Z02 or negative APERAK.';
comment on column public.ediel_messages.response_overdue_at is
  'First time the canonical business-response SLA was escalated. Does not trigger resend.';

create index if not exists ediel_messages_z01_business_response_due_idx
  on public.ediel_messages (business_response_due_at, company_id)
  where direction = 'outbound'
    and upper(message_family) = 'PRODAT'
    and upper(coalesce(message_code,'')) = 'Z01'
    and message_sent_at is not null;

-- Repair stale technical ACK deadlines from historical prepare/queue time.
-- Once a message has actually been sent, message_sent_at is the only valid base.
update public.ediel_messages
set
  contrl_due_at = case
    when requires_contrl = true then message_sent_at + interval '30 minutes'
    else null
  end,
  ack_due_at = case
    when requires_contrl = true and contrl_status not in ('received','sent')
      then message_sent_at + interval '30 minutes'
    else null
  end,
  updated_at = now()
where direction = 'outbound'
  and message_sent_at is not null
  and requires_contrl = true
  and (
    contrl_due_at is distinct from message_sent_at + interval '30 minutes'
    or (
      contrl_status not in ('received','sent')
      and ack_due_at is distinct from message_sent_at + interval '30 minutes'
    )
  );

-- Business response SLA for Z01 is independent from CONTRL.
update public.ediel_messages
set
  business_response_due_at = message_sent_at + interval '30 minutes',
  updated_at = now()
where direction = 'outbound'
  and upper(message_family) = 'PRODAT'
  and upper(coalesce(message_code,'')) = 'Z01'
  and message_sent_at is not null
  and business_response_due_at is distinct from message_sent_at + interval '30 minutes';

-- Customer/business state should wait for Z02/negative APERAK directly.
-- CONTRL is tracked independently on ediel_messages and must not serialize Z02.
update public.customer_info_requests cir
set
  status = 'waiting_for_z02',
  next_required_action = 'Invänta Z02 eller negativ APERAK från nätägaren. CONTRL bevakas parallellt från faktisk Z01-sändtid.',
  updated_at = now()
from public.ediel_messages m
where cir.ediel_message_id = m.id
  and cir.company_id = m.company_id
  and m.direction = 'outbound'
  and upper(m.message_family) = 'PRODAT'
  and upper(coalesce(m.message_code,'')) = 'Z01'
  and cir.status in ('waiting_for_contrl','waiting_for_aperak')
  and cir.response_ediel_message_id is null;
  ...
  loop
    ...
    if rec.requires_contrl = true
      ...
      if not exists (
        ...
        where e.ediel_message_id = rec.ediel_message_id
          and e.event_type = 'contrl_sla_breached'
      ) then
    ...
    if rec.business_response_due_at is not null
      ...
      if not exists (
        select 1 from public.ediel_message_events e
        where e.ediel_message_id = rec.ediel_message_id
          and e.event_type = 'business_response_sla_breached'
      ) then
        insert into public.ediel_message_events (
          company_id, ediel_message_id, event_type, event_status, message, payload
        )
        values (
          rec.company_id, rec.ediel_message_id,
          'business_response_sla_breached', 'warning',
...
-- Keep the RPC internal. Service-role cron can execute it; browser roles cannot.
