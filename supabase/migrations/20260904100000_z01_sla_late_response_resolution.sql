-- Resolve previously opened Z01 SLA escalations when late inbound evidence arrives.
-- The watchdog preserves breach history while closing only the dimension that is now satisfied:
-- CONTRL resolves CONTRL; Z02/negative APERAK resolves the business-response SLA.
...
comment on function public.gridex_escalate_overdue_z01_responses(integer) is
  'Escalates overdue outbound PRODAT Z01 technical/business response SLAs without resending and resolves late-response alarms dimension-by-dimension.';
