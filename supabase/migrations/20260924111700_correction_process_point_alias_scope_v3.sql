-- Prospective process readset: resolve UUID point links for every producer that
-- stores the metering point row ID. A missing historical point remains a wildcard.
-- Only this private read function changes; published migrations stay immutable.
BEGIN;
CREATE OR REPLACE FUNCTION gridex_correction_process.open_readset_v1(
   ...
   AND (p_point_id IS NULL OR EXISTS (
    ...
    AND (coalesce(nullif(side.v->>'metering_point_id',''),nullif(side.v->>'meter_point_id',''),
     ...
     OR (f.table_name IN ('customer_supply_periods','customer_contracts',
      ...
      AND nullif(side.v->>'metering_point_id','') IS NOT NULL
