-- Forward-only immutable source scope projection for bounded cold reads.
BEGIN;
-- Only a complete, single-object PRODAT envelope establishes a point;
-- malformed, multi-object and other families remain unknown wildcards.
...
BEGIN
 ...
 OR (SELECT count(*) FROM jsonb_array_elements(tokens) t WHERE t->>'tag'='BGM'
  AND t#>>'{elements,1,0}' IN ('Z04','Z05'))<>1
...
-- Compute the immutable wire projection at capture/replay insertion, so a
-- budget read does not parse every unrelated source twice under the runtime
-- snapshot timeout. NULL means unknown and is always included in the budget.
...
CREATE FUNCTION gridex_received_sources.selection_body_v5(
...
BEGIN
 ...
 WITH candidates AS MATERIALIZED (
  ...
  WHERE s.company_id=p_company_id AND s.environment=p_environment AND s.captured_at<=p_cutoff
   ...
   AND (p_point_ids IS NULL OR s.scope_point IS NULL
    OR s.scope_point=ANY(p_point_ids))
 ...
 RETURN body;
END $$;
REVOKE ALL ON FUNCTION gridex_received_sources.selection_body_v5(uuid,text,timestamptz,timestamptz,text[])
...
BEGIN
 ...
 SELECT gridex_received_sources.selection_body_v5(p_company_id,p_environment,p_cutoff,observed,aliases.points),
