-- Bind the LK exemption to the physical Z08 BGM, not only the mutable row profile.
-- The H dispatch fence still holds malformed or inconsistent originals before provider entry.
BEGIN;
...
BEGIN
 IF action='prepare' THEN
  ...
  IF o.message_id IS NULL AND m.direction='outbound' AND m.message_family='PRODAT'
  ...
  THEN RETURN jsonb_build_object('scoped',false,'unscopedReason','canonical_lk_exemption'); END IF;
